	...
	<table style="width:760px">
		...
		<tr>
			<td style="width:70%;vertical-align:top;" class="left">
				{$List@iteration}.: 
				{if !($isBusy.research && ($ID == 6 || $ID == 31)) && !($isBusy.shipyard && ($ID == 15 || $ID == 21)) && !($isBusy.defense && ($ID == 15 || $ID == 20)) && $RoomIsOk && $CanBuildElement && $BuildInfoList[$ID].buyable}
...
<table>
	...
	<tr>
		<th colspan="2">
			<a href="#" onclick="return Dialog.info({$ID})">{$LNG.tech.{$ID}}</a>{if $Element.level > 0} ({$LNG.bd_lvl} {$Element.level}){/if}
		</th>
	</tr>
	<tr>
		...
		<td>
			<table style="width:100%">
				<tr>
					<td class="transparent middle" style="width:90%;padding:10px;">{$LNG.shortDescription.{$ID}}
					<br>{foreach $Element.costRessources as $RessID => $RessAmount}
					<br>{$LNG.tech.{$RessID}}: <span style="color:{if $Element.costOverflow[$RessID] == 0}lime{else}red{/if}">{$RessAmount|number}</span>
					{/foreach}</td>
				</tr>
					...
					{elseif ($isBusy.research && ($ID == 6 || $ID == 31)) || ($isBusy.defense && ($ID == 15 || $ID == 20)) || ($isBusy.shipyard && ($ID == 15 || $ID == 21))}
					...
					{else}
						{if $RoomIsOk}
							...
							<form action="game.php?page=buildings" method="post" class="build_form">
								<input type="hidden" name="cmd" value="insert">
								<input type="hidden" name="building" value="{$ID}">
								<button type="submit" class="build_submit"><b>{if $Element.level == 0}{$LNG.bd_build}{else}{$LNG.bd_build_next_level}{$Element.levelToBuild + 1}{/if}</b></button>
							</form>
							{else}
							<span style="color:red"><b>{if $Element.level == 0}{$LNG.bd_build}{else}{$LNG.bd_build_next_level}{$Element.levelToBuild + 1}{/if}</b></span>
							{/if}
						{else}
						<span style="color:red"><b>{$LNG.bd_no_more_fields}</b></span>
						{/if}
					{/if}
					</td>
			</table>
			...
			<table style="width:100%">
				<tr>
					<td class="transparent left">
						<font color="#0871a2"><b>{$LNG.bd_remaining}</b></font>
						...
						{if !empty($Element.infoEnergy)}
							<font color="#0871a2"><b>{$LNG.bd_next_level}</b></font>
							{$Element.infoEnergy}
						{/if}
					</td>
					<td class="transparent">
						<font color="orange"><b>{$LNG.fgf_time}:</b></font><br>{$Element.elementTime|time}
					</td>
				</tr>
			</table>
		</td>
				...
				<tr>		
					...
					{if $Element.level > 0}
							...
							{if ($ID == 44 && !$HaveMissiles) ||  $ID != 44}<br><a class="tooltip_sticky" data-tooltip-content="
								...
								"><font color="#0871a2"><b>{$LNG.bd_dismantle}</b></font></a>{/if}
						{else}
							&nbsp;
						{/if}
					</td>
