...
<tr>
	...
	<td>{if $RangeInfo.id != $CUser_id}<a href="#" onclick="return Dialog.Buddy({$RangeInfo.id});"><img src="{$dpath}img/b.gif" style="background:white" title="{$LNG.pl_buddy}" alt="{$LNG.pl_buddy}"></a>{/if}</td>
	...
	<td>{$RangeInfo.race}</td>
	<td style="color:orange">{$RangeInfo.points}</td>
	<td>{$RangeInfo.Erfolge}</td>
</tr>
{/foreach}