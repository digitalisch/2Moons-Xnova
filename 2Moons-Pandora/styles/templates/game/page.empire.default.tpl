...
<table style="width:100%;">
	<tbody>
		...
		<tr>
			<td style="color:gold">{$LNG.tech.$elementID}</td>
			<td style="color:lime">{array_sum($buildArray)|number}</td>
			{foreach $buildArray as $planetID => $build}
				<td style="color:lime">{$build|number}</td>
			{/foreach}
		</tr>
		{/foreach}
		<tr>
		...
		<tr>
			<td style="color:gold">{$LNG.tech.$elementID}</td>
			<td style="color:lime">{$tech|number}</td>
			{foreach $planetList.name as $name}
				<td style="color:lime">{$tech|number}</td>
			{/foreach}
		</tr>
		...
		<tr>
			<td style="color:gold">{$LNG.tech.$elementID}</td>
			<td style="color:lime">{array_sum($fleetArray)|number}</td>
			{foreach $fleetArray as $planetID => $fleet}
				<td style="color:lime">{$fleet|number}</td>
			{/foreach}
		</tr>
		{/foreach}
		<tr>
