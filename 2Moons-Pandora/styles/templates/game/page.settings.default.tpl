...
<input type="hidden" name="mode" value="send">
	...
	<tbody>
		...
		<tr>
			<td width="50%">{$LNG.ov_race}</td>
			<td width="50%" style="height:22px;">
				{if $changeRaceTime < 0 && $raceChange == 1}<select name="race" id="race">{html_options options=$raceSelect}</select>{else}{$race}{/if}
				<input name="raceChange" type="checkbox" value="1" {if $raceChange == 1}checked="checked"{/if}>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td>{$LNG.op_signature}</td>
			<td><textarea name="signature" id="text" cols="80" rows="5" value="{$signature}" type="text">{$signature}</textarea></td>
		</tr>
		<tr>
		<td>{$LNG.user_image}</td>
		<td><input type="text" name="image" value="{$user_image}" size="70"></td>
		</tr>
		<tr>
