...
{block name="content"}
		...
		<tr>
			<td class="transparent">
				<table>
					...
					<tr>
						<td><img src="{$dpath}img/left.png" title="Links" input type="button" name="galaxyLeft" value="&lt;-" onclick="galaxy_submit('galaxyLeft')"></td>
						<td><input type="text" name="galaxy" value="{$galaxy}" size="5" maxlength="3" tabindex="1"></td>
						<td><img src="{$dpath}img/right.png" title="Rechts" input type="button" name="galaxyRight" value="-&gt;" onclick="galaxy_submit('galaxyRight')"></td>
					</tr>
				</table>
			</td>
			<td class="transparent">
				<table>
					...
					<tr>
						<td><img src="{$dpath}img/left.png" title="Links" input type="button" name="systemLeft" value="&lt;-" onclick="galaxy_submit('systemLeft')"></td>
						<td><input type="text" name="system" value="{$system}" size="5" maxlength="3" tabindex="2"></td>
						<td><img src="{$dpath}img/right.png" title="Rechts" input type="button" name="systemRight" value="-&gt;" onclick="galaxy_submit('systemRight')"></td>
					</tr>
				</table>
			</td>
		</tr>
    ...
    {else}
		...
		<td>
			<a class="tooltip_sticky" data-tooltip-content="<table style='width:240px;background-color: rgba(0,0,0, 1.0);'><tr><th colspan='2'>{$currentPlanet.user.playerrank}</th></tr>
			<tr>{if !$currentPlanet.ownPlanet}{if $currentPlanet.user.isBuddy}<tr><td><a href='#' onclick='return Dialog.Buddy({$currentPlanet.user.id})'>{$LNG.gl_buddy_request}</a></td></tr>{/if}
			<tr><td><a href='#' onclick='return Dialog.Playercard({$currentPlanet.user.id});'>{$LNG.gl_playercard}</a></td></tr>{/if}
			<tr><td><a href='?page=statistics&amp;who=1&amp;start={$currentPlanet.user.rank}'>{$LNG.gl_see_on_stats}</a></td></tr></table>">
		...
		<td style="white-space: nowrap;">
			...
			<a class="tooltip_sticky" data-tooltip-content="<table style='width:240px;background-color: rgba(0,0,0, 1.0);'><tr><th>{$LNG.gl_alliance} {$currentPlanet.alliance.name} {$currentPlanet.alliance.member}</th></tr>
			<td><table><tr><td><a href='?page=alliance&amp;mode=info&amp;id={$currentPlanet.alliance.id}'>{$LNG.gl_alliance_page}</a></td></tr>
			<tr><td><a href='?page=statistics&amp;start={$currentPlanet.alliance.rank}&amp;who=2'>{$LNG.gl_see_on_stats}</a></td></tr>
			{if $currentPlanet.alliance.web}<tr><td><a href='{$currentPlanet.alliance.web}' target='allyweb'>{$LNG.gl_alliance_web_page}</td></tr>{/if}</table></td></table>">
	...
	<tr>
		...
		<td colspan="2">
			<a class="tooltip" data-tooltip-content="<table style='width:240px;background-color: rgba(0,0,0, 1.0);'><tr><th colspan='2'>{$LNG.gl_legend}</td></tr><tr><td style='width:220px'>{$LNG.gl_strong_player}</td><td><span class='galaxy-short-strong'>{$LNG.gl_short_strong}</span></td></tr><tr><td style='width:220px'>{$LNG.gl_week_player}</td><td><span class='galaxy-short-noob'>{$LNG.gl_short_newbie}</span></td></tr><tr><td style='width:220px'>{$LNG.gl_vacation}</td><td><span class='galaxy-short-vacation'>{$LNG.gl_short_vacation}</span></td></tr><tr><td style='width:220px'>{$LNG.gl_banned}</td><td><span class='galaxy-short-banned'>{$LNG.gl_short_ban}</span></td></tr><tr><td style='width:220px'>{$LNG.gl_inactive_seven}</td><td><span class='galaxy-short-inactive'>{$LNG.gl_short_inactive}</span></td></tr><tr><td style='width:220px'>{$LNG.gl_inactive_twentyeight}</td><td><span class='galaxy-short-longinactive'>{$LNG.gl_short_long_inactive}</span></td></tr><tr><td style='width:220px'>{$LNG.gl_admin}</td><td><span class='galaxy-short-admin'>{$LNG.gl_short_admin}</span></td></tr><tr><td style='width:220px'>{$LNG.gl_ops}</td><td><span class='galaxy-short-ops'>{$LNG.gl_short_ops}</span></td></tr><tr><td style='width:220px'>{$LNG.gl_mod}</td><td><span class='galaxy-short-mod'>{$LNG.gl_short_mod}</span></td></tr></table>">{$LNG.gl_legend}</a> 
		</td>
	</tr>
	<tr>
		<td colspan="8"><span id="slots">{$maxfleetcount}</span>/{$fleetmax} {$LNG.gl_fleets}</td>
	</tr>
	<tr>
		<td colspan="4"><span id="missiles">{$currentmip|number}</span> {$LNG.gl_avaible_missiles}</td>
		<td colspan="4"><span id="elementID210">{$spyprobes|number}</span> {$LNG.gl_avaible_spyprobes}</td>
	</tr>
	<tr>
		<td colspan="4"><span id="elementID209">{$recyclers|number}</span> {$LNG.gl_avaible_recyclers}</td>
		<td colspan="4"><span id="elementID219">{$grecyclers|number}</span> {$LNG.gl_avaible_grecyclers}</td>
	</tr>
