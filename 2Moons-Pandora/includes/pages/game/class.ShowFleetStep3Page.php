...
class ShowFleetStep3Page extends AbstractPage
{
	...
	public function show()
	{
		...
		if($targetPlayerData['onlinetime'] > TIMESTAMP - INACTIVE && $targetMission == 1 || $targetMission == 2 || $targetMission == 9) {
			if(FleetFunctions::CheckBash($targetPlanetData['id'])) {
				$this->printMessage($LNG['fl_bash_protection']);
			}
		}

		if ($targetMission == 5 || $targetMission == 3)
		{	
		...
		$buddy	= $GLOBALS['DATABASE']->getFirstCell("
				...
				(owner = ".$targetPlayerData['id']." AND sender = ".$USER['id'].") OR
				(owner = ".$USER['id']." AND sender = ".$targetPlayerData['id'].");");
		if($targetMission == 1 || $targetMission == 2 || $targetPlayerData['ally_id'] != $USER['ally_id'] && $buddy < 0 && $targetMission == 5 || $targetMission == 6 || $targetMission == 9) {
