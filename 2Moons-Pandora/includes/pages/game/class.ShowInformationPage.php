...
class ShowInformationPage extends AbstractPage
{
	...
	public function show()
	{
		...
		$costTable = array();        
		if (!in_array($elementID, $reslist['officier'])) {            
		if (isset($PLANET[$resource[$elementID]])) {                
		$CurrentLevel = $PLANET[$resource[$elementID]];            } 
		else if ($USER[$resource[$elementID]]) {                
		$CurrentLevel = $USER[$resource[$elementID]];            }            
		if (in_array($elementID, $reslist['fleet']) || in_array($elementID, 
		$reslist['defense'])) {                
		$CurrentLevel = 0;            }            
		$CostLevel = max($CurrentLevel + 1, 1);            
		$CostStartLvl = $CostLevel;
            for ($CostLevel; $CostLevel < $CostStartLvl + 3; $CostLevel++) {                
			$costTable['costs'][$CostLevel] = BuildFunctions::getElementPrice($USER, $PLANET, $elementID, false, $CostLevel);                
			if (in_array($elementID, $reslist['fleet']) || in_array($elementID, $reslist['defense']) || in_array($elementID, $reslist['missile'])) {                    
			break;                
			}            
			}            
			$costTable['usedResource'] = array_keys($costTable['costs'][$CostStartLvl]);        }
