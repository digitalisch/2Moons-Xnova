...
abstract class AbstractPage 
{
    ...
    {
		global $PLANET, $LNG, $USER, $THEME, $resource, $reslist, $CONF;
		...
		$Alarm       = $GLOBALS['DATABASE']->uniquequery("SELECT `fleet_mission` FROM ".FLEETS." WHERE `fleet_target_owner` = '". $USER['id'] ."' AND (`fleet_mission` = '1' OR `fleet_mission` = '2' OR `fleet_mission` = '6') AND `fleet_mess` ='0';");
            ...
            $this->tplObj->assign_vars(array(
                        'scancheck'=>'<img src="styles/resource/images/red.gif" >',
                        ));
            }
            if($Alarm['fleet_mission'] != 1 && $Alarm['fleet_mission'] != 2 && $Alarm['fleet_mission'] != 6){
            $this->tplObj->assign_vars(array(
                        'scancheck'=>'<img src="styles/resource/images/green.gif" >',
                        ));
                }
				$Alarm1       = $GLOBALS['DATABASE']->uniquequery("SELECT `fleet_mission` FROM ".FLEETS." WHERE `fleet_target_owner` = '". $USER['id'] ."' AND `fleet_mission` = '6' AND `fleet_mess` ='0';");
            if($Alarm1['fleet_mission'] == 6){
            $this->tplObj->assign_vars(array(
                        'scancheck'=>'<img src="styles/resource/images/blue.gif" >',
                        ));
            }
