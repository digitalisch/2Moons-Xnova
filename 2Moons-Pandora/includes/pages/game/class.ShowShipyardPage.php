...
class ShowShipyardPage extends AbstractPage
{
	...
	private function CancelAuftr() 
	{
		...
		if ($Element == 214 && $USER['rpg_destructeur'] == 1){
			if(isset($costRessources[901])) { $PLANET[$resource[901]]	+= $costRessources[901] * FACTOR_CANCEL_SHIPYARD /2; }
			if(isset($costRessources[902])) { $PLANET[$resource[902]]	+= $costRessources[902] * FACTOR_CANCEL_SHIPYARD /2; }
			if(isset($costRessources[903])) { $PLANET[$resource[903]]	+= $costRessources[903] * FACTOR_CANCEL_SHIPYARD /2; }
			if(isset($costRessources[921])) { $USER[$resource[921]]		+= $costRessources[921] * FACTOR_CANCEL_SHIPYARD /2; }
	...
	private function BuildAuftr($fmenge)
	{
		...
		{
			if(empty($Count)
				|| !in_array($Element, array_merge($reslist['fleet']))
