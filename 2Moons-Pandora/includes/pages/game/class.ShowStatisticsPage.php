    ...
    function show()
    {
        ...
        switch($who)
        {
            case 1:
                ...
                $stats_sql	=	'SELECT DISTINCT s.*, u.id, u.username,u.race, u.Erfolge, u.ally_id, r.race_id, r.race_name, a.ally_name FROM '.STATPOINTS.' as s
                INNER JOIN '.USERS.' as u ON u.id = s.id_owner
                INNER JOIN '.RACES.' as r ON r.race_id = u.race
                ...
                {
                    $RangeList[]	= array(
                        'race'		=> $StatRow['race_name'],
                        'id'		=> $StatRow['id'],
                        'name'		=> $StatRow['username'],
                        'Erfolge'	=> pretty_number($StatRow[$Erfolge]),
