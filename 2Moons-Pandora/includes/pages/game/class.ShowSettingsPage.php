...
class ShowSettingsPage extends AbstractPage
{
	...
	public function show()
	{
		...
		else
		{
			$this->tplObj->assign_vars(array(				
				...
				'changeRaceTime'	=> ($USER['rtime'] + RACE_CHANGETIME) - TIMESTAMP,
	...
	private function sendDefault()
	{
		...
		$race           	= HTTP::_GP('race', '');
		$raceChange			= HTTP::_GP('raceChange', 0);	
		...
		$signature          = HTTP::_GP('signature', '');
		$textMode  			= HTTP::_GP('textMode', 'signature');
		$text 				= HTTP::_GP('text', '', true);
		$user_image 		= filter_var(HTTP::_GP('image', ''), FILTER_VALIDATE_URL);
		...
		if (!empty($race) && $USER['race'] != $race)
		{
			if($USER['rtime'] >= TIMESTAMP - RACE_CHANGETIME) {
				$this->printMessage($LNG['op_change_race_pro_month']);
			} else {
				
					$SQL		.= "UPDATE ".USERS." SET race = '".$GLOBALS['DATABASE']->sql_escape($race)."', rtime = ".TIMESTAMP." WHERE id = ".$USER['id'].";";
				}
			}
