...
class ShowResearchPage extends AbstractPage
{
	...
	private function AddBuildingToQueue($Element, $AddMode = true)
	{
		...
		$raceBonus = $GLOBALS['DATABASE']->getFirstRow("SELECT * FROM ".RACES." WHERE race_id = '".$USER['race_id']."';");
		...
		if(Config::get('max_elements_tech')+ $raceBonus['race_research_add'] != 0 && Config::get('max_elements_tech')+ $raceBonus['race_research_add'] <= $ActualCount)
			return false;
	...
	public function show()
	{
		...
		$this->tplObj->assign_vars(array(
			...
			'IsFullQueue'	=> Config::get('max_elements_tech')+ $raceBonus['race_research_add'] == 0 || Config::get('max_elements_tech')+ $raceBonus['race_research_add'] == count($TechQueue),
			'Queue'			=> $TechQueue,
		));
