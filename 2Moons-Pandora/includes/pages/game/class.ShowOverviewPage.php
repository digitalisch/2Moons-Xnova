...
class ShowOverviewPage extends AbstractPage 
{
	...
	function show()
	{
		...
		$query = $GLOBALS['DATABASE']->uniquequery("SELECT count(*) AS planet_count FROM ".PLANETS." WHERE `id_owner` = '". $USER['id'] ."'AND `planet_type` = '1';");
	...
	$ally    	= $GLOBALS['DATABASE']->getFirstRow("SELECT     
	user.*,           
	alliance.ally_name,        
	alliance.id,
	alliance.ally_owner,
	...
	FROM ".USERS." as user            
	LEFT JOIN ".ALLIANCE." as alliance ON alliance.id = user.ally_id    
	WHERE user.id = ".$USER['id'].";");	
	
		if ($ally['ally_owner'] == $USER['id']) {
			$allyrank	= ($ally['ally_owner_range'] != '') ? $ally['ally_owner_range'] : $LNG['al_founder_rank_text'];
		} elseif ($USER['ally_rank_id'] != 0) {
			$allyrank	= $GLOBALS['DATABASE']->getFirstCell("SELECT rankName FROM ".ALLIANCE_RANK." WHERE rankID = ".$USER['ally_rank_id'].";");	
		}
		if (empty($allyrank)) {
			$allyrank	= $LNG['al_new_member_rank_text'];
		}
		
		$this->tplObj->assign_vars(array(
			'allyrank'					=> $allyrank,
			'allyid'					=> $USER['ally_id'],
			'allyname'					=> $ally['ally_name'],
			'id'						=> $USER['ref_id'],
			'name'						=> $query['username'],
			'refs'						=> $refs['ref_count'],
			'refmax'					=> makebr(Config::get('ref_max_referals')),
			'erfolge'					=> $USER['Erfolge'],
			'rankInfo'					=> $rankInfo,
