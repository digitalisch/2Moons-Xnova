 ...
 * @info $Id: class.ShowFleetDealerPage.php 2632 2013-03-18 19:05:14Z slaver7 $
 * @link http://2moons.cc/
 */
...
class ShowFleetDealerPage extends AbstractPage
{
	...
	public function send()
	{
		...
		if ($shipID == 214 && $USER['rpg_destructeur'] == 1){
			$PLANET[$resource[901]]			+= $Count * $pricelist[$shipID]['cost'][901] * (1 - (Config::get('trade_charge') / 100)) / 2;
			$PLANET[$resource[902]]			+= $Count * $pricelist[$shipID]['cost'][902] * (1 - (Config::get('trade_charge') / 100)) / 2;
			$PLANET[$resource[903]]			+= $Count * $pricelist[$shipID]['cost'][903] * (1 - (Config::get('trade_charge') / 100)) / 2;
			$USER[$resource[921]]			+= $Count * $pricelist[$shipID]['cost'][921] * (1 - (Config::get('trade_charge') / 100)) / 2;
		...
		elseif(!empty($shipID) && in_array($shipID, $allowedShipIDs) && $PLANET[$resource[$shipID]] >= $Count)
		{
	...
	function show()
	{
		...
		{
			if(in_array($shipID, $reslist['fleet']) || in_array($shipID, $reslist['defense']) || in_array($shipID, $reslist['missile'])) {
