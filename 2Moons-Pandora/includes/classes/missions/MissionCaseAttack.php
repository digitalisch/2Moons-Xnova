...
HTML;
		...
		foreach($userAttack as $userID => $userName)
		{
			...
			if($combatResult['won'] === 'a' && ($combatResult['unitLost']['attacker'] + $combatResult['unitLost']['defender']) > 1000000)
			{
					...
					else if($zufall>10 && $zufall<=25)
					{
					$sql = "
						UPDATE
						`" . USERS . "`
						SET
						`darkmatter` = `darkmatter` + 75
						WHERE
						`id` = " . $userID . "
						LIMIT 1;";
					...
					else if($zufall>25 && $zufall<=45)
					{
					$sql = "
						UPDATE
						`" . USERS . "`
						SET
						`darkmatter` = `darkmatter` + 50
						WHERE
						`id` = " . $userID . "
						LIMIT 1;";
					...
					else if($zufall>45 && $zufall<=70)
					{
					$sql = "
						UPDATE
						`" . USERS . "`
						SET
						`darkmatter` = `darkmatter` + 25
						WHERE
						`id` = " . $userID . "
						LIMIT 1;";
