...
class FithReferralCronJob
{
	...
	{		
		...
		$Users	= $GLOBALS['DATABASE']->query("SELECT user.`username`, user.`ref_id`, user.`id`, user.`lang` 
							  ...
							  ON stats.`id_owner` = user.`id` AND stats.`stat_type` = '1' AND stats.`total_points` >= ".$points."
							  WHERE user.`ref_bonus` = 5;");
		...
		while($User	= $GLOBALS['DATABASE']->fetch_array($Users))
		{
			...
			$GLOBALS['DATABASE']->multi_query("UPDATE ".USERS." SET `darkmatter` = `darkmatter` + ".$win." WHERE `id` = ".$User['ref_id'].";
											   UPDATE ".USERS." SET `ref_bonus` = `ref_bonus` + 1 WHERE `id` = ".$User['id'].";");

			$Message	= sprintf($LNG['sys_refferal_text'], $User['username'], pretty_number($points), pretty_number($win), $LNG['tech'][921]);
