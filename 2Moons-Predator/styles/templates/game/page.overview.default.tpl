...
<table class="table519">
	...
	<tr>
		...
		<td colspan="2" class="servertime"><font color=lime> {$servertime} </td></font>
	</tr>
	{if $is_news}
	<tr>
	...
	{/if}
	{if $race}	<tr>	 <td>{$LNG.ov_race}</td><td colspan="2"><font color=lime> {$race} </font></td>	</tr>	{/if}
	<tr>
		<td style="white-space: nowrap;">{$LNG.ov_admins_online}</td>
		<td colspan="2">{foreach $AdminsOnline as $ID => $Name}{if !$Name@first}&nbsp;&bull;&nbsp;{/if}<a href="#" onclick="return Dialog.PM({$ID})"><font color="red"> {$Name}</a>{foreachelse}{$LNG.ov_no_admins_online}{/foreach}</font></td>
	</tr>
	{if !empty($chatOnline)}
	<tr>
