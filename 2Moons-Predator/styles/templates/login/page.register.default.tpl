...
<input type="hidden" value="{$referralData.id}" name="referralID">
	...
	{if count($raceSelect) > 1}        <div class="rowForm">		<label for="race">{$LNG.registerRace}</label>		<select name="race" id="race">{html_options options=$raceSelect}</select>		{if !empty($error.race)}<span class="error errorRace"></span>{/if}		<div class="clear"></div>	</div>        {/if}
