...
{
    global $xgp_root, $lang, $languser;
    
    if ($languser != '') 
        include ($xgp_root . "language/". $languser ."/". $filename.$ext);
    else
...
function doquery($query, $table, $fetch = false)
{
	...
	$sqlquery     = mysql_query($sql) or $debug->error(mysql_error()."<br />$sql<br />Archivo $file:$line<br />","SQL Error");

	unset($dbsettings);

	global $numqueries,$debug;

    $e=new Exception();//mejora debug
        $e=$e->getTrace();
        foreach($e as $a => $b){
                foreach($b as $c => $d){
                        if($d=="doquery"){
...
//Version 0.0.2
unset($e);//Liberaremos la memoria
	$numqueries++;


	$debug->add("<tr><th>Query $numqueries: </th><th>$query</th><th>$table</th><th>$fetch</th></tr>
                     <tr><th colspan=2>Archivo $file: </th><th colspan=2>$line</th></tr>");
