...
if(!defined('INSIDE')){ die(header("location:../../"));}
	...
	function GetBuildingTime ($user, $planet, $Element, $level = false)
	{
        ...
        // -10% (Diminution du temps des constructions)
        if ( $user['staatsform'] == 2 && $user['rpg_constructeur'] >= 0 )
            $time *= 0.9; 
        ...
        // -10% (Diminution du temps des recherches des technologies)
        if ( $user['staatsform'] == 1 && $user['rpg_scientifique'] >= 0 )
            $time *= 0.9; 
		}
        ...
        // Gouvernement Dictaturial
        // -5% (Diminution du temps de construction des flottes et des défenses).
        if ( $user['staatsform'] == 3 && $user['rpg_technocrate'] >= 0 )
            $time *= 0.95; 
		}
