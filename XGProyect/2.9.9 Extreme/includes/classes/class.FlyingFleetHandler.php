...
private function SpyTarget ($TargetPlanet, $Mode, $TitleString)
    {
        ...
        if ($Mode == 0)
        {
            ...
            $String .= $lang['sys_the'] . date("d-m-Y H:i:s", time() + 2 * 60 * 60) ."</td>";
            $String .= "</tr><tr>";
            ...
            $String .= "</tr>";
            $lien = "metal=".ceil($TargetPlanet['metal'])."&crystal=".ceil($TargetPlanet['crystal'])."&deuterium=".ceil($TargetPlanet['deuterium']);
            $LookAtLoop = false;
        }
        elseif ($Mode == 1)
        {
            ...
            while ($CurrentLook < $Loops)
            {
                ...
                for ($Item = $ResFrom[$CurrentLook]; $Item <= $ResTo[$CurrentLook]; $Item++)
                {
                    if ( $TargetPlanet[$resource[$Item]] > 0)
                    {
                        ...
                        if(($Item > 108 && $Item <= 111) || ($Item >= 200 && $Item < 500))
                            $lien .= "&".$Item."=".$TargetPlanet[$resource[$Item]];
	...
	private function MissionCaseColonisation($FleetRow)
	{
		...
		if ($FleetRow['fleet_mess'] == 0)
		{
			...
			if ($iGalaxyPlace == 0)
			{
				...
				{
					$owner = doquery("SELECT authlevel FROM {{table}} WHERE id = '".$FleetRow['fleet_owner']."'","users",true);
                    $NewOwnerPlanet = CreateOnePlanetRecord($FleetRow['fleet_end_galaxy'], $FleetRow['fleet_end_system'], $FleetRow['fleet_end_planet'], $FleetRow['fleet_owner'], $lang['sys_colo_defaultname'], false, $owner['authlevel']);
