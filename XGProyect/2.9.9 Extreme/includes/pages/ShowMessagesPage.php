...
function ShowMessagesPage($CurrentUser)
{
	...
	$UsrOwnMess    = doquery("SELECT * FROM {{table}} WHERE `message_sender`  = '".intval($CurrentUser['id'])."' ORDER BY `message_time` DESC;",  'messages'); 
    $UnRead        = doquery("SELECT * FROM {{table}} WHERE `id` = '". intval($CurrentUser['id']) ."';", 'users', true);

	$MessageType   = array ( 0, 1, 2, 3, 4, 5, 15, 99, 100, 101 );
	$TitleColor    = array ( 0 => '#FFFF00', 1 => '#FF6699', 2 => '#FF3300', 3 => '#FF9900', 4 => '#773399', 5 => '#009933', 15 => '#030070', 99 => '#007070', 100 => '#ABABAB', 101=>'lightblue' );  
    ...
    while ($CurOwnMess = mysql_fetch_array($UsrOwnMess)) 
    { 
        $MessType              = $CurMess['message_type']; 
        $TotalMess[$MessType] += 1; 
        $TotalMess[101]       += 1; 
	...
	switch ($MessPageMode)
	{
		...
		case 'show':
			...
			if ($MessCategory == 100)
			{
				...
				while ($CurMess = mysql_fetch_array($UsrMess))
				{
					...
					$page .= "</tr><tr>";
					$page .= "<td>&nbsp;</td>";
					$page .= "<td colspan=\"3\" class=\"b\">". str_replace("&lt;br /&gt;","",stripslashes( nl2br($CurMess['message_text'] ) ) ) ."</td>";
                 	$page .= "</tr>";
				}
			}
			else
            {
                if($MessCategory != 101)
                    ...
                    $UsrMess        = doquery("SELECT * FROM {{table}} WHERE `message_sender` = '".$CurrentUser['id']."' ORDER BY `message_time` DESC;", 'messages');
                    
                while ($CurMess = mysql_fetch_array($UsrMess))
                {
                    if ($CurMess['message_type'] == $MessCategory OR $MessCategory == 101 AND $CurMess['message_sender'] == $CurrentUser['id'])
                    {
                        $page .= "\n<tr>";
                        $page .= "<input name=\"showmes". $CurMess['message_id'] . "\" type=\"hidden\" value=\"1\">";
                        $page .= "<th><input name=\"delmes". $CurMess['message_id'] ."\" type=\"checkbox\"></th>";
                        ...
                        $page .= "</tr><tr>";
                        $page .= "<td class=\"b\"> </td>";
                        $page .= "<td colspan=\"3\" class=\"b\">". nl2br( str_replace("&lt;br /&gt;","",stripslashes( $CurMess['message_text'] ) ) ) ."</td>";  
                        $page .= "</tr>";
                        
                        if($CurrentUser['new_message'] == 0 OR $MessCategory == 101)
                            continue;
		...
		default:
			...
			for ($MessType = 0; $MessType <= 101; $MessType++)
            {
                if ( !in_array($MessType, $MessageType) OR $MessType == 100)
                    continue;
