...
{
	$parse['boletos_p_u']	=	"<font color=red>".$boletos_usuario." (Maximo permitido)</font>";
}
...
if($Tiempo < $game_config['Loteria'])
{
	...
	function segundos_tiempo($segundos)
	{
		...
		$horas		=	floor($minutos/60);
      	...
      	if($segundos <60)
		{ 
      		$resultado	=	'La loteria estara disponible en '.round($segundos).' segundos';
      	}
		elseif($segundos > 60 && $segundos < 3600)
		{
      		$resultado	= 	'La loteria estara disponible en '.$minutos2.' minutos y '.$segundos_2.' segundos';
      	}
		else
		{
     		$resultado	= 	'La loteria estara disponible en '.$horas.' horas, '.$minutos2.' minutos y '.$segundos_2.' segundos';
      	}
	...
	{	
		...
		if ($post == 'si')
		{
			...
			if ($materia	>	$user['darkmatter'])
			{
				...
				$ff_materia	=	"<tr><th class=\"error2\">Materia oscura: <font color=lime>".$f_materia."</font></th></tr>";
			}
			if ($metal	>	$planetrow['metal'])
			{
				$f_metal	=	($metal - $planetrow['metal']);
				$comun		=	"<tr><th class=\"error2\">Para comprar <font color=lime>".$ticket."</font> boleto/s te falta:</th></tr>";
				$ff_metal	=	"<tr><th class=\"error2\">Metal: <font color=lime>".$f_metal."</font></th></tr>";
			}
			if ($cristal	>	$planetrow['crystal'])
			{
				$f_cristal	=	($cristal - $planetrow['crystal']);
				$comun		=	"<tr><th class=\"error2\">Para comprar <font color=lime>".$ticket."</font> boleto/s te falta:</th></tr>";
				$ff_cristal	=	"<tr><th class=\"error2\">Cristal: <font color=lime>".$f_cristal."</font></th></tr>";
			}
			if ($deute	>	$planetrow['deuterium'])
			{
				$f_deute	=	($deute - $planetrow['deuterium']);
				$comun		=	"<tr><th class=\"error2\">Para comprar <font color=lime>".$ticket."</font> boleto/s te falta:</th></tr>";
				$ff_deute	=	"<tr><th class=\"error2\">Deuterio: <font color=lime>".$f_deute."</font></th></tr>";
			}
				...
				$error_boletos <= $boletos_por_u && $error_boletos2 <= $boletos_max)
			{
					...
					{		
						...
						$Time    		= time();
  						$From    		= "<font color='#01DF01'>Loteria</font>"; 
  						$Subject 		= "<font color='#01DF01'>Resultado</font>";
						...
						{   
             				if($query_1['id'] == $b['id'])
							{ 
             					$Message  = "<b>Felicidades<br>Sos el ganador de la loteria intergalactica, tu premio es de: ";
								$Message .= "<br><font color=lime>".$metal_p."</font> de Metal <br>";
								$Message .= "<font color=lime>".$cristal_p."</font> de Cristal <br>";
								$Message .= "<font color=lime>".$deute_p."</font> de Deuterio <br>";
								$Message .= "<font color=lime>".$materia_p."</font> de Materia oscura <br><br>";
								$Message .= "Esperamos verte de nuevo, gracias por participar.</b>";
             				}
							else
							{   
            					$Message  = "<b>Lo siento, no tenias el ticket ganador.<br>";
								$Message .= "De todas formas te deseamos suerte y esperamos verte de vuelta participando en la loteria.</b>";
             				}
             				SendSimpleMessage ( $b['id'], $b['id'], $Time, 1, $From, $Subject, $Message);
    					}
						// Fin de mensajes ##############################################################################
					...
					message("Compraste: <font color=lime>".$ticket."</font> ticket/s", "game.php?page=loteria", 2);
			}
		}
		// Fin del trámite ##############################################################################
	}
