...
class ShowAlliancePage
{
	...
	public function __construct($CurrentUser)
	{
		...
		if ($_GET['mode'] == 'ainfo')
		{
			...
			if ($ally_description != "")
				$ally_description = "<tr><th colspan=2 height=100>".($this->bbcode($ally_description))."</th></tr>";
			else
			...
			if ($mode == 'admin' && $edit == 'ally')
			{
				...
				elseif ($_POST['t'])
				{
					if ($t == 3)
					{
						...
						$ally['ally_request'] = str_replace("&lt;br /&gt;","",str_replace("rn","\r\n",mysql_escape_string(strip_tags(stripslashes($_POST['text'])))));
						doquery("UPDATE {{table}} SET
					...
					elseif ($t == 2)
					{
						$ally['ally_text'] = str_replace("&lt;br /&gt;","",str_replace("rn","\r\n",mysql_escape_string(strip_tags(stripslashes($_POST['text'])))));
						doquery("UPDATE {{table}} SET
					...
					{
						$ally['ally_description'] = str_replace("&lt;br /&gt;","",str_replace("rn","\r\n",mysql_escape_string(strip_tags(stripslashes($_POST['text'])))));
				...
				if ($t == 2)
					$lang['text'] = str_replace("&lt;br /&gt;","",$ally['ally_text']);
				else
					$lang['text'] = str_replace("&lt;br /&gt;","",$ally['ally_description']);

				if ($t == 3)
					$lang['text'] = str_replace("&lt;br /&gt;","",$ally['ally_request']);
