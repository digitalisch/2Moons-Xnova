...
function ShowStatisticsPage($CurrentUser)
{
	...
	else
	{
		...
		$stats_sql    =    'SELECT s.*, u.id, u.username, u.ally_id, u.ally_name,u.humano,u.vampiro,u.lobo,u.asgard FROM {{table}}statpoints as s
		...
		while ($StatRow = mysql_fetch_assoc($query))
		{
           ...
           if ($StatRow['humano'] == 1)
                {$parse['player_raza'] = 'Humano';}
                    elseif ($StatRow['vampiro'] == 1)
                        {$parse['player_raza'] = 'Vampiro';}
                            elseif ($StatRow['lobo'] == 1)
                                {$parse['player_raza'] = 'Lobo';}
                                    elseif ($StatRow['asgard'] == 1)
                                        {$parse['player_raza'] = 'Asgard';}  
