...
class ShowShipyardPage
{
	private function GetMaxConstructibleElements ($Element, $Ressources)
	{
        ...
        elseif ($MaxElements > MAX_FLEET_OR_DEFS_PER_ROW)
			...
			if ($Element > 201 && $Element <= 399)
			{
				if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element))
				{
					...
					if ($CanBuildOne && $NotBuilding)
					{
                        ...
                        $PageTable .= "<br><th><a href='javascript:' onclick=\"document.getElementsByName('fmenge[".$Element."]')[0].value = '$maxElement';\">Máx.</a></th>";  
				        }


					if($NotBuilding)
					{
						$parse[build_fleet] = "<tr><td class=\"c\" colspan=\"4\" align=\"center\"><input type=\"submit\" value=\"".$lang['bd_build_ships']."\"></td></tr>";
					}
			...
			if ($Element > 400 && $Element <= 599)
			{
				if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element))
				{
					...
					if ($CanBuildOne)
					{
						...
						if($NotBuilding)
						{
							$parse[build_defenses] = "<tr><td class=\"c\" colspan=\"4\" align=\"center\"><input type=\"submit\" value=\"".$lang['bd_build_defenses']."\"></td></tr>";
						}
					}
					else
					{
