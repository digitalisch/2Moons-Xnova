...
function shortInfo() {
  ...
  var durat=duration();
    document.getElementById("duration").innerHTML = dataToString(durat);
  var stor = storage();
    var cons = consumption();
...
var timer;
function shortInfo2(duratx) {
  
  var date= Date.today();
    date.addSeconds(duratx+getTimeInSeconds());
    document.getElementById("arrive_time").innerHTML=date.toString("dd-MM-yyyy HH:mm:ss");
    date.addSeconds(duratx);
    document.getElementById("return_time").innerHTML=date.toString("dd-MM-yyyy HH:mm:ss");
     
     if(timer!=null)
       clearTimeout(timer);
     timer=setTimeout("shortInfo2("+duratx+");", 1000); 
}    
    //end mod by jstar  
