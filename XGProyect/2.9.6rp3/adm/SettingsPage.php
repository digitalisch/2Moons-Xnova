...
function DisplayGameSettingsPage ( $CurrentUser )
{
	...
	if ($_POST['opt_save'] == "1")
	{
		...
		if (isset($_POST['loginImage']) && is_numeric($_POST['loginImage'])) {
		$game_config['background'] = intval($_POST['loginImage']);
		$Log    .=  "Login image: ". ($_POST['forum_url']=="egg!") ? "An easter egg!" : $game_config['background'];
		}
		...
		if (isset($_POST['tritium_basic_income']) && is_numeric($_POST['tritium_basic_income'])) {
		$game_config['tritium_basic_income'] = $_POST['tritium_basic_income'];
		$Log	.=	$lang['log_sett_basic_t'].": ".$_POST['tritium_basic_income']."\n";
		}
		...
		doquery("UPDATE {{table}} SET `config_value` = '". $game_config['background']             ."' WHERE `config_name` = 'background';", 'config');
		...
		doquery("UPDATE {{table}} SET `config_value` = '". $game_config['tritium_basic_income']   ."' WHERE `config_name` = 'tritium_basic_income';", 'config');
	...
	else
	{
		...
		$parse['selectedLogin'.$game_config['background']] = 'selected';
