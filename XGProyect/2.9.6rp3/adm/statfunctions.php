	...
	foreach ( $reslist['tech'] as $n => $Techno )
	{
		if ( $CurrentUser[ $resource[ $Techno ] ] > 0 ) {
			for ( $Level = 0; $Level < $CurrentUser[ $resource[ $Techno ] ]; $Level++ ) {
				$Units       = $pricelist[ $Techno ]['metal'] + $pricelist[ $Techno ]['crystal'] + $pricelist[ $Techno ]['deuterium'] + $pricelist[ $Techno ]['tritium'];
...
function GetDefensePoints ( $CurrentPlanet ) {
	...
	foreach($reslist['defense'] as $n => $Defense) {
		if ($CurrentPlanet[ $resource[ $Defense ] ] > 0) {
			$Units          = $pricelist[ $Defense ]['metal'] + $pricelist[ $Defense ]['crystal'] + $pricelist[ $Defense ]['deuterium'] + $pricelist[ $Defense ]['tritium'];
...
function GetFleetPoints ( $CurrentPlanet ) {
	...
	foreach($reslist['fleet'] as $n => $Fleet) {
		if ($CurrentPlanet[ $resource[ $Fleet ] ] > 0) {
			$Units          = $pricelist[ $Fleet ]['metal'] + $pricelist[ $Fleet ]['crystal'] + $pricelist[ $Fleet ]['deuterium'] + $pricelist[ $Fleet ]['tritium'];
...
{global $resource, $pricelist, $reslist;
	...
	if(is_array($FleetRec))
	{
		foreach($FleetRec as $Item => $Group)
		{
			...
			{
				$Ship    = explode(",", $Group);
				$Units         = $pricelist[ $Ship[0] ]['metal'] + $pricelist[ $Ship[0] ]['crystal'] + $pricelist[ $Ship[0] ]['deuterium'] + $pricelist[ $Ship[0] ]['tritium'];
