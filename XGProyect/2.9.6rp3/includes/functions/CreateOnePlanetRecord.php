...
if(!defined('INSIDE')){ die(header("location:../../"));}
	...
	function PlanetSizeRandomiser ($Position, $HomeWorld = false, $Static = false)
	{
		...
		if($Static == true){
			return array($CalculMin + 10, $CalculMax + 110);
		}
		return $return;
	}
	...
	function CreateOnePlanetRecord($Galaxy, $System, $Position, $PlanetOwnerID, $PlanetName = '', $HomeWorld = false)
	{
		...
		if (!$PlanetExist)
		{
			...
			$QryInsertPlanet .= "`tritium` = '".           $planet['tritium']           ."', ";
			$QryInsertPlanet .= "`tritium_perhour` = '".   $planet['tritium_perhour']   ."', ";
			$QryInsertPlanet .= "`tritium_max` = '".       $planet['tritium_max']       ."';";
