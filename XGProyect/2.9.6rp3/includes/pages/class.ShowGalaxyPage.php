...
function GalaxyDeleteWormholes(){
			$GalaxyRowWormHoles = doquery("SELECT * FROM {{table}} WHERE `time` < '". time() ."' AND `state` = '0';", 'wormholes', false);
			while($GalaxyRowWormHole = mysql_fetch_array($GalaxyRowWormHoles)){
					$flottencount = 0;
					$WormFleets = doquery("SELECT * FROM {{table}} WHERE `fleet_wormhole` != '0';", 'fleets', false);
					while($FleetWorm = mysql_fetch_array($WormFleets)){
						$FleetInfo = explode(',' , $FleetWorm['fleet_wormhole']);
						if($FleetInfo[0] == $GalaxyRowWormHole['id']){
							$flottencount++;
						}
					}
					if($flottencount == 0){
						doquery("DELETE FROM {{table}} WHERE `id` = '".$GalaxyRowWormHole['id']."';", 'wormholes');
					}
...
class ShowGalaxyPage extends GalaxyRows
{
	...
	private function ShowGalaxyRows($Galaxy, $System, $HavePhalanx, $CurrentGalaxy, $CurrentSystem, $CurrentRC, $CurrentMIP)
	{
		...
		for ($Planet = 1; $Planet < 1+(MAX_PLANET_IN_SYSTEM); $Planet++)
		{
			...
			$GalaxyRowWormHole = doquery("SELECT * FROM {{table}} WHERE (`start_galaxy` = '".$Galaxy."' AND `start_system` = '".$System."' AND `start_planet` = '".$Planet."') OR (`end_galaxy` = '".$Galaxy."' AND `end_system` = '".$System."' AND `end_planet` = '".$Planet."') LIMIT 1;", 'wormholes', true);
			...
			if ($GalaxyRow)
			{
				...
				if($GalaxyRowWormHole['time'] < time() and $GalaxyRowWormHole['state'] == 1){
					$GalaxyRowWormHole['state'] = 0;
					doquery("UPDATE {{table}} SET `state` = '0' WHERE `id` = '".$GalaxyRowWormHole['id']."';", 'wormholes');
				}
			}

			$Result .= $this->GalaxyRowPos        ( $GalaxyRow, $Galaxy, $System, $Planet, 1 );
			$Result .= $this->GalaxyRowPlanet     ( $GalaxyRow, $GalaxyRowPlanet, $GalaxyRowPlayer, $Galaxy, $System, $Planet, 1, $HavePhalanx, $CurrentGalaxy, $CurrentSystem, $GalaxyRowWormHole);
			$Result .= $this->GalaxyRowPlanetName ( $GalaxyRow, $GalaxyRowPlanet, $GalaxyRowPlayer, $Galaxy, $System, $Planet, 1, $HavePhalanx, $CurrentGalaxy, $CurrentSystem, $GalaxyRowWormHole);
