    ...
    if (isset($_POST['ress']) && $_POST['ress'] != '')
    {
        switch ($_POST['ress'])
        {
            ...
            case 'tritium':
            {
                ...
                {
                    $necessaire   = ((abs($_POST['metal']) * 0.125) + (abs($_POST['cristal']) * 0.25) * (abs($_POST['deuterium']) * 0.5));

                    if ($CurrentPlanet['tritium'] > $necessaire)
                    {
                        $QryUpdatePlanet  = "UPDATE {{table}} SET ";
                        $QryUpdatePlanet .= "`metal` = `metal` + ".round($_POST['metal']).", ";
                        $QryUpdatePlanet .= "`crystal` = `crystal` + ".round($_POST['cristal']).", ";
						$QryUpdatePlanet .= "`deuterium` = `deuterium` + ".round($_POST['deuterium']).", ";
                        $QryUpdatePlanet .= "`tritium` = `tritium` - ".round($necessaire)." ";
                        $QryUpdatePlanet .= "WHERE ";
                        $QryUpdatePlanet .= "`id` = '".$CurrentPlanet['id']."';";
                        ...
                        $CurrentPlanet['metal']     += $_POST['metal'];
                        $CurrentPlanet['cristal']   += $_POST['cristal'];
						$CurrentPlanet['deuterium']   += $_POST['deuterium'];
                        $CurrentPlanet['tritium'] -= $necessaire;
                        doquery("UPDATE `{{table}}` SET `darkmatter` = `darkmatter` - ".TR_DARK_MATTER." WHERE `id` = ".$CurrentUser['id']."", 'users');
                    }
                    else
                    {
                        message($lang['tr_not_enought_tritium'], "game." . $phpEx . "?page=trader",1);
                    }
                }
                break;
            }
        }
        doquery("UPDATE {{table}} SET `tut_6_mer` = '1' WHERE `id` = '".$CurrentUser['id']."';", 'users');
    ...
    else
    {
        ...
        else
        {
            ...
            switch ($_POST['choix'])
            {
                ...
                break;
                case 'tritium':
                $template = gettemplate('trader/trader_tritium');
                $parse['mod_ma_res_a'] = '0.125';
                $parse['mod_ma_res_b'] = '0.25';
				$parse['mod_ma_res_c'] = '0.5';
                break;
            }
        }
    }
