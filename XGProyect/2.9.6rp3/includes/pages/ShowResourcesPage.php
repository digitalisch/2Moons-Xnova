...
function ShowResourcesPage($CurrentUser, $CurrentPlanet)
{
	...
	if ($CurrentPlanet['planet_type'] == 3)
	{
		...
		$game_config['tritium_basic_income']   = 0;
	}
	...
	$CurrentPlanet['tritium_max']	=	(BASE_STORAGE_SIZE + 50000 * (roundUp(pow(1.6,$CurrentPlanet[ $resource[25] ])) -1)) * (1 + ($CurrentUser['rpg_stockeur'] * STOCKEUR));

	$parse['resource_row']               = "";
	$CurrentPlanet['metal_perhour']      = 0;
	$CurrentPlanet['crystal_perhour']    = 0;
	$CurrentPlanet['deuterium_perhour']  = 0;
	$CurrentPlanet['tritium_perhour']  	 = 0;
	...
	foreach($reslist['prod'] as $ProdID)
	{
		if ($CurrentPlanet[$resource[$ProdID]] > 0 && isset($ProdGrid[$ProdID]))
		{
			...
			$tritium   = floor( eval ( $ProdGrid[$ProdID]['formule']['tritium']   ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * GEOLOGUE ) ) );

			if( $ProdID >= 4 and $ProdID != 5)
			{
			...
			$CurrRow['tritium_type']             = pretty_number ( $tritium   );
			...
			$CurrRow['tritium_type']             = colorNumber ( $CurrRow['tritium_type']   );
	...
	$parse['tritium_basic_income']   = $game_config['tritium_basic_income'];
	...
	if ($CurrentPlanet['tritium_max'] < $CurrentPlanet['tritium'])
	{
		$parse['tritium_max']     = "<font color=\"#ff0000\">";
	}
	else
	{
		$parse['tritium_max']     = "<font color=\"#00ff00\">";
	}
	$parse['tritium_max']        .= pretty_number($CurrentPlanet['tritium_max'] / 1000) ."k</font>";
	...
	$parse['tritium_total']         = colorNumber( pretty_number( floor( ( ($CurrentPlanet['tritium_perhour']   * 0.01 * $parse['production_level'] ) + $parse['tritium_basic_income']))));
	...
	$parse['daily_tritium']       = floor($CurrentPlanet['tritium_perhour'] * 24      * 0.01 * $parse['production_level']  + $parse['tritium_basic_income']  * 24      );
	$parse['weekly_tritium']      = floor($CurrentPlanet['tritium_perhour'] * 24 * 7  * 0.01 * $parse['production_level']  + $parse['tritium_basic_income']  * 24 * 7  );

	$parse['daily_tritium']       = colorNumber(pretty_number($parse['daily_tritium']));
	$parse['weekly_tritium']      = colorNumber(pretty_number($parse['weekly_tritium']));
