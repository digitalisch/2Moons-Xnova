...
class ShowOfficierPage
{
	...
	public function __construct ( &$CurrentUser )
	{
		...
		if ($_GET['mode'] == 2 and $CurrentUser['tut_9'] == 1)
		{
		...
		elseif($CurrentUser['tut_9'] == 1)
		{
			...
			$parse['alv_points']   	= floor($CurrentUser['darkmatter'] / 1000);
			$parse['disp_off_tbl']	= '<tr><th colspan="3">Haz el <a href="game.php?page=tutorial">Tutorial</a> para desbloquear los Oficiales</th></tr>';
			$page = parsetemplate( gettemplate('officier/officier_table'), $parse);
		}
		if(!defined("UGAPAY_ACTIVATED") or UGAPAY_ACTIVATED == false){
			$page = str_get_html($page);
			$page->find("table[id=UgaPay]", 0)->style = "display:none;"; 			
		}
		display($page);
	}
}
