...
function CreateWormHole($Percent, $Galaxy, $System, $Planet){

    $WormHoleTime = (time() + (($Percent * 2) * 60 * 60));
    if($Percent > 60){
        $Percent = 60;
    }
    $Percent2 = mt_rand(0, 100);
    if($Percent2 <= $Percent){
        $CreateHole = true;        
    ...
    if($CreateHole == true){
        ...
        $QryInsertPlanet .= "`start_galaxy` = '".          $Galaxy          ."', ";
        $QryInsertPlanet .= "`start_system` = '".          $System          ."', ";
        $QryInsertPlanet .= "`start_planet` = '".          $Planet          ."', ";
