...
class GalaxyRows
{
	...
	public function GalaxyRowUser($GalaxyRow, $GalaxyRowPlanet, $GalaxyRowUser, $Galaxy, $System, $Planet, $PlanetType, $UserPoints)
	{
		...
		if ($GalaxyRowUser && $GalaxyRowPlanet["destruyed"] == 0)
		{
			...
			$UserPoints			= '&nbsp;<span style="color:orange;">#'.pretty_number($User2Points['total_rank']).'</span>';
			if ($GalaxyRowUser['bana'] == 1 && $GalaxyRowUser['urlaubs_modus'] == 1)
			{
				...
				$UserPoints		= '&nbsp;<span style="color:grey;">#'.pretty_number($User2Points['total_rank']).'</span>';
			}
			...
			elseif ($GalaxyRowUser['onlinetime'] < (time()-60 * 60 * 24 * 7) && $GalaxyRowUser['onlinetime'] > (time()-60 * 60 * 24 * 28))
			{
				...
				$UserPoints		= '&nbsp;<span style="color:red;">#'.pretty_number($User2Points['total_rank']).'</span>';
			}
			...
			$Result .= "</span></a><a href=game.php?page=statistics&who=player&start=".$Systemtart.">".$UserPoints."</a>";
		}
		$Result .= "</th>";
