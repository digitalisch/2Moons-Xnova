...
class FlyingFleetHandler
{
	...
	private function SpyTarget ($TargetPlanet, $Mode, $TitleString)
	{
		...
		if ($Mode == 0)
		{
			...
			$String .= "<td width=220>". $lang['Tritium'] ."</td><td width=220 align=right>". pretty_number($TargetPlanet['tritium'])  ."</td>";
			$String .= "</tr><tr>";
			$String .= "<td width=220 colspan=2>". $lang['Energy']    ."</td><td width=220 align=right>". pretty_number($TargetPlanet['energy_max']) ."</td>";
			$String .= "</tr>";
			$LookAtLoop = false;
		}
		elseif ($Mode == 1)
	...
	private function RestoreFleetToPlanet ($FleetRow, $Start = true)
	{
		...
		$QryUpdatePlanet  .= "`tritium` = `tritium` + '". $FleetRow['fleet_resource_tritium'] ."' ";
	...
	private function MissionCaseAttack ($FleetRow)
	{
		...
		if ($FleetRow['fleet_mess'] == 0 && $FleetRow['fleet_start_time'] <= time())
		{
			...
			$steal = array('metal' => 0, 'crystal' => 0, 'deuterium' => 0, 'tritium' => 0);
			...
			foreach ($defense as $fleetID => $defender)
			{
				...
				else
				{
					...
					$QryUpdateTarget .= "`tritium` = `tritium` - '". $steal['tritium'] ."' ";
			...
			// mod TOP KB
                ...
                $QryInserttopkb .= "`time` = '". $FleetRow['fleet_start_time'] ."', ";
                ...
                $QryInserttopkb .= "`a_zestrzelona` = '', ";
                $QryInserttopkb .= "`raport` = '',";
		...
		elseif ($FleetRow['fleet_end_time'] <= time())
		{
			$Message         = sprintf( $lang['sys_fleet_won'],
						...
						pretty_number($FleetRow['fleet_resource_tritium']), $lang['Tritium'] );
		...
		if ($FleetRow['fleet_mess'] == 0)
		{
			if ($FleetRow['fleet_start_time'] < time())
			{
				...
				$Message         = sprintf( $lang['sys_tran_mess_owner'],
							...
							$FleetRow['fleet_resource_tritium'], $lang['Tritium'] );
				...
				$QryUpdateFleet .= "`fleet_resource_tritium` = '0' , ";
	...
	private function MissionCaseStay($FleetRow)
	{
		...
		if ($FleetRow['fleet_mess'] == 0)
		{
			if ($FleetRow['fleet_start_time'] <= time())
			{
				...
				$lang['Tritium'], pretty_number($FleetRow['fleet_resource_tritium']));
	...
	private function MissionCaseSpy($FleetRow)
	{
		...
		if ($FleetRow['fleet_start_time'] <= time())
		{
			...
			$fquery              = "";
			doquery("UPDATE {{table}} SET `tut_7_esp` = `tut_7_esp` + 1 WHERE `id` = '".$CurrentUser['id']."';", 'users');
			PlanetResourceUpdate ( $TargetUser, $TargetPlanet, time() );
	...
	private function MissionCaseRecycling ($FleetRow)
	{
		...
		if ($FleetRow["fleet_mess"] == "0")
		{
			if ($FleetRow['fleet_start_time'] <= time())
			{
				...
				$QryUpdateGalaxy .= "LIMIT 1;";
				doquery( $QryUpdateGalaxy, 'galaxy');
				doquery("UPDATE {{table}} SET `tut_10_rec` = `tut_10_rec` + 1 WHERE `id` = '".$FleetRow['fleet_owner']."';", 'users');
	...
	private function MissionCaseExpedition($FleetRow)
	{
		...
		if ($FleetRow['fleet_mess'] == 0)
		{
			...
			{
		doquery("UPDATE {{table}} SET `tut_8_exp` = `tut_8_exp` + 1 WHERE `id` = '".$FleetOwner."';", 'users');
				...
				$FleetUsedCapacity  = $FleetRow['fleet_resource_metal'] + $FleetRow['fleet_resource_crystal'] + $FleetRow['fleet_resource_deuterium'] + $FleetRow['fleet_resource_tritium'] + $FleetRow['fleet_resource_darkmatter'];
