...
class FlyingFleetsTable
{
	private function CreateFleetPopupedMissionLink($FleetRow, $Texte, $FleetType)
	{
		...
		$FleetTotalC  = $FleetRow['fleet_resource_metal'] + $FleetRow['fleet_resource_crystal'] + $FleetRow['fleet_resource_deuterium'] + $FleetRow['fleet_resource_tritium'];
		if ($FleetTotalC <> 0)
		{
			...
			$FRessource  .= "<tr><td width=50% align=left><font color=white>".$lang['Tritium']."<font></td><td width=50% align=right><font color=white>". pretty_number($FleetRow['fleet_resource_tritium']) ."<font></td></tr>";
	...
	public function BuildFleetEventTable($FleetRow, $Status, $Owner, $Label, $Record)
	{
		...
		if ($Status != 2)
		{
			...
			if($FleetRow['fleet_wormhole'] != 0){
				$WormHoleInfo = explode(',' , $FleetRow['fleet_wormhole']);
				if($WormHoleInfo[1] == 'end'){$WormHoleInfo[1] = 'start';}else{$WormHoleInfo[1] = 'end';}
				$WormHole = doquery("SELECT * FROM {{table}} WHERE `id` = '".$WormHoleInfo[0]."';", 'wormholes', true);
				$TargetID .= ' a traves del Agujero de gusano <a href="game.php?page=galaxy&mode=3&galaxy='.$WormHole[$WormHoleInfo[1].'_galaxy'].'&system='.$WormHole[$WormHoleInfo[1].'_system'].'">['.$WormHole[$WormHoleInfo[1].'_galaxy'].':'.$WormHole[$WormHoleInfo[1].'_system'].':'.$WormHole[$WormHoleInfo[1].'_planet'].']</a>';
			}	
		}
		else
		{
