...
switch($_GET[page])
{
	case'lostpassword':
		...
		else
		{
			...
			$parse['background']   = ($game_config['background'] >= 0) ? $game_config['background'] : mt_rand(0,6);
	...
	default:
		if ($_POST)
		{
			$login = doquery("SELECT `id`,`username`,`password`,`banaday` FROM {{table}} WHERE `email` = '" . mysql_escape_string($_POST['email']) . "' AND `password` = '" . md5($_POST['password']) . "' LIMIT 1", "users", true);
			...
			{
				message($lang['login_error'], "./", 4, false, false);
			}
		}
		else
		{
