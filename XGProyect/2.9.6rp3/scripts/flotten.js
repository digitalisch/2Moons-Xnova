...
function maxResource(id) {
	var thisresource=Number(document.getElementsByName("thisresource"+id)[0].value);
	var thisresourcechosen=Number(document.getElementsByName("resource"+id)[0].value);
...
var storCap = storage();
	...
	var tritiumToTransport = document.getElementsByName("resource4")[0].value;
	...
	if (isNaN(tritiumToTransport)){
		tritiumToTransport=0;
	}
	
	var freeCapacity = Math.max(storCap - metalToTransport - crystalToTransport - deuteriumToTransport - tritiumToTransport, 0);
...
function maxResources() {
	...
	var tritiumToTransport = Math.round(document.getElementsByName("thisresource4")[0].value);


	var freeCapacity = storCap - metalToTransport - crystalToTransport - deuteriumToTransport - tritiumToTransport;
	if (freeCapacity < 0) {
		...
		tritiumToTransport = Math.min(deuteriumToTransport, storCap - metalToTransport - crystalToTransport - deuteriumToTransport);
	}
	...
	document.getElementsByName("resource4")[0].value = Math.max(tritiumToTransport, 0);
	calculateTransportCapacity();
}
...
function calculateTransportCapacity() {
	...
	var tritium = Math.round(Math.abs(document.getElementsByName("resource4")[0].value));
	 
	transportCapacity =  storage() - metal - crystal - deuterium - tritium;
