		...
		<tr>
			<td class="transparent">
				<table>
					...
					<tr>
						<td><input type="button" class="button medium" name="galaxyLeft" value="Prev" onclick="galaxy_submit('galaxyLeft')"></td>
						<td><input type="text" class="medium" name="galaxy" value="{$galaxy}" maxlength="3" tabindex="1"></td>
						<td><input type="button" class="button medium" name="galaxyRight" value="Next" onclick="galaxy_submit('galaxyRight')"></td>
					</tr>
				</table>
			</td>
			<td class="transparent">
				<table>
					...
					<tr>
						<td><input type="button" class="button medium" name="systemLeft" value="Prev" onclick="galaxy_submit('systemLeft')"></td>
						<td><input type="text" class="medium"  name="system" value="{$system}" size="5" maxlength="3" tabindex="2"></td>
						<td><input type="button" class="button medium" name="systemRight" value="Next" onclick="galaxy_submit('systemRight')"></td>
					</tr>
				</table>
			</td>
			<td style="background-color:transparent;border:0px;" colspan="2"> 
				<input type="submit" class="button" value="{$LNG.gl_show}">
			</td>
		</tr>
	</table>
...
<div class="box">
<div class="inner"> 
<div class="titlebar">{$LNG.gl_solar_system} {$galaxy}:{$system}</div>
<div class="contents">
	<table style="width:100%;">
	<tr>
		...
		<td style="white-space: nowrap;">
			{if $currentPlanet.action}
				...
				<a href="javascript:doit(6,{$currentPlanet.planet.id},{$spyShips|json|escape:'html'})">
					<span class="icon awesome eye-open tipsy-header" title="{$LNG.gl_spy}" alt="">
				</a>{/if}
				{if $currentPlanet.action.message}
				<a href="#" onclick="return Dialog.PM({$currentPlanet.user.id})">
					<span class="icon awesome inbox tipsy-header" title="{$LNG.write_message}" alt="">
				</a>{/if}
				{if $currentPlanet.action.buddy}
                <a href="#" onclick="return Dialog.Buddy({$currentPlanet.user.id})">
					<span class="icon awesome user tipsy-header" title="{$LNG.gl_buddy_request}" alt="">
				</a>{/if}
				{if $currentPlanet.action.missle}<a href="?page=galaxy&amp;action=sendMissle&amp;galaxy={$galaxy}&amp;system={$system}&amp;planet={$planet}&amp;type=1">
					<span class="icon entypo warning tipsy-header" title="{$LNG.gl_missile_attack}" alt="">
				</a>{/if}
				{if $currentPlanet.planet.phalanx}<a href='javascript:OpenPopup(&quot;?page=phalanx&amp;galaxy={$galaxy}&amp;system={$system}&amp;planet={$planet}&amp;planettype=1&quot;, &quot;&quot;, 640, 510);'>
				<span class="icon entypo broadcast tipsy-header" title="{$LNG.gl_phalanx}" alt="">
				</a>{/if}
			{else}-{/if}
		</td>
	{/if}
