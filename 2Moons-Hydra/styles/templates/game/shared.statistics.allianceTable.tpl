...
<tr>
	<td>{$RangeInfo.rank} ({if $RangeInfo.ranking == 0}<span style='color:#5F9EA0'>*</span>{elseif $RangeInfo.ranking < 0}<span style='color:red'>{$RangeInfo.ranking}</span>{elseif $RangeInfo.ranking > 0}<span style='color:green'>+{$RangeInfo.ranking}</span>{/if})</a></td>
