	...
	else
	{
		...
		while ($StatRow = mysql_fetch_assoc($query))
		{
			...
			if ($StatRow['id'] == $CurrentUser['id'])
				$parse['player_name']     = "<a href=\"#\" onclick=\"javascript:window.open('playercard.php?playerid=" . $StatRow['id'] . "?ownid=" . $user['id'] . "',this.target,'width=650,height=600,resizable=0,status=0,menubar=0,location=0,directories=0,toolbar=0,copyhistory=0,scrollbars=1');\"><font color=\"lime\">".$StatRow['username']."</font></a>";
			else
				$parse['player_name']     = "<a href=\"playercard.php?playerid=" . $StatRow['id'] . "?ownid=" . $user['id'] . "\">".$StatRow['username']."</a>";
