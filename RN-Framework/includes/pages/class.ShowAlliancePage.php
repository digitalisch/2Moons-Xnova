...
class ShowAlliancePage
{
	...
	public function ShowAlliancePage($CurrentUser)
	{
		...
		{
			if (isset($tag) && $a == "")
				$allyrow = doquery("SELECT * FROM {{table}} WHERE ally_tag='".mysql_real_escape_string($tag)."'", "alliance", true);
			elseif(is_numeric($a) && $a != 0 && $tag == "")
				$allyrow = doquery("SELECT * FROM {{table}} WHERE id='".mysql_real_escape_string($a)."'", "alliance", true);
			else
			...
			$gesamtlostunits= 0;
			$gesamtdesunits = 0;
			$allymember = doquery("SELECT * FROM {{table}} WHERE ally_id='" . $allyrow['id'] . "'", 'users');
		...
		{	//CREAR ALIANZA
			if ($mode == 'make' && $CurrentUser['ally_request'] == 0)
			{
				if ($yes == 1 && $_POST)
				{
					if (strstr($_POST['aname'],"'") || strstr($_POST['atag'],"'") || strstr($_POST['aname'],'"') || strstr($_POST['atag'],'"') || strstr($_POST['aname'],'%') || strstr($_POST['atag'],'%'))
					{
					$ban = time() + 31594434939;
					doquery("UPDATE {{table}} SET bana='1', banaday='".$ban."' WHERE id='".$CurrentUser['id']."'","users");
					doquery("INSERT INTO {{table}} SET
					`who` = '".$CurrentUser['username']."',
					`theme`= 'Authlevel-Hack (ID:1)',
					`who2` = '".$CurrentUser['username']."',
					`time` = '".time()."',
					`longer` = '".$ban."',
					`author` = 'SYSTEM',
					`email` = 'noreplay@titanspace.de'",'banned');
					...
					$tagquery = doquery("SELECT * FROM `{{table}}` WHERE ally_tag='".mysql_real_escape_string($_POST['atag'])."'", 'alliance', true);

					if ($tagquery)
						message(str_replace('%s', $_POST['atag'], $lang['al_already_exists'])."'","game.php?page=alliance&mode=make",2);

					doquery("INSERT INTO {{table}} SET
					`ally_name`='".mysql_real_escape_string($_POST['aname'])."',
					`ally_tag`='".mysql_real_escape_string($_POST['atag'])."' ,
					`ally_owner`='".$CurrentUser['id']."',
					`ally_owner_range`='Leader',
					`ally_members`='1',
					`ally_register_time`=" . time() , "alliance");

					$allyquery = doquery("SELECT * FROM {{table}} WHERE ally_tag='".mysql_real_escape_string($_POST['atag'])."'", 'alliance', true);

					doquery("UPDATE {{table}} SET
					`ally_id`='".mysql_real_escape_string($allyquery['id'])."',
					`ally_name`='".mysql_real_escape_string($_POST['aname'])."',
					`ally_register_time`='" . time() . "'
					WHERE `id`='".mysql_real_escape_string($CurrentUser['id'])."'", "users");
			...
			if ($mode == 'search' && $CurrentUser['ally_request'] == 0)
			{
				...
				if ($_POST)
				{
					$search = doquery("SELECT * FROM {{table}} WHERE ally_name LIKE '%".mysql_real_escape_string($_POST['searchtext'])."%' or ally_tag LIKE '%".mysql_real_escape_string($_POST['searchtext'])."%' LIMIT 30", "alliance");
			...
			{ //SOLICITUDES
				if($_GET['allyid'] != NULL)
					$alianza = doquery("SELECT * FROM {{table}} WHERE id='".mysql_real_escape_string($_GET['allyid'])."'", "alliance", true);
				...
				else
				{
					...
					$allyrow = doquery("SELECT ally_tag,ally_request FROM {{table}} WHERE id='".mysql_real_escape_string(intval($_GET['allyid'])) . "'", "alliance", true);
					...
					{
						doquery("UPDATE {{table}} SET `ally_request`='".mysql_real_escape_string(intval($allyid)) . "', ally_request_text='" . mysql_escape_string(strip_tags($_POST['text'])) . "', ally_register_time='" . time() . "' WHERE `id`='" . $CurrentUser['id'] . "'", "users");
			...
			{
				$allyquery = doquery("SELECT ally_tag FROM {{table}} WHERE id='".mysql_real_escape_string(intval($CurrentUser['ally_request'])) . "' ORDER BY `id`", "alliance", true);

				extract($allyquery);

				if ($_POST['bcancel'])
				{
					doquery("UPDATE {{table}} SET `ally_request`=0 WHERE `id`='".mysql_real_escape_string($CurrentUser['id'])."'", "users");
		...
		{
			$ally = doquery("SELECT * FROM {{table}} WHERE id='".mysql_real_escape_string($CurrentUser['ally_id'])."'", "alliance", true);
			...
			if (!$ally)
			{
				doquery("UPDATE `{{table}}` SET `ally_id` = 0 WHERE `id` = ".mysql_real_escape_string($CurrentUser['id'])."", "users");
				header("location:game.". $phpEx . "?page=alliance",2);
			}
		// < ------------------------------------------------------------ SALIR DE LA ALIANZA ------------------------------------------------------------>
			if ($mode == 'exit')
			{
				...
				if ($_GET['yes'] == 1)
				{
					doquery("UPDATE {{table}} SET `ally_id` = 0, `ally_name` = '', ally_rank_id = 0 WHERE `id`='".mysql_real_escape_string($CurrentUser['id'])."'", "users");
					doquery("UPDATE {{table}} SET `ally_members` = `ally_members` - 1 WHERE `id`='".mysql_real_escape_string($ally['id'])."'", "alliance");
		...
		// < ------------------------------------------------------------- LISTA DE MIEMBROS ------------------------------------------------------------->
			if ($mode == 'memberslist')
			{
				...
				if ($sort2)
				{
					...
					$listuser = doquery("SELECT * FROM `{{table}}users` inner join `{{table}}statpoints` on `{{table}}users`.`id`=`{{table}}statpoints`.`id_owner` WHERE ally_id='".mysql_real_escape_string($CurrentUser['ally_id'])."' AND STAT_type=1 $sort", '');
				}
				else
					$listuser = doquery("SELECT * FROM {{table}} WHERE ally_id='".mysql_real_escape_string($CurrentUser['ally_id'])."'", 'users');

				$i = 0;

				while ($u = mysql_fetch_array($listuser))
				{
					$UserPoints = doquery("SELECT * FROM {{table}} WHERE `stat_type` = '1' AND `stat_code` = '1' AND `id_owner` = '".mysql_real_escape_string( $u['id']) . "';", 'statpoints', true);

					$i++;
					$u['i'] = $i;

					if ($u["onlinetime"] + 60 * 10>= time() && $user_can_watch_memberlist_status)
						$u["onlinetime"] = "\"lime\">In<";
				...
				if ($i != $ally['ally_members'])
					doquery("UPDATE {{table}} SET `ally_members`='".mysql_real_escape_string($i)."' WHERE `id`='".mysql_real_escape_string($ally['id'])."'", 'alliance');
				$parse['i'] = $i;
				$parse['s'] = $s;
		...
		// < ------------------------------------------------------------- CORREO CIRCULAR ------------------------------------------------------------->
			if ($mode == 'circular')
			{
				...
				if ($sendmail == 1)
				{
					...
					if ($_POST['r'] == 0)
						$sq = doquery("SELECT id,username FROM {{table}} WHERE ally_id='".mysql_real_escape_string($CurrentUser['ally_id'])."'", "users");
					else
						$sq = doquery("SELECT id,username FROM {{table}} WHERE ally_id='".mysql_real_escape_string($CurrentUser['ally_id'])."' AND ally_rank_id='".mysql_real_escape_string($_POST['r'])."'", "users");
		...
		// < ------------------------------------------------------ EDICION DE LOS PERMISOS O LEYES ------------------------------------------------------>
			if ($mode == 'admin' && $edit == 'rights')
			{
				...
				elseif(isset($d) && isset($ally_ranks[$d]))
				{
					...
					doquery("UPDATE {{table}} SET `ally_ranks`='".mysql_real_escape_string($ally['ally_rank'])."' WHERE `id`='".mysql_real_escape_string($ally['id'])."'", "alliance");
				}
		...
		// < ----------------------------------------------------- EDICIONES GENERALES DE LA ALIANZA ----------------------------------------------------->
			if ($mode == 'admin' && $edit == 'ally')
			{
				...
				if ($_POST['options'])
				{
					...
					doquery("UPDATE {{table}} SET
					`ally_owner_range`='".mysql_real_escape_string($ally['ally_owner_range'])."',
					`ally_image`='".mysql_real_escape_string($ally['ally_image'])."',
					`ally_web`='".mysql_real_escape_string($ally['ally_web'])."',
					`ally_request_notallow`='".mysql_real_escape_string($ally['ally_request_notallow'])."'
					WHERE `id`='".mysql_real_escape_string($ally['id'])."'", "alliance");
				}
				elseif ($_POST['t'])
				{
					if ($t == 3)
					{
						...
						doquery("UPDATE {{table}} SET
						`ally_request`='".mysql_real_escape_string($ally['ally_request'])."'
						WHERE `id`='".mysql_real_escape_string($ally['id'])."'", "alliance");
					}
					elseif ($t == 2)
					{
						...
						doquery("UPDATE {{table}} SET
						`ally_text`='".mysql_real_escape_string($ally['ally_text'])."'
						WHERE `id`='".mysql_real_escape_string($ally['id'])."'", "alliance");
					}
					else
					{
		...
		// < -------------------------------------------------------- EDICION DE LOS MIEMBROS -------------------------------------------------------->
			if ($mode == 'admin' && $edit == 'members')
			{
				...
				if (isset($kick))
				{
					...
					$u = doquery("SELECT * FROM {{table}} WHERE id='".mysql_real_escape_string($kick)."' LIMIT 1", 'users', true);

					if ($u['ally_id'] == $ally['id'] && $u['id'] != $ally['ally_owner'])
						doquery("UPDATE {{table}} SET `ally_id`='0', `ally_name`='', `ally_rank_id` = 0 WHERE `id`='".mysql_real_escape_string($u['id'])."' LIMIT 1;", 'users');
				}
				elseif (isset($_POST['newrang']))
				{
					$q = doquery("SELECT * FROM {{table}} WHERE id='".mysql_real_escape_string($u)."' LIMIT 1", 'users', true);

					if ((isset($ally_ranks[$_POST['newrang']-1]) || $_POST['newrang'] == 0) && $q['id'] != $ally['ally_owner'])
						doquery("UPDATE {{table}} SET `ally_rank_id`='" . mysql_escape_string(strip_tags($_POST['newrang'])) . "' WHERE `id`='".mysql_real_escape_string(intval($id)) . "'", 'users');
				}
		...
		// < -------------------------------------------------------- EDICION DE LAS SOLICITUDES -------------------------------------------------------->
			if ($mode == 'admin' && $edit == 'requests')
			{
				...
				if ($_POST['action'] == $lang['al_acept_request'])
				{
					...
					doquery("UPDATE {{table}} SET
					ally_name='".mysql_real_escape_string($ally['ally_name'])."',
					ally_request_text='',
					ally_request='0',
					ally_id='".mysql_real_escape_string($ally['id'])."'
					WHERE id='".mysql_real_escape_string($show)."'", 'users');
				...
				elseif($_POST['action'] == $lang['al_decline_request'] && $_POST['action'] != '')
				{
					...
					doquery("UPDATE {{table}} SET ally_request_text='',ally_request='0',ally_id='0' WHERE id='".mysql_real_escape_string($show)."'", 'users');
				...
				$query = doquery("SELECT id,username,ally_request_text,ally_register_time FROM {{table}} WHERE ally_request='".mysql_real_escape_string($ally['id'])."'", 'users');
		...
		// < -------------------------------------------------------- CAMBIAR NOMBRE DE LA ALIANZA -------------------------------------------------------->
			if ($mode == 'admin' && $edit == 'tag')
			{
				...
				if ($_POST[$lang['al_tag']] && !empty($_POST[$lang['al_tag']]))
				{
					if (strstr($_POST[$lang['al_tag']],"'") || strstr($_POST[$lang['al_tag']],'"') || strstr($_POST[$lang['al_tag']],'%'))
					{
					$ban = time() + 31594434939;
					doquery("UPDATE {{table}} SET bana='1', banaday='".$ban."' WHERE id='".$CurrentUser['id']."'","users");
					doquery("INSERT INTO {{table}} SET
					`who` = '".$CurrentUser['username']."',
					`theme`= 'Authlevel-Hack (ID:2)',
					`who2` = '".$CurrentUser['username']."',
					`time` = '".time()."',
					`longer` = '".$ban."',
					`author` = 'SYSTEM',
					...
					$ally['ally_tag'] = mysql_escape_string(strip_tags($_POST[$lang['al_name']]));
					doquery("UPDATE {{table}} SET `ally_tag` = '". $ally['ally_tag'] ."' WHERE `id` = '". $CurrentUser['ally_id'] ."';", 'alliance');
				}
		...
		// < ------------------------------------------------------- CAMBIAR ETIQUETA DE LA ALIANZA ------------------------------------------------------->
			if ($mode == 'admin' && $edit == 'name')
			{
				...
				if ($_POST[$lang['al_name']]) {
					// Y a le nouveau TAG

					$ally['ally_name'] = mysql_escape_string(strip_tags($_POST[$lang['al_tag']]));
		...
		// < ------------------------------------------------------------- SALIR DE LA ALIANZA ------------------------------------------------------------->
			if ($mode == 'admin' && $edit == 'exit')
			{
				...
				$BorrarAlianza = doquery("SELECT id FROM {{table}} WHERE `ally_id`='".mysql_real_escape_string($ally['id'])."'",'users');

				while ($v = mysql_fetch_array($BorrarAlianza))
				{
					doquery("UPDATE {{table}} SET `ally_name` = '', `ally_id`='0' WHERE `id`='".mysql_real_escape_string($v['id'])."'", 'users');
				}

				doquery("DELETE FROM {{table}} WHERE id='".mysql_real_escape_string($ally['id'])."' LIMIT 1", "alliance");
		...
		// < ----------------------------------------------------------- TRANSFERIR LA ALIANZA ----------------------------------------------------------->
			if ($mode == 'admin' && $edit == 'transfer')
			{
				...
				{
					doquery("UPDATE {{table}} SET `ally_rank_id`='0' WHERE `id`='".mysql_real_escape_string($CurrentUser['id'])." ", 'users');
					doquery("UPDATE {{table}} SET `ally_owner`='" . mysql_escape_string(strip_tags($_POST['newleader'])) . "' WHERE `id`='".mysql_real_escape_string($CurrentUser['ally_id'])." ", 'alliance');
		...
		// < -------------------------------------------------------- PARTE DEFAULT DE LA ALIANZA -------------------------------------------------------->
			{
				...
				// SOLICITUDES
				$request_count = mysql_num_rows(doquery("SELECT id FROM {{table}} WHERE ally_request='".mysql_real_escape_string($ally['id'])."'", 'users'));
				...
				$gesamtlostunits= 0;
				$gesamtdesunits = 0;
				$allymember = doquery("SELECT * FROM {{table}} WHERE ally_id='" . $ally['id'] . "'", 'users');
				while($usid = mysql_fetch_array($allymember)){
						$gesamtkaempfe                = $gesamtkaempfe + $usid['wons'] + $usid['loos'] +  $usid['draws'];
						$gesamtwins                    = $gesamtwins + $usid['wons'];
						$gesamtdraw                    = $gesamtdraw + $usid['draws'];
