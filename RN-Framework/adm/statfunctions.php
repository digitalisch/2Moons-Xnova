...
{global $resource, $pricelist, $reslist, $game_config, $xgp_root, $phpEx;
	...
	$ChooseToDelete = doquery("SELECT `id`,`db_deaktjava`,`onlinetime` FROM `{{table}}` WHERE ((`db_deaktjava` < '".$del_deleted."' AND `db_deaktjava` <> 0) OR `onlinetime` < '".$del_inactive."') and authlevel = 0", 'users');
