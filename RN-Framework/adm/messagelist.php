...
if ($user['authlevel'] < 2) die(message ($lang['not_enough_permissions']));
	...
	while ($row = mysql_fetch_assoc($Messages))
	{
		...
		$parse['mlst_data_rows'] .= parsetemplate(gettemplate('adm/messagelist_table_rows'), $bloc);
	}
	display (parsetemplate(gettemplate('adm/messagelist_body'), $parse), false, '', true, false);
?>