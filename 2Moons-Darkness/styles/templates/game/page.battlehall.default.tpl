...
<tbody>
<tr>
    <th colspan="5">{$LNG.tkb_top}</th>
</tr>
<tr>
    <td colspan="5">{$LNG.tkb_gratz}</td>
</tr>
<tr>
    <td>{$LNG.tkb_platz}</td>
	...
	<td><a href="game.php?page=battleHall&order=units&sort={if $sort == "ASC"}DESC{else}ASC{/if}"{if $order == "units"} style="font-weight:bold;"{/if}>{$LNG.tkb_units}</a></td>
    	<td><a href="game.php?page=battleHall&order=units&sort={if $sort == "ASC"}DESC{else}ASC{/if}"{if $order == "units"} style="font-weight:bold;"{/if}>Comments</a></td>
...
<tr class="day{floor($hallRow.date / 86400)} week{floor($hallRow.date / 604800)}">
    ...
    <td>{if empty($hallRow.comments)}No comments{else}{$hallRow.comments}{/if}</td>
</tr>
{/foreach}
{/foreach}
<tr>
<td colspan="5">{$LNG.tkb_legende}<span style="color:#00FF00">{$LNG.tkb_gewinner}</span><span style="color:#FF0000">{$LNG.tkb_verlierer}</span></td></tr>
</tbody>
</table>
{/block}