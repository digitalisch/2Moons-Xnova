...
<div id="buildlist" class="buildlist">
	<table style="width:760px">
		...
		<tr>
			<td style="width:70%;vertical-align:top;" class="left">
				...
				<form class="build_form" action="game.php?page=buildingsInstant" method="post">
			...
			<td>
				<div id="time" data-time="{$List.time}"><br></div>
					...
					<form action="game.php?page=buildingsInstant" method="post" class="build_form"> 	
	...
	<button type="submit" class="build_submit onlist">{$LNG.bd_cancel}</button> 		
				 ...
				 {if $modinstant == 1 && $modinstantbuilds == 1}
					...
					{if $dmavaible < 10000}
					<form action="game.php?page=buildingsInstant" method="post" class="post">
					<button type="submit" class="buttonbau gray" disabled>Instant Build</button></form>
					{else}
						<form action="game.php?page=buildingsInstant" method="post" class="post">
							<input type="hidden" name="cmd" value="instant">
                                                        <button type="submit" class="buttonbau greenbau" enabled>{$LNG.build3}</button>
						</form>
					{/if}{/if}
				{else}
			</td>
