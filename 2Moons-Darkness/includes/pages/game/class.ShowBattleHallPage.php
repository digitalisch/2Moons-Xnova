 ...
 * @info $Id: class.ShowBattleHallPage.php 2632 2013-03-18 19:05:14Z slaver7 $
 * @link http://2moons.cc/
 */
...
class ShowBattleHallPage extends AbstractPage
{
	...
	function show()
	{
		...
		while($data = $GLOBALS['DATABASE']->fetch_array($top))
		{
			...
			$nr_comm = 0;
			$comments_count = $GLOBALS['DATABASE']->query("SELECT *from `uni1_comments` where `hof_id` = '".$data['rid']."' ;"); 
			if($GLOBALS['DATABASE']->numRows($comments_count))
				$nr_comm = $GLOBALS['DATABASE']->numRows($comments_count);
