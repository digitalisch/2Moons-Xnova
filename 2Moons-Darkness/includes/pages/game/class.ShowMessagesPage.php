...
class ShowMessagesPage extends AbstractPage
{
	...
	function send() 
	{
		...
		if (empty($Subject))
		{
		 ...
		 if($USER['message_ban'] == 1){
			$this->sendJSON("You have been banned for spamming messages, for unban write ticket to admin");
		}
		...
		if($count >= 2){
			$GLOBALS['DATABASE']->query("UPDATE ".USERS." set `message_ban` = '1' where `id` = '".$USER['id']."';");
