	...
	{	
		...
		elseif ($MissionInfo['planettype'] == 2) {
			...
			if (!$UsedPlanet) {
				...
				if (!$YourPlanet) {
					if(isModulAvalible(MODULE_MISSION_ATTACK) && $GetInfoPlanet['id_owner'] != $USER['immunity_until'] > TIMESTAMP )
						$avalibleMissions[]	= 1;
					if(isModulAvalible(MODULE_MISSION_HOLD) && $GetInfoPlanet['id_owner'] !=  $USER['immunity_until'] > TIMESTAMP )
						$avalibleMissions[]	= 5;}
						
				elseif(isModulAvalible(MODULE_MISSION_STATION)) {
					$avalibleMissions[]	= 4;}
					
				if (!empty($MissionInfo['IsAKS']) && !$YourPlanet && isModulAvalible(MODULE_MISSION_ATTACK) && isModulAvalible(MODULE_MISSION_ACS) && $GetInfoPlanet['id_owner'] !=  $USER['immunity_until'] > TIMESTAMP )
					$avalibleMissions[]	= 2;

				if (!$YourPlanet && $MissionInfo['planettype'] == 3 && isset($MissionInfo['Ship'][214]) && isModulAvalible(MODULE_MISSION_DESTROY) && $GetInfoPlanet['id_owner'] !=  $USER['immunity_until'] > TIMESTAMP )
					$avalibleMissions[]	= 9;

				if ($YourPlanet && $MissionInfo['planettype'] == 3 && self::OnlyShipByID($MissionInfo['Ship'], 220) && isModulAvalible(MODULE_MISSION_DARKMATTER) && $GetInfoPlanet['id_owner'] != $USER['immunity_until'] > TIMESTAMP )
					$avalibleMissions[]	= 11;
			}
		}
