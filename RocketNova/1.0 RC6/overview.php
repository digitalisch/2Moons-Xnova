	...
	switch ($mode) {
		case 'renameplanet':
            ...
            $newname        = mysql_escape_string(strip_tags(trim( $UserPlanet )));
               ...
               message('Fehler! Sie haben ein ung&uuml;ltiges Zeichen eingegeben. ','Fehler');
               }
            ...
            if (CheckFleets($planetrow)){
               $strMessage = "Der Planet kann nich gel&ouml;scht werden, w�hrend deine Flotte unterwegs ist.";
			...
			$QryUpdateUser2    = "DELETE FROM {{table}} ";
			$QryUpdateUser2   .= "WHERE ";
			$QryUpdateUser2   .= "`galaxy` = '". $galaxyrow['galaxy'] ."'";
			$QryUpdateUser2   .= "`system` = '". $galaxyrow['system'] ."'";
			$QryUpdateUser2   .= "`lunapos` = '". $galaxyrow['planet'] ."' LIMIT 1"; // ; nach LIMIT 1 weg gemacht
			doquery( $QryUpdateUser2, "lunas");
		...
		default:
			...
			$XpMinierUp  = $user['lvl_minier'] * 1000;
...
// Bewegte Resourcen ENDE
      ...
      $parse['colonie_empire'] = $lang['colonie_empire'];
			...
			$parse['lvl_up_minier'] = $LvlMinier * 1000;
