...
if ($user['urlaubs_modus'] == 1){
	...
	if (($user[$resource[108]] + ($user['rpg_commandant'] * 3) + 1 ) <= $CurrentFlyingFleets) {
	...
	//FIX SONDAS ESPIONAJE Y RECICLADORES
   if($_POST['mission'] == 6 AND $UserSpyProbes < 1){
      $ResultMessage = "604;".$lang['gs_c604k']."|".$CurrentFlyingFleets." ".$UserSpyProbes." ".$UserRecycles." ".$UserMissiles;
      die($ResultMessage);
   }
   if($_POST['mission'] == 8 AND $UserRecycles < 1){
	...
	$fleet['end_time']   = ($Duration * 2.2) + time();
	...
	foreach ($FleetArray as $Ship => $Count) {
		...
		$spd              = 40000 / ($Duration * $SpeedFactor - 10) * sqrt($Distance * 10 / $ShipSpeed);
		$basicConsumption = $pricelist[$Ship]["consumption"] * $Count ;
		$consumption     += $basicConsumption * $Distance / 40000 * (($spd / 10) + 1) * (($spd / 10) + 1);
