...
function ShowProductionTable ($CurrentUser, $CurrentPlanet, $BuildID, $Template) {
	...
	$Prod[3]          = ((eval($ProdGrid[$BuildID]['formule']['deuterium']) * $game_config['resource_multiplier']) * (1 + ($CurrentUser['rpg_geologue']  * 0.05)));
	$Prod[4] 		      = ((eval($ProdGrid[$BuildID]['formule']['energy'])    * $game_config['resource_multiplier'])); // * (1 + ($CurrentUser['rpg_ingenieur'] * 0.05))); // * (1 + ($CurrentUser['energy_tech'] * 0.01)));
	
  $BuildLevel       = "";

	$ActualProd       = floor($Prod[$BuildID]);
	if ($BuildID != 12) {
		$ActualNeed       = ceil($Prod[4]);
	} else {
		...
		if ($BuildID != 42) {
			...
			if ($ProdFirst > 0) {
				if ($BuildID != 12) {
					...
					$bloc['build_gain']      = "<font color=\"lime\">(". pretty_number(ceil($Prod[4] - $ProdFirst)) .")</font>";
				}
			} else {
				$bloc['build_gain']      = "";
			}
			if ($BuildID != 12) {
				...
				$bloc['build_need']      = colorNumber( pretty_number(ceil($Prod[4])) );
				$bloc['build_need_diff'] = colorNumber( pretty_number(ceil($Prod[4] - $ActualNeed)) );
			} else {
				$bloc['build_prod']      = pretty_number(ceil($Prod[4]));
				$bloc['build_prod_diff'] = colorNumber( pretty_number(ceil($Prod[4] - $ActualProd)) );
