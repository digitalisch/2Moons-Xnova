...
function GalaxyRowUser ( $GalaxyRow, $GalaxyRowPlanet, $GalaxyRowUser, $Galaxy, $System, $Planet, $PlanetType ) {
	...
	if ($GalaxyRowUser && $GalaxyRowPlanet["destruyed"] == 0) {
		...
		if ($GalaxyRowUser['authlevel'] == 1) {
			$go = "<font color=\"lime\"><blink>Go</blink></font>";
		}
		$sgo = "";
		if ($GalaxyRowUser['authlevel'] == 2) {
			$go = "<font color=\"blue\"><blink>SGo</blink></font>";
		}
		$admin = "";
		if ($GalaxyRowUser['authlevel'] == 3) {
			$go = "<font color=\"red\"><blink>A</blink></font>";
		}
		$serveradmin = "";
		if ($GalaxyRowUser['authlevel'] == 4) {
			$go = "<font color=\"red\"><blink>SA</blink></font>";
		}
		$Systemtart = $User2Points['total_rank'];
		if (strlen($Systemtart) < 4) {
			$Systemtart = 1;
		} else {
		...
		if ($GalaxyRowUser['id'] != $user['id']) {
			...
			$Result .= "</tr><tr>";
			$Result .= "<td><a href=profil.php?id=".$GalaxyRowUser['id'].">".$lang['profile']."</a></td>";
			$Result .= "</tr><tr>";
		}
		...
		$Result .= $Systemtatus;
		$Result .= $Systemtatus3;
		$Result .= $GalaxyRowUser['username'];
		$Result .= $Systemtatus6;
		$Result .= $Systemtatus;
		$Result .= $Systemtatus2;
		$Result .= $Systemtatus10." ".$go;
		$Result .= $Systemtatus7;
		$Result .= $Systemtatus9;
		$Result .= "</span></a>";
	}
