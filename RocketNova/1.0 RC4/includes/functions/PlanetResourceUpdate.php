...
function PlanetResourceUpdate ( $CurrentUser, &$CurrentPlanet, $UpdateTime, $Simul = false ) {
	...
	for ( $ProdID = 0; $ProdID < 300; $ProdID++ ) {
		if ( in_array( $ProdID, $reslist['prod']) ) {
			...
			$Caps['deuterium_perhour'] +=  ( eval  ( $ProdGrid[$ProdID]['formule']['deuterium'] ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.05 ) ) );
		if ($ProdID < 4) {
            $Caps['energy_used']   +=  ( eval  ( $ProdGrid[$ProdID]['formule']['energy']) * ( $game_config['resource_multiplier'] ) );
         } elseif ($ProdID >= 4 ) {
            $Caps['energy_max']    +=  ( eval  ( $ProdGrid[$ProdID]['formule']['energy']    ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_ingenieur'] * 0.05 ) )); //* ( 1 + ( $CurrentUser['energy_tech'] * 0.01 ) ) );
			}
		}
	}
...
if ( $CurrentPlanet['metal'] <= $MaxMetalStorage ) {
	...
	if($Simul == true) {
		...
		Metall pro Stunde: 		'.intval($CurrentPlanet['metal_perhour'])		.'<br>     
		Kristall pro Stunde: 	'.intval($CurrentPlanet['crystal_perhour'])		.'<br>   
		Deuterium pro Stunde: 	'.intval($CurrentPlanet['deuterium_perhour'])	.'<br> 
