	...
	if ($GalaxyRowUser['ally_id'] && $GalaxyRowUser['ally_id'] != 0) {
		...
		if ($allyquery) {
			...
			if(($user['id'] == $id_owner['ally_owner']) && ($allyquery['ally_name'] != $user['ally_name'])) {
				$Result .= "<tr>";
				$Result .= "<td><a href=alliance_diplo.php?add=".$allyquery['id'].">Allianze einen Pakt anbieten</a></td>";
				$Result .= "</tr>";
			}
