...
function getnamebyid ($id) {
	switch ($id) {
		...
		case '216':
			...
			break;
		case '217':
			$name = "Evolutions Transporter";
			break;
		case '218':
			$name = "Sternenzerst&ouml;rer";
			break;
		case '401':
			$name = "Rak.";
			break;
					...
					while ($fire == true) {
						...
						if ($killed1 != 1) {
							...
							if ($feuerkraft <= ($initialisierung['verteidiger']['schilde'][$schuss_nach_id2][$schuss_nach_id] / 100)) {
								...
								if ($ship_rapidfire) {
									if ($rapidfire[$schiffid][$rapidfire_vs] > 1 && $rapidfire_anzahl <= 1250) {
...
$pricelist = array(
	...
	217 => array ( 'metal' =>   90000, 'crystal' =>   60000, 'deuterium' =>   1500, 'energy' => 0, 'factor' => 1, 'consumption' => 80 , 'consumption2' => 160 , 'speed' => 6500, 'speed2' => 10000, 'capacity' => 300000 ),
