...
switch ($MessPageMode):
	case 'write':
		...
		$subject = (empty($_REQUEST['subject']))
						? $lang['mess_no_subject']
						: strip_tags($_REQUEST['subject']);
...
//                                        $_POST['text'] = str_replace('\r\n', '<br />', $_POST['text']);
	...
	case 'show':
		...
		if ($MessCategory == 100) {
			...
			while ($CurMess = mysql_fetch_array($UsrMess)) {
				if($CurMess['message_class'] == 1){
					$class = $lang['MESSAGES_PRIORITY_LOW'];
				}elseif($CurMess['message_class'] == 2){
					$class = $lang['MESSAGES_PRIORITY_NORMAL'];
				}elseif($CurMess['message_class'] == 3){
					$class = "<blink><font color=\"red\">".$lang['MESSAGES_PRIORITY_HIGH']."</blink></font>";
				}else{
					$class = $lang['MESSAGES_PRIORITY_NORMAL'];
				}
				$page .= "\n<tr>";
		...
		if($_GET['del'] == 'ja'){
			if($query['message_owner'] == $user['id']){
				...
				if ($MessHere) {
					...
					message($lang['MESSAGES_DELETION_SUCCESS'], $lang['MESSAGES_SUCCESS_TITLE']);
				}
			}
		}else{
			$query           = doquery("SELECT * FROM {{table}}messages WHERE `message_id` = '{$_GET['ms']}';", '', true);
			if($query['message_class'] == 1){
				$class = $lang['MESSAGES_PRIORITY_LOW'];
			}elseif($query['message_class'] == 2){
				$class = $lang['MESSAGES_PRIORITY_NORMAL'];
			}elseif($query['message_class'] == 3){
				$class = "<blink><font color=\"red\">".$lang['MESSAGES_PRIORITY_HIGH']."</blink></font>";
			}else{
				$class = $lang['MESSAGES_PRIORITY_NORMAL'];
			}
			...
			if($query['message_sender'] == $user['id'] or $query['message_owner'] == 		$user['id']){
				...
				if($query['message_owner'] == $user['id']){
					$page .= "<th colspan=\"3\"><form action=\"messages.php\" method=\"post\"><input name=\"meldentyp\" type=\"hidden\" value=\"melden\" />
						<input name=\"meldenid\" type=\"hidden\" value=\"{$query['message_id']}\" />
						<input name=\"\" type=\"submit\" value=\"".$lang['MESSAGES_BUTTON_SUBMIT']."\" /></form></th>";
					$page .= "</tr><tr>";
				}
	...
	case 'melden':
		if($_POST['gemeldet'] == 'gemeldet'){
			$Melden = doquery("UPDATE {{table}} SET `message_melden_grund` = '{$_POST['message_melde_grund']}',`message_melden` = '1',`message_melden_time` = '{$_POST['message_melde_time']}' WHERE `message_id` = '{$_POST['meldenid']}'", 'messages');
			message($lang['MESSAGES_MESSAGE_REPORTED'],$lang['MESSAGES_SUCCESS_TITLE']);
		}else{
			...
			if($query['message_owner'] == $user['id']){
				...
				$page .= "</tr><tr>";
				$page .= "                <th colspan=\"3\"><input name=\"\" type=\"submit\" value=\"".$lang['MESSAGES_BUTTON_SUBMIT']."\" /></th>";
				$page .= "</tr><tr>";
