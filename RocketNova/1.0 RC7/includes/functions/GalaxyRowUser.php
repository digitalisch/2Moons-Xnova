...
function GalaxyRowUser ( $GalaxyRow, $GalaxyRowPlanet, $GalaxyRowUser, $Galaxy, $System, $Planet, $PlanetType ) {
	...
	if ($GalaxyRowUser && $GalaxyRowPlanet["destruyed"] == 0) {
		...
		$Systemtatus12 = "<br><font color=\"yellow\">".$lang['angriffszone']." ".$GalaxyRowUser[angriffszone]."</font>";
		
		$Systemtatus4 = $User2Points['total_rank'];
		if ($Systemtatus2 != '') {
			$Systemtatus6 = "<font color=\"white\">(</font>";
			$Systemtatus11 = "<font color=\"white\">)</font>";
		}
		if ($Systemtatus2 == '') {
			$Systemtatus6 = "";
			$Systemtatus11 = "";
		...
		if ($GalaxyRowUser['authlevel'] == 4) {
			$admin = "<font color=\"red\">".$lang['ADMIN_SHORTCUT']."</font>";
		}
		$sgo = "";
		if ($GalaxyRowUser['authlevel'] == 3) {
			$sgo = "<font color=\"orange\">".$lang['SGO_SHORTCUT']."</font>";
		}
		$go = "";
		if ($GalaxyRowUser['authlevel'] == 2) {
			$go = "<font color=\"green\">".$lang['GO_SHORTCUT']."</font>";
		}
		$mod = "";
		if ($GalaxyRowUser['authlevel'] == 1) {
			$mod = "<font color=\"green\">".$lang['MOD_SHORTCUT']."</font>";
		}
		$Systemtatus7 = "";
		if ($GalaxyRowUser['authlevel'] == 4) {
		...
		$Result .= "<tr></tr>";
		$Result .= "<td><a href=playercard.php?playerid=".$GalaxyRowUser['id']."&ownid=".$user['id'].">".$lang['gl_overview']."</a></td>";
		$Result .= "</tr>";
		$Result .= "</table>\"";
