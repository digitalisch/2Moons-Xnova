...
function BuildFlyingFleetTable () {
	...
	while ( $CurrentFleet = mysql_fetch_assoc( $FlyingFleets ) ) {
		...
		$Bloc['Mission'] .= "<br>". (($CurrentFleet['fleet_mess'] == 1) ? $lang['flying_return'] : $lang['flying_away'] );
		...
		$Bloc['St_Posit'] = "[".$CurrentFleet['fleet_start_galaxy'] .":". $CurrentFleet['fleet_start_system']
				.":". $CurrentFleet['fleet_start_planet'] ."]<br>"
				. ( ($CurrentFleet['fleet_start_type'] == 1) ? "[".$lang['flying_planet']."]" : (($CurrentFleet['fleet_start_type'] == 2) ? $lang['flying_debris'] : $lang['flying_moon']  )) ."";
		...
		$Bloc['En_Posit'] = "[".$CurrentFleet['fleet_end_galaxy'] .":". $CurrentFleet['fleet_end_system'] .":". $CurrentFleet['fleet_end_planet']."]<br>"
			. ( ($CurrentFleet['fleet_end_type'] == 1) ? "[".$lang['flying_planet']."]" : (($CurrentFleet['fleet_end_type'] == 2) ? $lang['flying_debris'] : $lang['flying_moon']  )) ."";
