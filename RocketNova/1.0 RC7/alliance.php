...
if ($_GET['mode'] == 'ainfo') {
	...
	if (isset($_GET['tag'])) {
		...
		message($lang['ALLIANCE_NOT_FOUND'], $lang['ALLIANCE_TITLE']);
	}
	// Si no existe
	if (!$allyrow) {
		message($lang['ALLIANCE_NOT_FOUND'], $lang['ALLIANCE_TITLE']);
	...
	if ($ally_description != "") {
		...
		$ally_description = "<tr><th colspan=2 height=100>".$lang['ALLIANCE_MISSING_DESC']."</th></tr>";
	...
	if ($ally_web != "") {
		...
		#############################################
		# Anzeige der Pakte auf der Allypage Extern #
		#############################################
		
		$ally_pakt = doquery("SELECT `pakt_owner_id_1`, `pakt_owner_id_2`, `pakt_name_1`, `pakt_name_2`, `pakt` FROM {{table}} ".
		"WHERE (`pakt_owner_id_1` = '". $a ."' OR `pakt_owner_id_2` = '". $a ."')".
		"OR (`pakt_name_1` = '". $tag ."' OR `pakt_name_2` = '". $tag ."') AND `akzeptiert` = '0'", "diplo");
				
		$ally['ally_pakt_BND'] = "<center><b><u><font color=\"#FF9900\">".$lang['ALLIANCE_ALLY']."</font></u></b><br /><br />";
		$ally['ally_pakt_NAP'] = "<center><b><u><font color=\"#FFFF00\">".$lang['ALLIANCE_NEUTRAL']."</font></u></b><br /><br />";
		$ally['ally_pakt_WAR'] = "<center><b><u><font color=\"#FF0000\">".$lang['ALLIANCE_WAR']."</font></u></b><br /><br />";
		while($Anzeigen = mysql_fetch_array($ally_pakt)){
			if(($Anzeigen['pakt'] == 1) && (($Anzeigen['pakt_owner_id_1'] == $a) or ($Anzeigen['pakt_name_1'] == $tag))){
				$ally['ally_pakt_BND'] .= $Anzeigen['pakt_name_2']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 1) && (($Anzeigen['pakt_owner_id_2'] == $a) or ($Anzeigen['pakt_name_2'] == $tag))){
				$ally['ally_pakt_BND'] .= $Anzeigen['pakt_name_1']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 2) && (($Anzeigen['pakt_owner_id_1'] == $a) or ($Anzeigen['pakt_name_1'] == $tag))){
				$ally['ally_pakt_NAP'] .= $Anzeigen['pakt_name_2']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 2) && (($Anzeigen['pakt_owner_id_2'] == $a) or ($Anzeigen['pakt_name_2'] == $tag))){
				$ally['ally_pakt_NAP'] .= $Anzeigen['pakt_name_1']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 3) && (($Anzeigen['pakt_owner_id_1'] == $a) or ($Anzeigen['pakt_name_1'] == $tag))){
				$ally['ally_pakt_WAR'] .= $Anzeigen['pakt_name_2']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 3) && (($Anzeigen['pakt_owner_id_2'] == $a) or ($Anzeigen['pakt_name_2'] == $tag))){
				$ally['ally_pakt_WAR'] .= $Anzeigen['pakt_name_1']."<br />";
			}
		}
		
		$ally['ally_pakt_BND'] .= "</center><br />";
		$ally['ally_pakt_NAP'] .= "</center><br />";
		$ally['ally_pakt_WAR'] .= "</center>";
		
		$lang['ally_pakt_BND'] = $ally['ally_pakt_BND'];
		$lang['ally_pakt_NAP'] = $ally['ally_pakt_NAP'];
		$lang['ally_pakt_WAR'] = $ally['ally_pakt_WAR'];
	...
	/*Eumele mod */
	$seeallyid = $allyrow['id'];
	include($rocketnova_root_path . 'alliance_stat.' . $phpEx);
	/*eumele mod*/
	...
	if ($user['ally_id'] == 0) {
		$lang['bewerbung'] = "<tr>
	  <th>".$lang['ALLIANCE_APPLY']."</th>
	  <th><a href=\"alliance.php?mode=apply&amp;allyid=" . $id . "\">".$lang['ALLIANCE_APPLY_NOW']."</a></th>
...
if ($user['ally_id'] == 0) { // Sin alianza
	...
	*/
		if ($yes == 1 && $_POST) {
			...
			$allyquery = doquery("SELECT * FROM {{table}} WHERE ally_tag='$ally_tag'", 'alliance', true);
	...
	if ($mode == 'search' && $user['ally_request'] == 0) { 
		$parse = $lang;
		$lang['searchtext'] = mysql_real_escape_string($_POST['searchtext']);
	...
	if ($user['ally_request'] != 0) { // Esperando una respuesta
		...
		display($page, $lang['ALLIANCE_APPLICATION_TITLE']);
...
elseif ($user['ally_id'] != 0 && $user['ally_request'] == 0) { 
	...
	if ($mode == 'memberslist') { // Mitgliederliste
		...
		$u['bava'] = $u['avatar'];
		$u['player_name'] = "<a href=\"playercard.php?playerid=" . $u['id'] . "?ownid=" . $user['id'] . "\">".$u['username']."--</a>";
	...
	*/
		...
		*/
			$page = MessageForm($lang['Circular_sended'], $lang['ALLIANCE_MEMBER_MAIL'] . $list, "alliance.php", $lang['Ok'], true);
	...
	if ($mode == 'admin' && $edit == 'ally') { // Administrar la alianza *pendiente urgente*
		...
		if ($_POST['options']) {
			...
			if ($ally['ally_request_notallow'] != 0 && $ally['ally_request_notallow'] != 1) {
				message($lang['ALLIANCE_APPLICATION_OPTIONS'], $lang['ALLIANCE_ERROR']);
				exit;
			}
			...
			if ($t == 3) {
				...
				$ally['ally_description'] = mysql_real_escape_string(strip_tags($_POST['text'], '<a><p><br><b><u><big><h1><h2><i><font>'));
		...
		$lang['Disolve_alliance'] = MessageForm($lang['ALLIANCE_CLOSE_ALLIANCE'], "", "?mode=admin&edit=exit", $lang['Continue']);
	...
	*/
	...
	if ($mode == 'admin' && $edit == 'requests') { // Administrar solicitudes
		...
		if ($_POST['action'] == "Akzeptieren") {
			...
			`message_type`='2',
			`message_from`='{$ally['ally_tag']}',
			`message_subject`='[" . $ally['ally_name'] . "] Bewerbung angenommen!',
			`message_text`='Gl&uuml;ckwunsch!<br>Die Allianz <b>" . $ally['ally_name'] . "</b> hat ihre Bewerbung <b>aktzeptiert</b>!<br>Beg&uuml;ndung:<br>" . mysql_real_escape_string($_POST['text']) . "'", "messages");
			...
			`message_type`='2',
			`message_from`='{$ally['ally_tag']}',
			`message_subject`='[" . $ally['ally_name'] . "] Bewerbung abgelehnt!',
			`message_text`='<br>Die Allianz <b>" . $ally['ally_name'] . "</b> hat ihre Bewerbung <b>nicht aktzeptiert</b>!<br>Begr&uuml;ndung:<br>" . mysql_real_escape_string($_POST['text']) . "'", "messages");
		...
		if ($parse['list'] == '') {
			$parse['list'] = '<tr><th colspan=2>'.$lang['ALLIANCE_NO_APPLICATIONS'].'</th></tr>';
		}
		// Con $show
	...
	if ($mode == 'admin' && $edit == 'give') { // Als Admin austreten
        ...
        foreach ($ally_ranks as $id => $x_rank) {
            
            if ($x_rank['rechtehand'] == 1) {
                
                $ally_user = doquery("SELECT * FROM {{table}} WHERE ally_id={$ally['id']} AND ally_rank_id = $id 1 AND id <> {$user['id']} LIMIT 1", "users", true);
                if ($ally_user != false ) {
                    
                    doquery("UPDATE {{table}} SET `ally_owner`={$ally_user[id]} WHERE id='{$ally['id']}'", "alliance");
                    header('Location: alliance.php?mode=exit&yes=1');
                    exit;
                } else {
        ...
        message($lang['ALLIANCE_DELETION_IMPOSSIBLE']);
    }
	...
	*/
	...
	
	{
	 // Default *falta revisar...*
		...
		#############################################
		# Anzeige der Pakte auf der Allypage intern #
		#############################################
		
		$ally_pakt = doquery("SELECT `pakt_owner_id_1`, `pakt_owner_id_2`, `pakt_name_1`, `pakt_name_2`, `pakt` FROM {{table}} ".
		"WHERE (`pakt_owner_id_1` = '". $ally['id'] ."' OR `pakt_owner_id_2` = '". $ally['id'] ."') AND `akzeptiert` = '0'", "diplo");
		
		$ally['ally_pakt_BND'] = "<center><b><u><font color=\"#FF9900\"> B&uuml;ndniss </font></u></b><br /><br />";
		$ally['ally_pakt_NAP'] = "<center><b><u><font color=\"#FFFF00\"> Friedensvertrag </font></u></b><br /><br />";
		$ally['ally_pakt_WAR'] = "<center><b><u><font color=\"#FF0000\"> Krieg </font></u></b><br /><br />";
		while($Anzeigen = mysql_fetch_array($ally_pakt)){
			if(($Anzeigen['pakt'] == 1) && ($Anzeigen['pakt_owner_id_1'] == $ally['id'])){
				$ally['ally_pakt_BND'] .= $Anzeigen['pakt_name_2']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 1) && ($Anzeigen['pakt_owner_id_2'] == $ally['id'])){
				$ally['ally_pakt_BND'] .= $Anzeigen['pakt_name_1']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 2) && ($Anzeigen['pakt_owner_id_1'] == $ally['id'])){
				$ally['ally_pakt_NAP'] .= $Anzeigen['pakt_name_2']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 2) && ($Anzeigen['pakt_owner_id_2'] == $ally['id'])){
				$ally['ally_pakt_NAP'] .= $Anzeigen['pakt_name_1']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 3) && ($Anzeigen['pakt_owner_id_1'] == $ally['id'])){
				$ally['ally_pakt_WAR'] .= $Anzeigen['pakt_name_2']."<br />";
				
			}elseif(($Anzeigen['pakt'] == 3) && ($Anzeigen['pakt_owner_id_2'] == $ally['id'])){
				$ally['ally_pakt_WAR'] .= $Anzeigen['pakt_name_1']."<br />";
			}
		}
