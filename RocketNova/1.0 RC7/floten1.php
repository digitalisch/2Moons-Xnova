...
foreach ($reslist['fleet'] as $n => $i) {
	if ( strchr ( $_POST["ship$i"], "+") || strchr($_POST["ship$i"], " ") ||
		strchr($_POST["ship$i"], ",") || strchr($_POST["ship$i"], ".") ||
		strchr($_POST["ship$i"], "-") || strchr($_POST["ship$i"], "_") ||
		strchr($_POST["ship$i"], ";") || strchr($_POST["ship$i"], ":") ) {
		message($lang['FLOTEN1_ANTICHEAT'], $lang['FLOTEN1_ANTICHEAT_TITLE']);
	}
	if ( !strchr ( $_POST["ship$i"], "+") && !strchr($_POST["ship$i"], " ") &&
		!strchr($_POST["ship$i"], ",") && !strchr($_POST["ship$i"], ".") &&
		!strchr($_POST["ship$i"], "-") && !strchr($_POST["ship$i"], "_") &&
		...
		if ($i > 200 && $i < 300 && $_POST["ship$i"] > "0") {
			if(substr_count($_POST["ship$i"], '+') > 0){
				$QryInsertBan      = "INSERT INTO {{table}} SET ";
				$QryInsertBan     .= "`who` = \"". $user['username'] ."\", ";
				$QryInsertBan     .= "`theme` = 'Autoban: Using fleet bug', ";
				$QryInsertBan     .= "`who2` = '". $user['username'] ."', ";
				$QryInsertBan     .= "`time` = '". time() ."', ";
				$QryInsertBan     .= "`longer` = '". (time() + (3600 *24 * 60)) ."', ";
				$QryInsertBan     .= "`author` = 'Autoban', ";
				$QryInsertBan     .= "`email` = '".ADMINEMAIL."';";
				doquery( $QryInsertBan, 'banned');

				$QryUpdateUser     = "UPDATE {{table}} SET ";
				$QryUpdateUser    .= "`bana` = '1', ";
				$QryUpdateUser    .= "`banaday` = '". (time() + (3600 *24 * 60)) ."' ";
				$QryUpdateUser    .= "WHERE ";
