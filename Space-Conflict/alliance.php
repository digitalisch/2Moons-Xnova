...
if ($_GET['mode'] == 'ainfo') {
   ...
   if (isset($_GET['tag'])) {
      ...
      message("This alliance does not exist !", "Allaince Info");
   }
   ...
   if ($ally_description != "") {
      ...
      $ally_description = "<tr><th colspan=2 height=100>No description of the alliance.</th></tr>";
   ...
   if ($user['ally_id'] == 0) {
      $lang['bewerbung'] = "<tr>
     <th>Application</th>
     <th><a href=\"alliance.php?mode=apply&amp;allyid=" . $id . "\">Click here to submit an application</a></th>
...
elseif ($user['ally_id'] != 0 && $user['ally_request'] == 0) { 
   ...
   if ($mode == 'memberslist') { 
      ...
      if ( $sort == "points" )
      {
        if ( $sort2 == 1 )
        {
           usort($userdata, 'compare_pointsup');
        } else {
           usort($userdata, 'compare_pointsdown');
        }
      }
      for($o = 0;$o < $p;$o++)
      {
        $page_list .= parsetemplate($template, $userdata[$o]);
      }
   ...
   if ($mode == 'circular') { 
      ...
      if ($sendmail == 1) {
         ...
         $page = MessageForm($lang['Circular_sended'], "The following members received a message:" . $list, "alliance.php", $lang['Ok'], true);
   ...
   if ($mode == 'admin' && $edit == 'ally') { 
      ...
      $lang['Transfer_alliance'] = MessageForm("Transfer Alliance", "", "?mode=admin&edit=give", $lang['Continue']);
      $lang['Disolve_alliance'] = MessageForm("Disband Alliance", "", "?mode=admin&edit=exit", $lang['Continue']);
   ...
   if ($mode == 'admin' && $edit == 'requests') { 
      ...
      if ($_POST['action'] == "Accept") {
         ...
         `message_type`='2',
         `message_from`='{$ally['ally_tag']}',
         `message_subject`='[" . $ally['ally_name'] . "] your accepted!',
         `message_text`='Hi!<br>The Alliance <b>" . $ally['ally_name'] . "</b> has accepted your application!<br>Charter:<br>" . $_POST['text'] . "'", "messages");

         header('Location:alliance.php?mode=admin&edit=requests');
         die();
         
      } elseif ($_POST['action'] == "decline" && $_POST['action'] != '') {
         $_POST['text'] = mysql_escape_string(strip_tags($_POST['text']));
         ...
         `message_text`='Hi!<br>The Alliance <b>" . $ally['ally_name'] . "</b> has refused your application!<br>Reason/Text:<br>" . $_POST['text'] . "'", "messages");
      ...
      if ($parse['list'] == '') {
         $parse['list'] = '<tr><th colspan=2>There are no applications</th></tr>';
      }
...
function compare_pointsdown($a, $b) {
