...
function check_urlaubmodus_time () {
	...
	if ($game_config['urlaubs_modus_erz'] == 1) {
		...
		if ($user['urlaubs_modus'] == 1 && $urlaub_modus_time_soll > $time_jetzt) {
			...
			message("You are in vacation mode!<br>Vacation mode ends $soll_datum $soll_uhrzeit<br>	Please wait until vacation mode is over to change your options.", "Vacation Mode");
		}
	}
}
...
function CalculateMaxPlanetFields (&$planet) {
	...
	return $planet["field_max"] + ($planet[ $resource[33] ] * FIELDS_BY_TERRAFORMER_LEVEL);
}
