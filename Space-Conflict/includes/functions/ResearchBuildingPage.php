		...
		if ( is_numeric($Techno) ) {
			if ( in_array($Techno, $reslist['tech']) ) {
				...
				switch($TheCommand){
					case 'cancel':
						if ($ThePlanet['b_tech_id'] == $Techno) {
							$costs                        = GetBuildingPrice($CurrentUser, $CurrentPlanet, $Techno, true, $ForDestroy);
							$CurrentPlanet['metal']      += $costs['metal'];
							$CurrentPlanet['crystal']    += $costs['crystal'];
							$CurrentPlanet['deuterium']  += $costs['deuterium'];
							$CurrentPlanet['tachyon']    += $costs['tachyon'];
					...
					case 'search':
						if ( IsTechnologieAccessible($CurrentUser, $WorkingPlanet, $Techno) &&
							...
							$WorkingPlanet['tachyon']    -= $costs['tachyon'];
				...
				if ($UpdateData == true) {
					...
					$QryUpdatePlanet .= "`tachyon` = '".     $WorkingPlanet['tachyon']     ."' ";
