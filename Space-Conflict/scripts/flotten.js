...
function maxResource(id) {
	...
	var tachyonToTransport = parseInt(document.getElementsByName("resource5")[0].value);
	...
	if (isNaN(tachyonToTransport)){
		tachyonToTransport=0;
	}

	var freeCapacity = Math.max(storCap - metalToTransport - crystalToTransport - deuteriumToTransport - tachyonToTransport, 0);
...
function maxResources() {
	...
	var tachyonToTransport = document.getElementsByName("thisresource5")[0].value;

	var freeCapacity = storCap - metalToTransport - crystalToTransport - deuteriumToTransport - tachyonToTransport;
	if (freeCapacity < 0) {
		...
		tachyonToTransport = Math.min(tachyonToTransport, storCap - metalToTransport - crystalToTransport - deuteriumToTransport);
	}
	...
	document.getElementsByName("resource5")[0].value = Math.max(tachyonToTransport, 0);
	calculateTransportCapacity();
}
...
function calculateTransportCapacity() {
	...
	var tachyon = Math.abs(document.getElementsByName("resource5")[0].value);

	transportCapacity =  storage() - metal - crystal - deuterium - tachyon;
...
document.forms[0].mission[6].disabled = false;
...
function showResources() {
	...
	document.getElementsByName('resource5')[0].disabled = false;
...
function hideResources() {
	...
	document.getElementsByName('resource5')[0].disabled = true;
	document.getElementsByName('holdingtime')[0].disabled = true;
}

function setShips(s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s27,s28,s29,s30){
