...
function BuildRessourcePage ( $CurrentUser, $CurrentPlanet ) {
	...
	foreach($reslist['prod'] as $ProdID) {
      if ($CurrentPlanet[$resource[$ProdID]] > 0 && isset($ProdGrid[$ProdID])) {
         ...
         $metal     = floor( eval ( $ProdGrid[$ProdID]['formule']['metal']     ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.1 ) ) * ( 1 + ( $CurrentUser['rpg_indy']  * 0.25 ) ) );
         $crystal   = floor( eval ( $ProdGrid[$ProdID]['formule']['crystal']   ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.1 ) ) * ( 1 + ( $CurrentUser['rpg_indy']  * 0.25 ) ) );
         $deuterium = floor( eval ( $ProdGrid[$ProdID]['formule']['deuterium'] ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.1 ) ) * ( 1 + ( $CurrentUser['rpg_indy']  * 0.25 ) ) );
         $tachyon   = floor( eval ( $ProdGrid[$ProdID]['formule']['tachyon']   ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['tach_extract_tech'] * 0.1) ) );

         if( $ProdID >= 5 ) {
            $energy = floor( eval ( $ProdGrid[$ProdID]['formule']['energy']    ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_ingenieur'] * 0.05 ) ) * ( 1 + ( $CurrentUser['rpg_acad']  * 0.25 ) ) );
         } else $energy = floor( eval ( $ProdGrid[$ProdID]['formule']['energy']    ) * ( $game_config['resource_multiplier'] ) );
         if ($energy > 0) {
            $CurrentPlanet['energy_max']    += ( $energy * ( 1 + ( $CurrentUser['rpg_ingenieur'] * 0.05 ) ) );
         } else {
         ...
         $CurrentPlanet['tachyon_perhour']   += $tachyon;
			...
			$CurrRow['tachyon_type']             = pretty_number ( $tachyon   );
			...
			$CurrRow['tachyon_type']             = colorNumber ( $CurrRow['tachyon_type']   );
	...
	$parse['tachyon_basic_income']   = $game_config['tachyon_basic_income']   * $game_config['resource_multiplier'];
	...
	if ($CurrentPlanet['tachyon_max'] < $CurrentPlanet['tachyon']) {
		$parse['tachyon_max']     = "<font color=\"#ff0000\">";
	} else {
		$parse['tachyon_max']     = "<font color=\"#00ff00\">";
	}

	$parse['tachyon_max']         .= pretty_number($CurrentPlanet['tachyon_max'] / 1000) ." ". $lang['k']."</font>";
	...
	$parse['tachyon_total']         = colorNumber( pretty_number( floor( ( $CurrentPlanet['tachyon_perhour']   * 0.01 * $parse['production_level'] ) + $parse['tachyon_basic_income'])));
	...
	$parse['daily_tachyon']        = floor($CurrentPlanet['tachyon_perhour']    * 24      * 0.01 * $parse['production_level'] + $parse['tachyon_basic_income']   * 24      );
	$parse['weekly_tachyon']       = floor($CurrentPlanet['tachyon_perhour']    * 24 * 7  * 0.01 * $parse['production_level'] + $parse['tachyon_basic_income']   * 24 * 7  );
	$parse['monthly_tachyon']      = floor($CurrentPlanet['tachyon_perhour']    * 24 * 30 * 0.01 * $parse['production_level'] + $parse['tachyon_basic_income']   * 24 * 30 );
	...
	$parse['daily_tachyon']        = colorNumber(pretty_number($parse['daily_tachyon']));
	$parse['weekly_tachyon']       = colorNumber(pretty_number($parse['weekly_tachyon']));
	$parse['monthly_tachyon']      = colorNumber(pretty_number($parse['monthly_tachyon']));
	...
	$parse['tachyon_storage']       = floor($CurrentPlanet['tachyon']   / $CurrentPlanet['tachyon_max']   * 100) . $lang['o/o'];
	...
	$parse['tachyon_storage_bar']   = floor(($CurrentPlanet['tachyon']   / $CurrentPlanet['tachyon_max']   * 100) * 2.5);
	...
	if ($parse['tachyon_storage_bar'] > (100 * 2.5)) {
		$parse['tachyon_storage_bar'] = 250;
		$parse['tachyon_storage_barcolor'] = '#C00000';
	} elseif ($parse['tachyon_storage_bar'] > (80 * 2.5)) {
		$parse['tachyon_storage_barcolor'] = '#C0C000';
	} else {
		$parse['tachyon_storage_barcolor'] = '#00C000';
	}
