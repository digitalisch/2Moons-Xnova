...
function ShowTopNavigationBar ( $CurrentUser, $CurrentPlanet ) {
	...
	if ($CurrentUser) {
		...
		$parse['show_umod_notice'] 	= $CurrentUser['urlaubs_modus'] ? '<table width="100%" style="border: 1px solid red; text-align:center;"><tr><td>'.$lang['VACATION_UNTIL'].date('d.m.Y h:i:s',$CurrentUser['urlaubs_modus_time']).'</td></tr></table>' : '';
		...
		$parse['energy_total'] = colorNumber(pretty_number(intval($CurrentPlanet['energy_max'] + $CurrentPlanet["energy_used"] + $parse['energy_basic_income']))).'/'.pretty_number(intval($CurrentPlanet['energy_max']));
    
		// $parse['energy_total'] = colorNumber(pretty_number(intval($parse['energy_total'])));
		...
		if ($CurrentPlanet['energy_max'] == 0 && abs($CurrentPlanet['energy_used']) >= 0) {
			...
			$plevel = ($CurrentPlanet['energy_max'] / abs($CurrentPlanet['energy_used']) * 100);
		} elseif ($CurrentPlanet['energy_max'] == 0 && abs($CurrentPlanet['energy_used']) > $CurrentPlanet['energy_max']) {
			$plevel = 0;
		} else {
			$plevel = 100;
		...
		$deut_used = abs($CurrentPlanet['deuterium_used']);
		...
		$parse['deuterium_perhour'] .= $CurrentPlanet['deuterium_perhour'] * 0.01 * $plevel + ($game_config['deuterium_basic_income'] * $game_config['resource_multiplier'] + $CurrentPlanet['deuterium_used']);
		$parse['metalh'] .= floor($CurrentPlanet["metal"]);
		$parse['crystalh'] .= floor($CurrentPlanet["crystal"]);
		$parse['deuteriumh'] .= floor($CurrentPlanet["deuterium"]);
		...
		$parse['deuts_used']       .= $CurrentPlanet['deuterium_used'] / 3600 ;
