...
function FleetBuildingPage ( &$CurrentPlanet, $CurrentUser ) {
	...
	if (! empty($_POST['fmenge']) AND is_array($_POST['fmenge'])) {
	...
	foreach($lang['tech'] as $Element => $ElementName) {
		if ($Element > 201 && $Element <= 399) {
			if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element)) {
				...
				$PageTable .= "<br><br>".$lang['buyable_ships']." ".$baubar;
				$PageTable .= "</td>";
				...
				if ($CanBuildOne) {
					$TabIndex++;
				#################
				$PageTable .= sprintf('<input type="text" name="fmenge[%d]" alt="%s" size="5" maxlength="8" value="0" tabindex="%d">',
							$Element,
							$lang['tech'][$Element],
							$TabIndex);
