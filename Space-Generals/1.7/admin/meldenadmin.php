...
if ($user['authlevel'] >= 1) {
	...
	if($_GET['mode'] == 'read'){
		if($_GET['del'] == 'ja'){
			$Melden = doquery("UPDATE {{table}} SET `message_melden` = '2' WHERE `message_id` = '{$_GET['ms']}'", 'messages');
			message($lang['meldenadmin_workedon'].'<br><a href="meldenadmin.php">'.$lang['meldenadmin_back'].'</a>',$lang['meldenadmin_success']);
		}else{
			$query	   = doquery("SELECT * FROM {{table}}messages WHERE `message_id` = '{$_GET['ms']}';", '', true);
			if($query['message_class'] == 1){$class = $lang['meldenadmin_unimportant'];
			}elseif($query['message_class'] == 2){$class = $lang['meldenadmin_normal'];
			}elseif($query['message_class'] == 3){$class = "<blink><font color=\"red\">".$lang['meldenadmin_important']."</blink></font>";
			}else{$class = $lang['meldenadmin_normal'];}
			...
			<input name=\"ms\" type=\"hidden\" value=\"{$_GET['ms']}\" />
					...
					<input name=\"\" type=\"submit\" value=\"".$lang['meldenadmin_button_done']."\" /></form></th>";
			$page .= "</tr><tr>";
		...
		$page .= "<tr>";
		$page .= "<td colspan=\"5\" class=\"c\"><a href=\"meldenadmin.php\">".$lang['meldenadmin_new_messages']."</a></td>";
		$page .= "</tr>";
		$page .= "</table>\n";
		$page .= "</center>";
	}else{
		...
		$page .= "<tr>";
		$page .= "<td colspan=\"5\" class=\"c\"><a href=\"meldenadmin.php?mode=alt\">".$lang['meldenadmin_done_messages']."</a></td>";
		$page .= "</tr>";
		$page .= "</table>\n";
		$page .= "</center>";
	}
