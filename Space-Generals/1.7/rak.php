...
if (file_exists($rocketnova_root_path . "includes/raketenangriff.php")) {
	include($rocketnova_root_path . "includes/raketenangriff.php");
	...
	while ($selected_row = mysql_fetch_array($iraks)) {
		if ($selected_row['zeit'] != '' && $selected_row['galaxy'] != '' && $selected_row['system'] != '' && $selected_row['planet'] != '' && is_numeric($selected_row['owner']) && is_numeric($selected_row['zielid']) && is_numeric($selected_row['anzahl']) && !empty($selected_row['anzahl'])) {
			...
			if (mysql_num_rows($planetrow) != 1 OR mysql_num_rows($TargetArmour) != 1) {
				...
				$message_vorlage  = $lang['RAK_ATTACKED_STRING1'].' ' . $name .  ' <a href="galaxy.php?mode=3&galaxy=' . $selected_row['galaxy_angreifer'] . '&system=' . $selected_row['system_angreifer'] . '&planet=' . $selected_row['planet_angreifer'] . '">[' . $selected_row['galaxy_angreifer'] . ':' . $selected_row['system_angreifer'] . ':' . $selected_row['planet_angreifer'] . ']</a>';
				$message_vorlage .= ' '.$lang['RAK_ATTACKED_STRING2'].' ' . $name_deffer . ' <a href="galaxy.php?mode=3&galaxy=' . $selected_row['galaxy'] . '&system=' . $selected_row['system'] . '&planet=' . $selected_row['planet'] . '">[' . $selected_row['galaxy'] . ':' . $selected_row['system'] . ':' . $selected_row['planet'] . ']</a> '.$lang['RAK_ATTACKED_STRING3'].'.<br><br>';

				if (empty($message)){
					$message = $lang['RAK_NOTHING_DESTROYED'];
				}
