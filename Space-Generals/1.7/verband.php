...
if (mysql_num_rows($query) != 1) {
	message($lang['VERBAND_ERROR_NO_FLEET'], $lang['VERBAND_ERROR_TITLE']);
}

$daten = mysql_fetch_array($query);

if ($daten['fleet_start_time'] <= time() || $daten['fleet_end_time'] < time() || $daten['fleet_mess'] == 1) {
	message($lang['VERBAND_ERROR_BACK'], $lang['VERBAND_ERROR_TITLE']);
}
...
if (!isset($_POST['send'])) {
	...
	if (mysql_num_rows($aks) != 1) {
		message($lang['VERBAND_ERROR_NOT_FOUND'], $lang['VERBAND_ERROR_TITLE']);
	}
	$aks = mysql_num_rows($aks);
}

$missiontype = array(
	1 => $lang['VERBAND_ATTACK'],
	2 => $lang['VERBAND_DESTROY'],
	3 => $lang['VERBAND_TRANSPORT'],
	4 => $lang['VERBAND_STATIO'],
	5 => $lang['VERBAND_HOLD'],
	6 => $lang['VERBAND_ESPIONAGE'],
	7 => $lang['VERBAND_COLONISE'],
	8 => $lang['VERBAND_RECYCLE'],
	9 => $lang['VERBAND_COLONISE']
	);
...
<tr height="20">
<td colspan="9" class="c">' . str_replace('##max##', $ile, $lang['VERBAND_FLEET']) . '</td>
</tr>
<tr height="20">
<th>'.$lang['VERBAND_ID'].'</th>
<th>'.$lang['VERBAND_MISSION'].'</th>
<th>'.$lang['VERBAND_NAME'].'</th>
<th>'.$lang['VERBAND_FROM'].'</th>
<th>'.$lang['VERBAND_TO'].'</th>
<th>'.$lang['VERBAND_MISSION'].'</th>
<th>'.$lang['VERBAND_TO_RETOUR'].'</th>
<th>'.$lang['VERBAND_TIME_BACK'].'</th>
<th>'.$lang['VERBAND_MISSION'].'</th>
</tr>';
/*
...
while ($f = mysql_fetch_array($fq)) {
	...
	if (($f['fleet_start_time'] + 1) == $f['fleet_end_time'])
		$page .= " <a title=\"".$lang['VERBAND_BACK']."\">(".$lang['VERBAND_BACK_SC'].")</a>";
	$page .= "</th><th><a title=\"";
	/*
	...
	*/
	...
	if ($f['fleet_mess'] == 0) {
		$page .= "     <form action=\"fleetback.php\" method=\"post\">
			...
			<input value=\"".$lang['VERBAND_CALL_BACK']."\" type=\"submit\">
			</form></th>";
	} else $page .= "&nbsp;</th>";
...
if ($ile == $maxfleet_count) {
	$maxflot = '<tr height="20"><th colspan="9"><font color="red">'.$lang['VERBAND_ERROR_NO_SLOT'].'</font></th></tr>';
}
...
if (!$planetrow) {
	message($lang['VERBAND_ERROR_TITLE'], $lang['VERBAND_ERROR_TITLE']);
} //uno nunca sabe xD
