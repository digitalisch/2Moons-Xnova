...
if ($_POST['mission'] != 3 and $protectiontime > 0)
{
$EigeneAZ = $user['angriffszone'];
$GegnerAZ = $HeDBRec['angriffszone'];

if(($EigeneAZ + 2) < $GegnerAZ)
{
$Schutz = message("<font color=\"red\">".$lang['FLOTEN3_PLAYER_TOO_STRONG'],$lang['FLOTEN3_NO_ATTACK_TITLE']);
}
elseif (($EigeneAZ - 2) > $GegnerAZ)
{
$Schutz = message("<font color=\"green\">".$lang['FLOTEN3_PLAYER_TOO_WEAK'],$lang['FLOTEN3_NO_ATTACK_TITLE']);
}
}
######## Angriffszonenkontrolle ##########
...
if (($user[$resource[108]] + ($user['rpg_commandant'] * 3) + 1) <= $ActualFleets) {
	message($lang['FLOTEN3_NO_SLOT_AVAILABLE'], $lang['FLOTEN3_ERROR_TITLE'], "fleet." . $phpEx, 1);
}
