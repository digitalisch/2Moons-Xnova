...
if(($_GET['ticket']) == 0){
	...
	while($ticket = mysql_fetch_array($query))
	{
		...
		if($ticket['status']==1)
		{
			$status = "<font color=green>".$lang['status_open']."</font>";
		}
		if($ticket['status']==0)
		{
			$status = "<font color=red>".$lang['status_closed']."</font>";
		}
		if($ticket['status']==2)
		{
			$status = "<font color=yellow>".$lang['status_admin']."</font>";
		}
		if($ticket['status']==3)
		{
			$status = "<font color=green>".$lang['status_player']."</font>";
		}
		// Status-anzeige ende
		$parse['tickets'] .= "<tr>"
	...
	while($ticket2 = mysql_fetch_array($query2))
	{
		...
		if($ticket2['status']>=1){
			$parse['eintrag'] ='
				...
				<center><input type="submit" value="'.$lang['SUPPORT_BUTTON_SUBMIT'].'"></center>';
		}
		if($ticket2['status']==1)
		{
			$status = "<font color=green>".$lang['status_open']."</font>";
		...
		{
			$status = "<font color=red>".$lang['status_closed']."</font>";
			$parse['answer_new'] = $lang['SUPPORT_TICKET_CLOSED'];
		}
		if($ticket2['status']==2)
		{
			$status = "<font color=yellow>".$lang['status_admin']."</font>";
