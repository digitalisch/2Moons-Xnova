...
if ($_POST && $mode == "change") { // Array ( [db_character]
	...
	if ($_POST["db_character"] <> $user["username"] AND $_POST["db_character"] <> '') {
		...
		if ($user_exist > 0) {
			$dpath = (!$user["dpath"]) ? DEFAULT_SKINPATH : $user["dpath"];
			message($lang['OPTIONS_ERROR_USERNAME'],$lang['OPTIONS_ERROR_TITLE'], 'options.php');
		} else {
	...
	if ($_POST["db_email"] <> $user["email"] AND $_POST["db_email"] <> '') {
		...
		if ($email_exist > 0) {
			$dpath = (!$user["dpath"]) ? DEFAULT_SKINPATH : $user["dpath"];
			message($lang['OPTIONS_ERROR_EMAIL'],$lang['OPTIONS_ERROR_TITLE'], 'options.php');
		} else {
	...
	$parse['opt_gala_ansicht_dat']  = "<option value =\"0\"".    (($user['galaxyansicht'] == 0) ? " selected": "") .">".$lang['OPTIONS_GALAXY_DEFAULT']."</option>";
	$parse['opt_gala_ansicht_dat'] .= "<option value =\"1\"".    (($user['galaxyansicht'] == 1) ? " selected": "") .">".$lang['OPTIONS_GALAXY_LIST']."</option>";
	$parse['opt_gala_ansicht_dat'] .= "<option value =\"2\"".    (($user['galaxyansicht'] == 2) ? " selected": "") .">".$lang['OPTIONS_GALAXY_2D']."</option>";
	$parse['opt_gala_ansicht_dat'] .= "<option value =\"3\"".    (($user['galaxyansicht'] == 3) ? " selected": "") .">".$lang['OPTIONS_GALAXY_3D']."</option>";
