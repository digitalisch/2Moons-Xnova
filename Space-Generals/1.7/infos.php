...
function ShowProductionTable ($CurrentUser, $CurrentPlanet, $BuildID, $Template) {
	...
	$Prod[1]          = ((eval($ProdGrid[$BuildID]['formule']['metal'])     * $game_config['resource_multiplier']) * (1 + ($CurrentUser['rpg_geologue']  * 0.05)));
	$Prod[2]          = ((eval($ProdGrid[$BuildID]['formule']['crystal'])   * $game_config['resource_multiplier']) * (1 + ($CurrentUser['rpg_geologue']  * 0.05)));
	$Prod[3]          = ((eval($ProdGrid[$BuildID]['formule']['deuterium']) * $game_config['resource_multiplier']));
	
	$Prod[4] 		      = eval($ProdGrid[$BuildID]['formule']['energy']);
	
	    if ($Prod[4] < 0) {
      $Prod[4] = ceil($Prod[4]);
      } else {
      $Prod[4] = floor($Prod[4] * (1 + ($CurrentUser['rpg_ingenieur'] * 0.05 + ($CurrentUser['energy_tech'] * 0.01))));
      }
      if ($Prod[3] < 0) {
      $Prod[3] = ceil($Prod[3]);
      } else {
      $Prod[3] = floor($Prod[3] * (1 + ($CurrentUser['rpg_geologue']  * 0.05)));
      }
      ...
      $Prod[3] = floor($Prod[3] * (1 + ($CurrentUser['rpg_geologue']  * 0.05)));
			...
			if ($ProdFirst > 0) {
				if ($BuildID != 12) {
					...
					$bloc['build_gain']      = "<font color=\"lime\">(".pretty_number($Prod[4] - $ProdFirst).")</font>";
				}
			} else {
				$bloc['build_gain']      = "";
			}
			if ($BuildID != 12) {
				...
				$bloc['build_need']      = colorNumber( pretty_number($Prod[4]));
				$bloc['build_need_diff'] = colorNumber( pretty_number($Prod[4] - $ActualNeed));
			} else {
				$bloc['build_prod']      = pretty_number($Prod[4]);
				$bloc['build_prod_diff'] = colorNumber( pretty_number($Prod[4] - $ActualProd));
				$bloc['build_need']      = colorNumber( pretty_number($Prod[3]));
				$bloc['build_need_diff'] = colorNumber( pretty_number($Prod[3] - $ActualNeed));
			}
			if ($ProdFirst == 0) {
				if ($BuildID != 12) {
