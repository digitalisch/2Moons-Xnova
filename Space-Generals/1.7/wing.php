...
if ($_GET['mode'] == 'ainfo') {
	...
	// query
	$lang['Alliance_information'] = $lang['ALLIANCE_TITLE'];
...
if ($user['wing_id'] == 0) { // Sin alianza
	if ($mode == 'make' && $user['wing_request'] == 0) { // Make alliance
		/*
		...
		if ($yes == 1 && $_POST) {
			...
			*/
			...
			doquery("UPDATE {{table}} SET
				`wing_id`='{$allyquery['id']}',
				`wing_name`='{$allyquery['ally_name']}',
				`wing_register_time`='" . time() . "'
	...
	if ($_POST['further'] == $lang['Send']) { // esta parte es igual que el buscador de search.php...
		doquery("UPDATE {{table}} SET `ally_request`='" . intval($allyid) . "', ally_request_text='".strip_tags($_POST['text'])."', ally_register_time='" . time() . "' WHERE `id`='" . $user['id'] . "'", "users");
...
elseif ($user['ally_id'] != 0 && $user['ally_request'] == 0) { // Con alianza
	...
	*/
	$ally = doquery("SELECT * FROM {{table}} WHERE id='{$user['wing_id']}'", "alliance", true);
	...
	if ($mode == 'admin' && $edit == 'ally') { // Administrar la alianza *pendiente urgente*
		...
		*/
		...
		$lang['Transfer_alliance'] = MessageForm("Abandonner / Transf&eacute;rer L'alliance", "", "?mode=admin&edit=transfer", $lang['Continue']);
	...
	if ($mode == 'admin' && $edit == 'members') { // Administrar a los miembros
		...
		*/
		if (isset($kick)) {
			...
			doquery("UPDATE {{table}} SET `ally_rank_id`='".strip_tags($_POST['newrang'])."' WHERE `id`='" . intval($id) . "'", 'users');
			}
		}
