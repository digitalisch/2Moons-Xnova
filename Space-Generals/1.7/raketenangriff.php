...
if ($planet['silo'] < 4) {
	...
	$error = 1;
}elseif ($anz > $iraks || $anz <= 0 ) {
	$error = 2;
}elseif ($ziel_id == $user['current_planet']) {
	$error = 3;
}elseif ((!is_numeric($pziel) && $pziel != "all") OR ($pziel < 0 && $pziel > 7 && $pziel != "all")) {
	$error = 1;
}
if ($error == 1) { 	message($lang['RAKETENANGRIFF_ERROR_IMPULS'], $lang['RAKETENANGRIFF_ERROR_TITLE']); 	exit(); }
if ($error == 2) { 	message($lang['RAKETENANGRIFF_ERROR_TOO_FEW'], $lang['RAKETENANGRIFF_ERROR_TITLE']); 	exit(); }
if ($error == 3) { 	message($lang['RAKETENANGRIFF_ERROR_SELF'], $lang['RAKETENANGRIFF_ERROR_TITLE']); 	exit(); }
