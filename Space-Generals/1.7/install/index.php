...
include($rocketnova_root_path . 'includes/databaseinfos.'.$phpEx);
include($rocketnova_root_path . 'includes/migrateinfo.'.$phpEx);
...
switch ($Mode) {
	...
	case 'ins':
		...
		elseif ($Page == 2) {
			...
			fwrite($dz, "<?php\n");
			fwrite($dz, "if(!defined(\"INSIDE\")){ die(\"hacking attempt\"); }\n");
			...
			doquery ( $QryTableUpdate     , 'update'     );
			doquery ( $QryTableImpres     , 'impressum'  );
			doquery ( $QryTableBannedip   , 'bannedip'   );
			doquery ( $QryTableTopkb      , 'topkb'      );
			doquery ( $QryTableDiplo      , 'diplo'      );
			doquery ( $QryTableSupp       , 'supp'       );
			doquery ( $QryTableSaveKB     , 'savek'      );
		...
		elseif ($Page == 4) {
			...
			include($rocketnova_root_path.'config.php');
...
display ($Displ, "Rocket Nova 1.0", false, '', true);
?>