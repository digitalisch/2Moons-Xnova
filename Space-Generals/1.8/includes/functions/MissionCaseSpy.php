 ...
 * @version 1
 * @copyright 2008
 * edit fix by Mori 2009 for Xnova (Team Rocket) 
 */

// ----------------------------------------------------------------------------------------------------------------
// Mission Case 6: -> Spionieren
//
function MissionCaseSpy ( $FleetRow ) {
	...
	if ($FleetRow['fleet_start_time'] <= time()) {
		...
		$CurrentSpyLvl       = $CurrentUser['spy_tech'] + ($CurrentUser['rpg_espion'] * 3);
		...
		$TargetPlanet     = doquery( $QryGetTargetPlanet, 'planets', true);
		$TargetUser       = doquery("SELECT * FROM {{table}} WHERE `id` = '".$TargetUserID."';", 'users', true);		
		$TargetSpyLvl        = $TargetUser['spy_tech']+($TargetUser['rpg_espion'] * 3);
