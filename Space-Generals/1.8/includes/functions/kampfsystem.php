	...
	*/
	...
	foreach ($angreifer_schiffe as $angr_id => $array) {
		...
		$kb .= '<th>
				...
				<br>
				'.$lang['kb_waffen'].': '.($angreifer_techniken[$angr_id]['waffen'] * 10).'% '.$lang['kb_schilde'].': '
				.($angreifer_techniken[$angr_id]['schilde'] * 10).'% '.$lang['kb_huelle'].': '.($angreifer_techniken[$angr_id]['panzerung'] * 10).'%';
	...
	<table border=1 width=100%>
		...
		if ($runde != 6) {
			...
			$kb .= '<br><center>'.$status1.'<br>'.$status2.'</center>';
			...
			if ($gesamt <= $cap) {
				...
				while ($weiterladen == true) {
                    ...
                    if ($u_max_met > 0) {
                        ...
                        if ($u_max_kris > 2 && $u_max_deut > 2) {
                            $lager = floor($restcap / 3);
                        } elseif ($u_max_kris > 2 || $u_max_deut > 2) {
                            $lager = floor($restcap / 2);
                        } elseif ($u_max_kris < 2 && $u_max_deut < 2) {
                            $lager = floor($restcap);
                        }
                        ...
                        if ($lager > $u_max_met) {
                            ...
                            $met += floor($lager);
                            $restcap -= floor($lager);
                            $u_max_met -= floor($lager);
                        }
                    } 
                    
                    if ($u_max_kris > 0) {
                        
                        if ($u_max_met > 2 && $u_max_deut > 2) {
                            $lager = floor($restcap / 3);
                        } elseif ($u_max_met > 2 || $u_max_deut > 2) {
                            $lager = floor($restcap / 2);
                        } elseif ($u_max_met < 2 && $u_max_deut < 2) {
                            $lager = floor($restcap);
                        }
	...
	$kb .= '	'.$beute_text.'<br><br><!-- Generation Time: '.$totalzeit_benchmark.'s - Script by UGamela Deutsch Member -= MoF =- Version: '.VERSION.' -->
	<br>'.$units_a.'<br>'.$units_v.'<br>'.$lang_tf.$mond.'';

	$return['ausgabe_angreifer'] = $kb.$footer.'</td></tr></table>';


	$return['ausgabe_verteidiger'] = $kb.$repairlist.'<br>'.$footer.'<br></td></tr></table>';
