...
<td>
      ...
      <table>
         <tr>
           <td align="center">
           <nobr><input style="width: 20px;" value="&lt;" onclick="eval('location=\''+pselector.options[pselector.selectedIndex-1].value+'\'');" type="button">
           <select id="pselector" size="1" onChange="eval('location=\''+this.options[this.selectedIndex].value+'\'');">
                       {planetlist}
          </select>
           <input style="width: 20px;" value="&gt;" onclick="eval('location=\''+pselector.options[pselector.selectedIndex+1].value+'\'');" type="button"></nobr>
...
function update() {
  ...
  val = ( val4 / 3600) * seconds + {deuteriumh};
