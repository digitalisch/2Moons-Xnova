...
if ($user['authlevel'] >= 1) {
	...
	if ($user['authlevel'] >=4) {
		...
		if ($fsock = @fsockopen('mmogbay.com', 80, $errno, $errstr, 10))
		{
			@fputs($fsock, "GET /releases/info.txt HTTP/1.1\r\n");
			@fputs($fsock, "HOST: spacegenerals.mmogbay.com\r\n");
			...
			{
				$Page .= "<br><p style='color:red'>".$lang['overview_new_version']."</p><br>";
			}else{
				$Page .= "<br><p style='color:lime'>".$lang['overview_new_version']."</p><br>";
			}
		}else{
			if ($errstr)
			{
				...
				$Page .= '<p>'.$lang['overview_socket_error'].'</p>';
			}
		}
	}
