...
{/if}																		
					...
					{elseif ($isBusy.research && ($ID == 6 || $ID == 31)) || ($isBusy.shipyard && ($ID == 15 || $ID == 21))}
						<span class="btn btn-success btn-flat" style="color:red">{$LNG.bd_working}</span>
					{else}
						{if $RoomIsOk}
							...
							{else}
							<span class="btn btn-success btn-flat" style="color:red">{if $Element.level == 0}{$LNG.bd_build}{else}{$LNG.bd_build_next_level}{$Element.levelToBuild + 1}{/if}</span>
							{/if}
						{else}
						<span class="btn btn-success btn-flat" style="color:red">{$LNG.bd_no_more_fields}</span>
						{/if}
					{/if}
																																																														</div>				
													</td>
