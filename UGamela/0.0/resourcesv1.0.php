...
if($_POST){
	if($_POST['last1']){$planetrow['metalmine_porcent'] = $_POST['last1'];}
	if($_POST['last2']){$planetrow['crystalmine_porcent'] = $_POST['last2'];}
	if($_POST['last3']){$planetrow['deuterium_porcent'] = $_POST['last3'];}
	if($_POST['last4']){$planetrow['solarplant_porcent'] = $_POST['last4'];}
	if($_POST['last12']){$planetrow['fusion_porcent'] = $_POST['last12'];}
	if($_POST['last212']){$planetrow['satelite_porcent'] = $_POST['last212'];}
...
if($planetrow[$resource[1]]>0){
	...
	*/
	$m = floor(30*$planetrow[$resource[1]]*pow((1.1),$planetrow[$resource[1]]));
	$mmax += $m;
...
if($planetrow[$resource[212]]>0){
	...
	$page .= r(212,0,0,0,0,0);
}

{//Los totales
$planetrow['metal_max'] /= 1000;
$planetrow['crystal_max'] /= 1000;
$planetrow['deuterium_max'] /= 1000;
...
$page .= <<<HTML
  ...
  <tr>
    ...
    <td class="k"><font color="#00ff00">{$planetrow[metal_max]}k</font></td>
    <td class="k"><font color="#00ff00">{$planetrow[crystal_max]}k</font></td>
    <td class="k"><font color="#00ff00">{$planetrow[deuterium_max]}k</font></td>
  ...
  <tr> 
    ...
    <td class="k"><font color="#00ff00">{$planetrow[energy_free]}</font></td>
      <tr>
  <tr>
   <td colspan="5">
   <center>
...
doquery("UPDATE {{table}} SET `metal_perhour`=$mmax, `crystal_perhour`=$cmax, `deuterium_perhour`=$dmax, `energy_free`=$efree, `energy_max`=$emax WHERE `id`={$planetrow[id]};",'planets');
