...
if($userrow['new_message'] == 1){
	$page .= "<tr><th colspan=4><a href=messages.php>{$lang['Have_new_message']}</a></th></tr>";
}elseif($userrow['new_message'] > 1){
	$page .= "<tr><th colspan=4><a href=messages.php>";
	$m = number_format($userrow['new_message'],0,',','.');
	$page .= str_replace('%m',$m,$lang['Have_new_messages']);
	$page .= "</a></th></tr>";
}

$page .= "<tr><th>{$lang['Server_time']}</th><th colspan=3>";
$page .= gmdate("D M d H:i:s",time()-3*60*60);
$page .= <<<HTML
	</th>
  </tr>
  <tr>
	...
	*/
	...
	while($p = mysql_fetch_array($planets_query)){
		...
		if($p["id"] != $userrow["current_planet"]){
			$page .= <<<HTML
		<th>{$p[name]}<br>
		  <a href="?cp={$p['id']}" title="{$p['name']}"><img src="{$dpath}planeten/small/s_{$p['image']}.jpg" height="50" width="50"></a><br>
		  <center>
HTML;
			/*
			...
			*/
			if($p['b_building_id'] != 0){
				if(check_building_progress($p)){$page .= $tech[$p['b_building_id']];}
				else{$page .= $lang['Free'];}
			}else{$page .= $lang['Free'];}
			
			$page .= "<center></center></center></th>";
			//Para ajustar a dos columnas
			if($c <= 1){$c++;}else{$page .= "</tr><tr>";$c = 1;	}
		}
	}
$page .= <<<HTML
		</tr>
  ...
  <tr>
      <th>
    {$lang['Diameter']}</th><th colspan="3">{$planetrow['diameter']} km (<a title="{$lang['Developed_fields']}">{$planetrow['field_current']} </a> / <a title="{$lang['max_eveloped_fields']}">{$planetrow['field_max']} </a> {$lang['fields']})</th>
  </tr>
  <tr>
	<th>{$lang['Temperature']}</th> <th colspan="3">{$lang['approx']} {$planetrow['temp_min']}°C {$lang['to']} {$planetrow['temp_max']}{$lang['°C']}</th> </tr>   <tr>
	<th>{$lang['Position']}</th><th colspan="3">{$galaxyrow['galaxy']}:{$galaxyrow['system']}:{$galaxyrow['planet']}</th></tr>   <tr>
	<th>{$lang['Points']}</th><th colspan="3">{$userrow['points']} ({$lang['Rank']} <a href="stat?start={$userrow['points']}">????</a> {$lang['of']} ????)</th>
  </tr>
</table>
 <br>
</center>
