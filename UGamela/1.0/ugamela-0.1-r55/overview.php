...
default:{//-----------------------------------------------------------------
	...
	if($user['new_message'] == 1){
		...
		$m = number_format($user['new_message'],0,',','.');
...
HTML;
	...
	$parse['galaxy_planet'] = $galaxyrow['planet'];
	$parse['galaxy_system'] = $galaxyrow['system'];
	$parse['user_points'] = $user['points'];
	$parse['u_user_rank'] = ($user['rank']==0)?1:$user['rank'];
	$parse['user_rank'] = $user['rank'];
	...
	$parse['time'] = gmdate("D M d H:i:s",time()-3*60*60);
