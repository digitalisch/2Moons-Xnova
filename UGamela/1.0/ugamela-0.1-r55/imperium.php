...
//{file_images}
$data['text'] = '<a href="overview.php?cp='.$p['id'].'&amp;re=0">
...
'.number_format(floor($p['metal']),0,",",".").'</a> / '.number_format(floor($p['metal_perhour']),0,",",".");
$parse['file_metal'] .=  parsetemplate($row2, $data);

//{file_crystal}
$data['text'] = '<a href="resources.php?cp='.$p['id'].'&amp;re=0&amp;planettype='.$p['planet_type'].'">
'.number_format(floor($p['crystal']),0,",",".").'</a> / '.number_format(floor($p['crystal_perhour']),0,",",".");
$parse['file_crystal'] .=  parsetemplate($row2, $data);

//{file_deuterium}
$data['text'] = '<a href="resources.php?cp='.$p['id'].'&amp;re=0&amp;planettype='.$p['planet_type'].'">
'.number_format(floor($p['deuterium']),0,",",".").'</a> / '.number_format(floor($p['deuterium_perhour']),0,",",".");
$parse['file_deuterium'] .=  parsetemplate($row2, $data);

//{file_energy}
$data['text'] = number_format(floor($p['energy_max']-$p['energy_used']),0,",",".").' / '.number_format(floor($p['energy_max']),0,",",".");
