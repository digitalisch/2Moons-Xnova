...
/*
  Un tutorial de instalacion.
  Se preguntaran datos como base de datos sql. Luego se crearan las tablas.
  
  etc...

  TO DO:
    revizar install_database()
  
*/

//Este es el skin...
$DEFAULT_SKINPATH = "http://80.237.203.201/download/use/brotstyle/";

define('BASE_DIR','../');

function page1(){ //una simple pagina de bienvenida
...
<table width=450>
  ...
  <tr>
	...
	<!--Bug Tracking: <a href="http://bugs.perberos.com.ar/">http://bugs.perberos.com.ar/</a><br>--><br><br>
	<b>International</b><br>
	Bugs et Problèmes, Idée et Réactions <a href="http://forum.cflo56.fr.nf/">http://forum.cflo56.fr.nf/</a><br><br>
	</td>
  </tr>
...
function page2(){ //Preguntamos los datos sql
	...
	if ($_POST) {
		...
		//si no existe la table, la creamos
...
function page3(){ //creamos las tablas y demas chiches
...
<table width=450>
  ...
  <tr>
	...
	<form action="" method="post">
		...
		<input type="submit" name="install" value="Crear" />
...
function display($dat,$title){ //un simple echo
	global $DEFAULT_SKINPATH;
...
function doquery($query, $table, $fetch = false){//query
  global $link;
  ...
  if(!$link){
	  $link = mysql_connect($dbsettings["server"], $dbsettings["user"], $dbsettings["pass"]) or error(mysql_error(),"SQL Error");
	  mysql_select_db($dbsettings["name"]) or error(mysql_error(),"SQL Error");
  }
  
	$sqlquery = mysql_query(str_replace("{{table}}", $dbsettings["prefix"] . $table, $query)) or error(mysql_error(),"SQL Error");
...
function make_configfile(){ //se crea el config.php
/*
...
*/
	...
	if (file_exists($configfile) ) {
		...
		is_writable($configfile) or die("<h1>Forbidden</h1>You don't have permission to write in on this server.");
	}
	//lets make the file :)
	$file = '<?php  //config.php :: Archivo de configuracion
// Este archivo es autogenerado | This files is autogenerated
if(!defined("INSIDE")){ die("attemp hacking");}
	$dbsettings = Array(
		"server"        => "'.$_POST['dbhost'].'", //MySQL server name. (Default: localhost)
		"user"          => "'.$_POST['dbuname'].'", // MySQL username.
		"pass"          => "'.$_POST['dbpass'].'", // GundamMySQL password.
		"name"          => "'.$_POST['dbname'].'", // MySQL database name.
		"prefix"        => "'.$_POST['db_prefix'].'", // Prefix for table names.
...
?>';
	...
	$nfile = @fopen($configfile,"w") or die("<h1>Forbidden</h1>You don't have permission to write in on this server.");
...
function install_database(){ //se crean las tablas


	define('INSIDE',TRUE);
	// para la lista de recursos
	include BASE_DIR."includes/vars.php";
	...
	VALUES ('allow_invetigate_while_lab_is_update', '0');";
		...
		doquery($query,"config");
		$query = "	INSERT INTO {{table}} ( `config_name` , `config_value` )
	VALUES ('copyright', '&copy; 2006 by Matsuri Soft. All rights reserved');";
...
";
  ...
  `ally_points` int(11) NOT NULL default '0',
