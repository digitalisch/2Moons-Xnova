...
if($_GET['mode'] == 'ainfo'){//Información de la alianza
	...
	$lang['ally_description'] = $ally_description;
...
if($mode == 'apply' && $user['ally_request'] == 0){//solicitudes
	...
	if($_POST['further'] == $lang['Send']){//esta parte es igual que el buscador de search.php...
		...
		doquery("UPDATE {{table}} SET `ally_request`='{$allyid}', ally_request_text='{$_POST['text']}', ally_register_time='".time()."' WHERE `id`='{$user['id']}'","users");
...
if($mode == 'memberslist'){//Lista de miembros.
	...
	*/
	...
	if($ally['ally_owner'] != $user['id']&&$ally_ranks[$user['ally_rank_id']-1][3]!=1){
	...
	while($u = mysql_fetch_array($listuser)){
		...
		}elseif(isset($ally_ranks[$u['ally_rank_id']])){
			$u["ally_range"] = $ally_ranks[$u['ally_rank_id']][0];
		}else{
		...
		$u['ally_register_time'] = date("Y-m-d h:m:s",$u['ally_register_time']);
	...
	//ahora una pequeña comprobacion, para arreglar la cantidad de usuarios
	...
	*/
	...
	elseif(!empty($_POST)&&$add!='name'){
		...
		*/
		...
		$ally['ally_rank'] = serialize($ally_ranks_new);
	...
	if(count($ally_ranks)==0 || $ally_ranks==''){//si no hay rangos
		...
		foreach($ally_ranks as $a => $b){
			...
			if($ally['ally_owner'] == $user['id']){
				...
				//Solo los que no son el dueño de la alianza
				$lang['id'] = $a;
				$lang['r0'] = "{$b[0]} {$b[1]}";
...
if($mode == 'admin' && $edit == 'ally'){//Administrar la alianza *pendiente urgente*
	...
	if($_POST['options']){
		...
		$ally['ally_request_notallow'] = $_POST['request_notallow'];
		doquery("UPDATE {{table}} SET
	...
	*/
	...
	if($t == 3){
		...
		//$page .= '<option value=0>no mostrar automátiicamente</option>';
	...
	*/
	if(isset($kick)){
		...
		if((isset($ally_ranks[$_POST['newrang']-1])||$_POST['newrang']==0)&&$q['id']!=$ally['ally_owner']){
			doquery("UPDATE {{table}} SET `ally_rank_id`='{$_POST['newrang']}' WHERE `id`='{$id}'",'users');
		}
	...
	while($u = mysql_fetch_array($listuser)){
		...
		$u["onlinetime"] = str_replace("%s",$days,$lang['Inactive_days']);
		//Nombre de rango
		if($ally['ally_owner'] == $u['id']){
		...
		*/
		...
		$u['ally_register_time'] = date("Y-m-d h:m:s",$u['ally_register_time']);
		$page_list .= parsetemplate($template, $u);
		//para mostrar un pequeño formulario
		if($rank==$u['id']){
...
if($mode == 'admin' && $edit == 'requests'){//Administrar solicitudes
	...
	if($_POST['action']==$lang['Ok']){
		...
		doquery("UPDATE {{table}} SET ally_request_text='',ally_request='0',ally_id='{$ally['id']}',new_message=new_message+1 WHERE id='{$show}'",'users');
	...
	while($r = mysql_fetch_array($query)){
		...
		$r['time'] = date("Y-m-d h:m:s",$r['ally_register_time']);
		$parse['list'] .= parsetemplate($row, $r);
		$i++;
	}
	if($parse['list']==''){
		$parse['list'] = '<tr><th colspan=2>No hay más solicitudes</th></tr>';
	}
	//Con $show
...
if($mode == 'admin' && $edit == 'name'){//renombrar alianza
	/*
...
if($mode == 'admin' && $edit == 'tag'){//renombrar etiqueta de la alianza
	/*
	  Tan simple como renombrar algo...
	*/
	//obtenemos el array de los rangos
...
{//Default *falta revisar...*
	...
	*/
	...
	if($ally['ally_owner'] == $user['id']||$ally_ranks[1][7]!=0){
	...
	if($request_count!=0){
	if($ally['ally_owner'] == $user['id']||$ally_ranks[1][2]!=0)
	...
	//pequenio form debajo del todo para salirse de la alianza
	if($ally['ally_owner'] != $user['id']){
