...
include($ugamela_root_path . 'common.' . $phpEx);
			...
			if((filesize($ugamela_root_path.'badquerys.txt') > 0) && ($user['authlevel'] >= 2)){
	echo "<a href=\"badquerys.txt\" target=\"_blank\" style=\"margin-left:120px;margin-top:60px\"><font color=\"red\">HACKING ALERT</font</a>";
}
...
//INICIO FIX Caracteres alfanum√©ricos en el nombre de planetas
               if (preg_match("/[^A-z0-9\Â\‰\ˆ\≈\ƒ\÷ _\-]/", $newname) == 1) {
               message('You have used an illegal character. Please try again.','Error');
               }
//FIN DEL FIX
				if ($newname != "") {
					// Deja on met jour la planete qu'on garde en memoire (pour le nom)
					$planetrow['name'] = $newname;
					// Ensuite, on enregistre dans la base de donn√©es
				...
				//INICIO FIX Abandonar colonias (funci√≥n recomendada)                
...
//FIN DEL FIX
		...
		default:
			...
			// --- Gestion Officiers -------------------------------------------------------------------------
			// Passage au niveau suivant, ajout du point de comp√©tence et affichage du passage au nouveau level
...
$query = doquery("UPDATE game_planets, game_users SET game_planets.arraki = game_users.arraki WHERE game_planets.id_owner = game_users.id");  
?>
