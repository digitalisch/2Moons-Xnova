<div id="theLayer" style="border:none;background-color:transparent;position:absolute;width:20px;left:150;top:50;visibility:visible">
<br>
<table width=519>
	<tr>
	  <td class=c colspan=2>{Apply_ally_overview} [{ally_tag}]</td>
	</tr>
	{request}
	<tr>
	  <th colspan=2>{There_is_hanging_request}</th>
	</tr>
	<tr>
	  <td class=c><center><a href="alliance.php?mode=admin&edit=requests&show=0&sort=1">{Candidate}</a></center></td>
	  <td class=c><center><a href="alliance.php?mode=admin&edit=requests&show=0&sort=0">{Request_date}</a></center></th>
	</tr>
	{list}
	<tr>
	  <td class=c colspan=2><a href="alliance.php">{Back}</a></td>
	</tr>
</table>
</div>