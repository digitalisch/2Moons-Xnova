...
function SpyTarget ( $TargetPlanet, $Mode, $TitleString ) {
	...
	if       ($Mode == 0) {
		...
		$String .= " <a href=\"?action=internalGalaxy&amp;mode=3&amp;galaxy=". $TargetPlanet["galaxy"] ."&amp;system=". $TargetPlanet["system"]. "\">";
		$String .= "[". $TargetPlanet["galaxy"] .":". $TargetPlanet["system"] .":". $TargetPlanet["planet"] ."]</a>";
		$String .= " vom  ". date("d-m-Y H:i:s", time()) ."</td>";
		$String .= "</tr><tr>";
	...
	if ($LookAtLoop == true) {
		$String  = "<table width=\"440\" cellspacing=\"1\"><tr><td class=\"c\" colspan=\"". ((2 * $game_config['spy_report_row']) + ($game_config['spy_report_row'] - 1))."\">". $TitleString ."</td></tr>";
		$Count       = 0;
		$CurrentLook = 0;
		while ($CurrentLook < $Loops) {
			...
			for ($Item = $ResFrom[$CurrentLook]; $Item <= $ResTo[$CurrentLook]; $Item++) {
				if ( $TargetPlanet[$resource[$Item]] > 0) {
					...
					if ($row < $game_config['spy_report_row'] - 1) {
						$String  .= "<td>&nbsp;</td>";
					}
					$Count   += $TargetPlanet[$resource[$Item]];
					$row++;
					if ($row == $game_config['spy_report_row']) {
						$String  .= "</tr>";
						$row      = 0;
					}
				}
