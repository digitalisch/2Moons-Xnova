	...
	if ($FleetRow['fleet_start_time'] <= time()) {
		if ($FleetRow['fleet_mess'] == 0) {
			if (!isset($CombatCaps[202]['sd'])) {
				message("<font color=\"red\">". $lang['sys_no_vars'] ."</font>", $lang['sys_error'], "?action=internalFleet", 2);
			}
            ...
            $raport  = "<a href=\"?action=ShowReport&amp;report=". $rid ." \" target=\"_blank\" >";
			$raport .= "<center>";
            if       ($FleetResult == "a") {
			...
			// Colorisation du résumé de rapport pour l'attaquant
			$raport2  = "<a href # OnClick=\"f( '?action=ShowReport&amp;report=". $rid ."', '');\" >";
			$raport2 .= "<center>";
			if       ($FleetResult == "a") {
