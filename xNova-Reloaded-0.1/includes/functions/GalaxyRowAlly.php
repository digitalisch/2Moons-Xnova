...
function GalaxyRowAlly ( $GalaxyRow, $GalaxyRowPlanet, $GalaxyRowUser, $Galaxy, $System, $Planet, $PlanetType ) {
	...
	if ($GalaxyRowUser['ally_id'] && $GalaxyRowUser['ally_id'] != 0) {
		...
		if ($allyquery) {
			...
			$Result .= "<table>";
			$Result .= "<tr>";
			$Result .= "<td><a href=?action=internalAlliance&amp;mode=ainfo&amp;a=". $allyquery['id'] .">".$lang['gl_ally_internal']."</a></td>";
			$Result .= "</tr><tr>";
			$Result .= "<td><a href=?action=internalStats&amp;start=101&amp;who=ally>".$lang['gl_stats']."</a></td>";
