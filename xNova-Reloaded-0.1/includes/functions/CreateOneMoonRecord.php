...
function CreateOneMoonRecord ( $Galaxy, $System, $Planet, $Owner, $MoonID, $MoonName, $Chance ) {
	...
	if ($MoonGalaxy['id_luna'] == 0) {
		if ($MoonPlanet['id'] != 0) {
			...
			$erstes_bild = 1;		//nummer des erstes Planetenbildes
			$letztes_bild = 10;		//Nummer des letzten Planetenbildes
			$MoonType         = 'moon_';
			$MoonNumber       = rand($erstes_bild,$letztes_bild);
			if ($MoonNumber < 10)
			{
				$MoonNumber = "0".$MoonNumber; //ne 0 davorsetzen, wenn Moonnumber < 10
			}
			$moonimage = "$MoonType"."$MoonNumber";
		

			$QryInsertMoonInLunas   = "INSERT INTO {{table}} SET ";
			$QryInsertMoonInLunas  .= "`name` = '". ( ($MoonName == '') ? $lang['sys_moon'] : $MoonName ) ."', ";
			$QryInsertMoonInLunas  .= "`image` = '$moonimage', ";
			...
			$GetPlanetPic  = "SELECT * FROM {{table}} ";
			$GetPlanetPic .= "WHERE ";
			$GetPlanetPic .= "`galaxy` = '". $Galaxy ."' AND ";
			$GetPlanetPic .= "`system` = '". $System ."' AND ";
			$GetPlanetPic .= "`planet` = '". $Planet ."' AND ";
			$GetPlanetPic .= "`planet_type` = '1' AND ";
			$GetPlanetPic .= "`id_owner` = '". $Owner ."';";
			$PlanetPic = doquery( $GetPlanetPic , 'planets', true);
			
			$NewPlanetPic = $PlanetPic['image']."_moon";
			
			$QryUpdatePlanetPic  = "UPDATE {{table}} SET ";
			$QryUpdatePlanetPic .= "`image` = '". $NewPlanetPic ."'";
			$QryUpdatePlanetPic .= "WHERE ";
			$QryUpdatePlanetPic .= "`planet_type` = '1' AND ";
			$QryUpdatePlanetPic .= "`galaxy` = '". $Galaxy ."' AND ";
			$QryUpdatePlanetPic .= "`system` = '". $System ."' AND ";
			$QryUpdatePlanetPic .= "`planet` = '". $Planet ."';";
			doquery( $QryUpdatePlanetPic , 'planets');
			...
			$QryInsertMoonInPlanet .= "`image` = '$moonimage', ";
