...
$Query = $DB->prepare("SELECT ally_name,ally_tag,ally_description,ally_web,ally_image FROM ".PREFIX."alliance id = :a");
$Query->BindParam("a", $_GET['a']);
$allyrow = $Query->fetch();
if(!$allyrow){ message("Alliance non trouv&eacute;e","Erreur");}

$Query2 = $DB->prepare("SELECT COUNT(DISTINCT(id)) FROM ".PREFIX."users WHERE ally_id = :a;");
$Query2->BindParam("a", $_GET['a']);
$count = $Query2->fetch();
