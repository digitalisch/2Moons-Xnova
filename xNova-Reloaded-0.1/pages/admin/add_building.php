...
define('LEFTMENU_NICHT_ANZEIGEN', true);
	...
	if ($user['authlevel'] >= 2) {
		includeLang('admin/buildings');

		$mode      = $_POST['mode'];

		$PageTpl   = gettemplate("admin/add_building");
		$parse     = $lang;
		...
		if ($mode == 'addit') {
			...
			AdminMessage ( $lang['add_message'].'<meta http-equiv="refresh" content="3; url=?action=administrativeMats">', $lang['add_message_title'] );
		}
