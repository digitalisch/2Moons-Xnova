...
$lang['PHP_SELF'] = '?action=internalNotes';
...
if($_POST["s"] == 1 || $_POST["s"] == 2){//Edicion y agregar notas
	...
	if($_POST["s"] ==1){
		...
		message($lang['NoteAdded'], $lang['Please_Wait'],'action=internalNotes',"3");
	}elseif($_POST["s"] == 2){
		/*
		...
		*/
		...
		message($lang['NoteUpdated'], $lang['Please_Wait'], 'action=internalNotes', "3");
	}
...
elseif($_POST){//Borrar
	...
	if($deleted){
		...
		message($mes,$lang['Please_Wait'],'action=internalNotes',"3");
	}else{header("Location: ?action=internalNotes");}
	...
	display(parsetemplate(gettemplate('notes_body'), $parse), $lang['Notes'], false);
	}
}
?>