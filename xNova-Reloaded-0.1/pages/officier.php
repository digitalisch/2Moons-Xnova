	...
	if (intval($_GET['mode']) == 2) {
		...
		for ( $Officier = 601; $Officier <= 615; $Officier++ ) {
			...
			if ( $Result != 0 ) {
				...
				if ($Result == 1) {
					$bloc['off_link'] = "<a href=\"?action=internalOfficiers&amp;mode=2&offi=".$Officier."\"><font color=\"#00ff00\">". $lang['link'][$Officier]."</font>";
				} else {
