...
if ( $s == 1 && isset( $_GET['bid'] ) ) {
	...
	if ( !$buddy ) {
		...
		message( $lang['Request_sent'], $lang['Buddy_request'], '?action=internalBuddy' );
	} else {
...
if ( $a == 2 && isset( $u ) ) {
	...
	if ( isset( $u ) && $u["id"] != $user["id"] ) {
		...
		<center>
			<form action=\"?action=internalBuddy\" method=post>
...
while ( $b = mysql_fetch_array( $buddyrow ) ) {
	...
	if ( $u["ally_id"] != 0 ) { // Alianza
		...
		// if($allyrow){
		$UserAlly .= "<a href=\"?action=internalAlliance&amp;mode=ainfo&amp;a=" . $u["id"] . "\">" . $u["ally_name"] . "</a>";
		// }
	}
	...
	<tr>
		<th width=20>" . $i . "</th>
		<th><a href=\"?action=internalMessages&amp;mode=write&amp;id=" . $u["id"] . "\">" . $u["username"] . "</a></th>
		<th>{$UserAlly}</th>
		<th><a href=\"?action=internalGalaxy&amp;mode=3&amp;galaxy=" . $u["galaxy"] . "&amp;system=" . $u["system"] . "\">" . $u["galaxy"] . ":" . $u["system"] . ":" . $u["planet"] . "</a></th>
		<th>{$LastOnline}</th>
		<th>{$UserCommand}</th>
	</tr>";
}
...
if ( $a == 1 ) {
	$page .= "
	<tr>
		<td colspan=6 class=c><a href=\"?action=internalBuddy\">{$lang['Back']}</a></td>
	</tr>";
}
