...
if(isset($_GET['mode'])){
	if($_POST){
		...
		message("Shortlink wurde gespeichert!","Shortlink","?action=internalFleetshortcut");
	}
	...
	<th><input type=\"reset\" value=\"Zur&uuml;cksetzen\"> <input type=\"submit\" value=\"Speichern\">";
	//Muestra un (L) si el destino pertenece a luna, lo mismo para escombros
	$page .= "</th></tr>";
	$page .= '<tr><td colspan=2 class=c><a href=\"?action=internalFleetshortcut\">Zur&uuml;ck</a></td></tr></tr></table></form>';
}
elseif(isset($_GET['a'])){
	if($_POST){
		//Armamos el array...
		...
		if($_POST["delete"]){
			...
			doquery("UPDATE {{table}} SET fleet_shortcut='{$user[fleet_shortcut]}' WHERE id={$user[id]}","users");
			message("Shortlink wurde gel&ouml;scht!","Gel&ouml;scht","?action=internalFleetshortcut");
		}
		else{
			$r = explode(",",$scarray[$a]);
			...
			$user['fleet_shortcut'] =  implode("\r\n",$scarray);
			doquery("UPDATE {{table}} SET fleet_shortcut='{$user[fleet_shortcut]}' WHERE id={$user[id]}","users");
			message("Shortlink wurde Editiert!","Editieren","?action=internalFleetshortcut");
		}
	}
	...
	}else{$page .= message("Shortlink wurde gespeichert!","Speichern","?action=internalFleetshortcut");}
	...
	<tr height="20">
	<td colspan="2" class="c">Shortlink (<a href="?mode=add">Hinzuf&uuml;gen</a>)</td>
	</tr>';
	...
	}else{$page .= "<th colspan=\"2\">Keine Shortlinks vorhanden</th>";}

	$page .= '<tr><td colspan=2 class=c><a href=\"?action=internalFleet\">Zur&uuml;ck</a></td></tr></tr></table>';
}
