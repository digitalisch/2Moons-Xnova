	...
	while ($selected_row = mysql_fetch_array($iraks)) {
		if ($selected_row['zeit'] != '' && $selected_row['galaxy'] != '' && $selected_row['system'] != '' && $selected_row['planet'] != '' && is_numeric($selected_row['owner']) && is_numeric($selected_row['zielid']) && is_numeric($selected_row['anzahl']) && !empty($selected_row['anzahl'])) {
			...
			if (mysql_num_rows($planetrow) != 1 OR mysql_num_rows($select_ziel) != 1) {
				...
				array(
						0 => "Raketenwerfer",
						1 => "Leichtes Lasergesch&uuml;tz",
						2 => "Schweres Lasergesch&uuml;tz",
						3 => "Gau&szlig;kanone",
						4 => "Ionengesch&uuml;tz",
						5 => "Plasmawerfer",
						6 => "Kleine Schildkuppel",
						7 => "Grand bouclier",
						8 => "Gro&szlig;e Schildkuppel",
						9 => "Interplanetarrakete",
						10 => "Abfangrakete"
						);
				...
				$message_vorlage  = 'Une attaque de missiles (' . $selected_row['anzahl'] . ') de ' . $name . ' <a href="?action=internalGalaxy&amp;mode=3&amp;galaxy=' . $selected_row['galaxy_angreifer'] . '&amp;system=' . $selected_row['system_angreifer'] . '&amp;planet=' . $selected_row['planet_angreifer'] . '">[' . $selected_row['galaxy_angreifer'] . ':' . $selected_row['system_angreifer'] . ':' . $selected_row['planet_angreifer'] . ']</a>';
				$message_vorlage .= 'de la planete ' . $name_deffer . ' <a href="?action=internalGalaxy&amp;mode=3&amp;galaxy=' . $selected_row['galaxy'] . '&amp;system=' . $selected_row['system'] . '&amp;planet=' . $selected_row['planet'] . '">[' . $selected_row['galaxy'] . ':' . $selected_row['system'] . ':' . $selected_row['planet'] . ']</a><br><br>';
