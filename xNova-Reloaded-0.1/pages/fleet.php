 ...
 */
	...
	$MaxFlyingFleets = sql_num_rows($Query);

    //Compteur de flotte en expéditions et nombre d'expédition maximum
    $MaxExpedition      = $user[$resource[124]];
    if ($MaxExpedition >= 1) {
		$Query = $DB->query("SELECT `fleet_owner` FROM `".PREFIX."fleets` WHERE `fleet_owner` = '".$user['id']."' AND `fleet_mission` = '15'");
	    $ExpeditionEnCours = sql_num_rows($Query);
	...
	$page .= "<br><center>";
	$page .= "<table width='700' border='0' cellpadding='0' cellspacing='1'>";
	$page .= "<tr height='20'>";
...
//	$page .= "<th>".$lang['fl_back_t']."</th>";
	...
	foreach($DB->query("SELECT * FROM `".PREFIX."fleets` WHERE `fleet_owner` = '".$user['id']."'") as $f) 
	{
		$i++;
		$page .= "<tr height=20>";
		// (01) Fleet ID
...
//		$page .= "<th><font color=\"lime\"><div id=\"time_0\"><font>". pretty_time(floor($f['fleet_end_time'] + 1 - time())) ."</font></th>";
		...
		$page .= "<th>";
		if ($f['fleet_mess'] == 0) {
				$page .= "<form action=\"?action=internalFleetback\" method=\"post\">";
			...
			if ($f[fleet_mission] == 1) {
				$page .= "<form action=\"?action=internalAKS\" method=\"post\">";
				$page .= "<input name=\"fleetid\" value=\"". $f['fleet_id'] ."\" type=\"hidden\">";
...
//		$page .= "<th>-</th>";
	...
	$page .= "<form action=\"?action=internalFleet1\" method=\"post\">";
	$page .= "<table width=\"700\" border=\"0\" cellpadding=\"0\" cellspacing=\"1\">";
	$page .= "<tr height=\"20\">";
