	...
	foreach($lang['tech'] as $Element => $ElementName) {
		if ($Element > 201 && $Element <= 399) {
			if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element)) {
				...
				if ($CanBuildOne) {
				//MOD Mostrar numero maximo de naves contruibles
					$TabIndex++;
					...
					$PageTable .= "<br><a href='javascript:;' onclick=\"document.getElementsByName('fmenge[".$Element."]')[0].value = '$baubar';\">(max. ".pretty_number($baubar).")</a>";
					//FIN MOD Mostrar numero maximo de naves contruibles
				}
				$PageTable .= "</th>";
