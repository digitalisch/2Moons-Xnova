...
//$listplanets = doquery("SELECT * FROM {{table}} WHERE `galaxy` = '".$Galaxy."' AND `system` = '".$System."' ;", 'galaxy');
//while ($g = mysql_fetch_array($listplanets)) {
	$Result = "";
...
//Paso a usar solo tabla planets para las lunas
				if ($GalaxyRow["id_luna"] != 0) {
					$GalaxyRowMoon   = doquery("SELECT * FROM {{table}} WHERE `id` = '". $GalaxyRow["id_luna"] ."';", 'planets', true);
