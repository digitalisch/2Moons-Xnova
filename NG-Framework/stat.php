...
include($ugamela_root_path . 'common.' . $phpEx);
	...
	$query = doquery("SELECT estado FROM {{table}} where modulo='Estadisticas'", 'modulos', true); //Sacamos el estado.
	if($query[0] == "0") { message("Modulo Inactivo.","Modulo Inactivo"); }
	//Fin comprobación
	
	//comprobación ESTADO modulo Topbk
	$query = doquery("SELECT estado FROM {{table}} where modulo='HallOFFame'", 'modulos', true); //Sacamos el estado.
	if($query[0] == 0) { $hall = 0; } else { $hall = 1; }
	//Fin comprobación
	
$Consulta = doquery("SELECT * FROM {{table}} WHERE config_name = 'confistats'", 'config', true); //Sacamos el config.
...
if($CStats[0] == 1) { $parse['who']  .= "<option value=\"2\"". (($who == 2) ? " SELECTED" : "") .">". $lang['stat_allys']  ."</option>"; }

$parse['type']  = "<option value=\"1\"". (($type == 1) ? " SELECTED" : "") .">". $lang['stat_main']     ."</option>"; 
if($CStats[1] == 1) { $parse['type'] .= "<option value=\"2\"". (($type == 2) ? " SELECTED" : "") .">". $lang['stat_fleet']    ."</option>"; }
if($CStats[2] == 1) { $parse['type'] .= "<option value=\"3\"". (($type == 3) ? " SELECTED" : "") .">". $lang['stat_research'] ."</option>"; }
if($CStats[3] == 1) { $parse['type'] .= "<option value=\"4\"". (($type == 4) ? " SELECTED" : "") .">". $lang['stat_building'] ."</option>"; }
if($CStats[4] == 1) { $parse['type'] .= "<option value=\"5\"". (($type == 5) ? " SELECTED" : "") .">". $lang['stat_defenses'] ."</option>"; }
...
if ($who == 2) {
	...
	while ($StatRow = mysql_fetch_assoc($query)) {
		...
		if ($UsrRow['id'] == $user['id']) {
		 if($hall == 1) {
			$parse['player_name']     = "<a href=\"playercard.php?playerid=".$UsrRow['id']."\"><font color=\"lime\">".$UsrRow['username']."</font></a>";
		} else {
		 ...
		 if($hall == 1) {
			$parse['player_name']     = "<a href=\"playercard.php?playerid=".$UsrRow['id']."\">".$UsrRow['username']."</a>";
			} else {
		...
		// INICIO FIX Enlace a alianzas en Estadisticas
		if ($UsrRow['ally_name'] == $user['ally_name']) {
			$parse['player_alliance'] = "<a href=\"alliance.php?mode=ainfo&a=".$UsrRow['ally_id']."\"><font color=\"#33CCFF\">".$UsrRow['ally_name']."</font></a>";
		} else {
