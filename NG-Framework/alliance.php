...
include($ugamela_root_path . 'common.' . $phpEx);
	...
	$query = doquery("SELECT estado FROM {{table}} where modulo='Alianzas'", 'modulos', true); //Sacamos el estado.
	...
	*/
		...
		*/
			...
			$page = MessageForm(str_replace('%s', $_POST['atag'], $lang['ally_maked']),
				...
				str_replace('%s', $_POST['atag'], $lang['ally_been_maked']) . "<br><br>", "", $lang['Ok']);
		} else {
...
elseif ($user['ally_id'] != 0 && $user['ally_request'] == 0) { // Con alianza
	...
	if ($mode == 'memberslist') { // Lista de miembros.
		...
		if ($sort2) {
			...
			$listuser = doquery("SELECT * FROM {{table}}users inner join {{table}}statpoints on {{table}}users.`id`={{table}}statpoints.`id_owner` WHERE ally_id='{$user['ally_id']}' AND STAT_type=1 $sort", '');
			
		} else {
			$listuser = doquery("SELECT * FROM {{table}}users inner join {{table}}statpoints on {{table}}users.`id`={{table}}statpoints.`id_owner` WHERE ally_id='{$user['ally_id']}' AND STAT_type=1 ", '');
		}
	...
	if ($mode == 'admin' && $edit == 'rights') { // Administrar leyes
		...
		if (count($ally_ranks) == 0 || $ally_ranks == '') { // si no hay rangos
			$list = "<th>{$lang['range_emptyfield']}</th>";
		} else { // Si hay rangos
	...
	*/
		...
		$lang['Disolve_alliance'] = MessageForm($lang['ally_dissolve'], "", "?mode=admin&edit=exit", $lang['Continue']);
	...
	*/
	...
	if ($mode == 'admin' && $edit == 'requests') { // Administrar solicitudes
       ...
       if ($_POST['action'] == $lang['Accept_cand']) {
			$_POST['text'] = mysql_escape_string(strip_tags($_POST['text']));
			...
			`message_text`='{$lang['Aplication_hello']}<b>" . $ally['ally_name'] . "</b>{$lang['Aplication_acepted']}" . $_POST['text'] . "'", "messages");

			header('Location:alliance.php?mode=admin&edit=requests');
			die();

       } elseif ($_POST['action'] == $lang['Reject_cand'] && $_POST['action'] != '') {
			$_POST['text'] = mysql_escape_string(strip_tags($_POST['text']));
			...
			`message_text`='{$lang['Aplication_hello']}<b>" . $ally['ally_name'] . "</b> {$lang['Aplication_rejected']}" . $_POST['text'] . "'", "messages");
		...
		if ($parse['list'] == '') {
			$parse['list'] = '<tr><th colspan=2> There are no applications before</th></tr>';
		}
		// Con $show
	...
	*/
	...
	
	{
	 // Default *falta revisar...*
		...
		if ($ally['ally_owner'] != $user['id']) {
			$lang['ally_owner'] .= MessageForm($lang['Desert'], "", "?mode=exit", $lang['Continue']);
		} else {
			$lang['ally_owner'] .= '';
		}
