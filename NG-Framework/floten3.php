...
//INICIO DEL FIX PARA EVITAR EL QUE ENTREN SIN ESTAR LOGEADOS
// blocking non-users
...
if(IsVacationMode($user)){
   message($lang['Vacation_mode'], $lang['Error'], "fleet.php", 1);
	...
	$mismali=doquery("SELECT * FROM {{table}} WHERE `id` = '". $TargetPlanet['id_owner'] ."'", 'users', true);

	$protection      = $game_config['noobprotection'];
	$protectiontime  = $game_config['noobprotectiontime'];
	$protectionmulti = $game_config['noobprotectionmulti']*5;
...
//En comentario para que permita la destrucción de luna
	...
	if ($fleetmission == 15) {
		...
		if ($_POST['planettype'] == "2") {
			...
			if ($_POST['ship208'] >= 1 && !$UsedPlanet) {
				...
				if($user['ally_id'] == $mismali['ally_id'] && $user['ally_id']!= '0'){
               message("&iexcl;No puedes espiar a miembros de la misma alianza!", "Alianza");
			   }else{
		...
		if (!$YourPlanet) {
				if($user['ally_id'] == $mismali['ally_id'] && $fleetmission==1 && $mismali['ally_id']!= '0'){
               	message("&iexcl;No puedes atacar a miembros de la misma alianza!", "Alianza");
				}else{
...
//INICIO FIX DESTRUIR LUNA
	if ( ($_POST['planettype'] == 3 || $_POST['planettype'] == 1) && ($_POST['ship214']) && ($fleet_group_mr > 0) && !$YourPlanet && $UsedPlanet) {
...
//fix no permitir destruir lunas debiles
...
//fix no permitir destruir lunas fuertes
...
//FIN FIX
	...
	if ((1+ $user[$resource[108]]) + ($user['rpg_commandant']*3) <= $ActualFleets) {
		message("Flota no disponible", "Error", "fleet." . $phpEx, 1);
	}
...
//	doquery("FLUSH TABLES", '');
	...
	display($page,$mensaje, $lang['fl_title']);
