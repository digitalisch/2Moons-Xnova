...
include($ugamela_root_path . 'admin/statfunctions.' . $phpEx);
	...
	/*
	//Empezamos con hall of fame
	$top = doquery ("SELECT * FROM {{table}} ORDER BY `gesamtunits` DESC LIMIT 100;", 'topkb'); //Consulta sacando las batallas
	doquery("TRUNCATE TABLE {{table}}", 'topkb'); //truncamos la tabla
	doquery("OPTIMIZE TABLE {{table}}", 'topkb'); //optimizamos la db
	while($data = mysql_fetch_assoc($top)) { //WHile, sacamos 1 pro 1 en este caso hacemos un insert por insert
    $QryInsertStats  = "INSERT INTO {{table}} SET ";
