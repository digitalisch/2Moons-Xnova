...
<!--  <tr>
  ...
  <tr>
	...
	<tr>
		<td class="row1">
			<p class="breadcrumbs"><a href="#" onClick="loadpage('./home.php','XNova Forum','bodyid');">Forumoverzicht</a> &#187; <a href="./gamehelp.php">Gameplay Help</a></p>
			<p class="datetime">Alle tijden zijn GMT + 1 uur [ Zomertijd ]</p>
		</td>
	</tr>
	</table>
	<br />	<div id="pageheader">
	        		<h2><a class="titles" href="./gamehelp.php">Gameplay Help</a></h2>
    		</div>
...
<div id="pagecontent">
		...
		<tr>
							<td align="left" valign="middle"><a href="./newtopic.php?forum=Gameplay Help" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('rollover','','img/button_topic_new2.gif',1)"><img src="img/button_topic_new.gif" alt="Nieuw Onderwerp" name="rollover" width="141" height="20" border="0" id="rollover" /></a><!--<a href="./newtopic.php"><img src="img/button_topic_new.gif" width="141" height="20" alt="Plaats een nieuw onderwerp" title="Plaats een nieuw onderwerp" /></a>--></td>
                            <?php 
							$res=mysql_query("select username, authlevel from beta_users where username ='".$_SESSION['suser']."'") or die(mysql_error());
							$row=mysql_fetch_assoc($res);
							if($row['authlevel'] > 0)
							{
								echo "<td align=\"left\" valign=\"middle\"><a href=\"./newmede.php?forum=Gameplay Help\" onmouseout=\"MM_swapImgRestore()\" onmouseover=\"MM_swapImage('rollover'		,'','img/button_topicmede_new2.gif',1)\"><img src=\"img/button_topicmede_new.gif\" alt=\"Nieuw Onderwerp\" name=\"rollover\" width=\"141\" height=\"20\" border=\"0\" id=\"rollover\" /></a></td>";
							}
							?>
								<td class="nav" valign="middle" nowrap="nowrap">&nbsp;Pagina <strong>1</strong> van <strong>1</strong><br /></td>
					<td class="gensmall" nowrap="nowrap">&nbsp;[ 
                    <?php 
					$res=mysql_query("select count(onderwerp) from beta_board_subject where forum = 'Gameplay Help'");
					$row=mysql_fetch_assoc($res);
		...
		<caption><div class="cap-left"><div class="cap-right">&nbsp;Gameplay Help&nbsp;</div></div></caption>		<tr>
            ...
            <?php 
		$res=mysql_query("select id,onderwerp,auteur,forum,type,topic,hits from beta_board_subject where topic='mededeling' and forum = 'Gameplay Help' order by id desc") or die(mysql_error());
		...
		<?php 
		$res=mysql_query("select id,onderwerp,auteur,forum,type,topic,hits from beta_board_subject where topic='onderwerp' and forum = 'Gameplay Help' order by id desc") or die(mysql_error());
		...
		<tr align="center">
        				...
        				<table width="100%" cellspacing="1">
		<tr>
							<td align="left" valign="middle"><a href="./newtopic.php?forum=Gameplay Help" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('rollover2','','img/button_topic_new2.gif',1)"><img src="img/button_topic_new.gif" alt="Nieuw Onderwerp" name="rollover2" width="141" height="20" border="0" id="rollover2" /></a></td>
                            <?php 
							$res=mysql_query("select username, authlevel from beta_users where username ='".$_SESSION['suser']."'") or die(mysql_error());
							$row=mysql_fetch_assoc($res);
							if($row['authlevel'] > 0)
