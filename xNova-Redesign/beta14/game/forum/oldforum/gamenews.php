						...
						if( "{$_GET['forum']}" != 'Handel') {
							if( "{$_GET['forum']}" != 'Diplomatie Uni 1') {
								if( "{$_GET['forum']}" == 'Diplomatie Uni 2') {
									echo '<a href="./dip2.php">'.$forum.'</a>';
								}
							}
							else {
								echo '<a href="./dip1.php">'.$forum.'</a>';
							}
						}
						else {
							echo '<a href="./handel.php">'.$forum.'</a>';
						}
					}
					else {
						echo '<a href="./gamehelp.php">'.$forum.'</a>';
					}
				}
				else {
					echo '<a href="./techassis.php">'.$forum.'</a>';
				}
			}
			else {
				echo '<a href="./gamenews.php">'.$forum.'</a>';
			} 			
			?></p>			<p class="datetime">Alle tijden zijn GMT + 1 uur [ Zomertijd ]</p>
		</td>
	</tr>
	</table>
	<br />	<div id="pageheader">
	        		<h2><a class="titles" href="./gamenews.php?<?php $forum = "{$_GET['forum']}"; echo $forum?>"><?php $forum = "{$_GET['forum']}"; echo $forum ?></a></h2>
    		</div>
...
<div id="pagecontent">
		...
		<tr>
							<td align="left" valign="middle"><a href="./newtopic.php?forum=<?php $forum = "{$_GET['forum']}"; echo $forum ?>" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('rollover','','img/button_topic_new2.gif',1)"><img src="img/button_topic_new.gif" alt="Nieuw Onderwerp" name="rollover" width="141" height="20" border="0" id="rollover" /></a></td>
							<?php 
							...
							if($row['authlevel'] > 0)
							{
								...
								$html .= "<a href=\"./newmede.php?forum=Game News\" onmouseout=\"MM_swapImgRestore()\" onmouseover=\"MM_swapImage('rollover3','','img/button_topicmede_new2.gif',1)\">";
		...
		<caption><div class="cap-left"><div class="cap-right">&nbsp;Game News&nbsp;</div></div></caption>		<tr>
		...
		<?php
		$forum = "{$_GET['forum']}";
		$res=mysql_query("select id,onderwerp,auteur,forum,type,topic,hits from beta_board_subject where topic='mededeling' and forum = '".$forum."' order by id desc") or die(mysql_error());
		$i=1;
		if(mysql_num_rows($res) > 0) {
		while ($row=mysql_fetch_assoc($res))
		{
		$res3=mysql_query("select time, reactie.auteur from beta_board_reactie, subject where subject.onderwerp = '".$row['onderwerp']."' and reactie.topic='mededeling' and reactie.onderwerp = subject.onderwerp order by reactie.id desc") or die(mysql_error());
		...
		$res5=mysql_query("select authlevel from beta_users, subject where beta_users.username = '".$row3['auteur']."'") or die(mysql_error());
		...
		<?php 
		$forum = "{$_GET['forum']}";
		$res=mysql_query("select id,onderwerp,auteur,forum,type,topic,hits from beta_board_subject where topic='onderwerp' and forum = '".$forum."' order by id desc") or die(mysql_error());
		$i=1;
		if(mysql_num_rows($res) > 0) {
		while ($row=mysql_fetch_assoc($res))
		{
		$res3=mysql_query("select time, reactie.auteur from beta_board_reactie, subject where subject.onderwerp = '".$row['onderwerp']."' and reactie.topic='onderwerp' and reactie.onderwerp = subject.onderwerp order by reactie.id desc") or die(mysql_error());
		$res2=mysql_query("select authlevel from beta_users, subject where beta_users.username = '".$row['auteur']."'") or die(mysql_error());
		$row2=mysql_fetch_assoc($res2);
		$row3=mysql_fetch_assoc($res3);
		$res5=mysql_query("select authlevel from beta_users, subject where beta_users.username = '".$row3['auteur']."'") or die(mysql_error());
		...
		$html .= "<a class=\"topictitle\" href=\"topic.php?onderwerp=".$row['onderwerp']."&forum=".$row['forum']."\" >".$row['onderwerp']."</a></td>";
		$html .= "<td class=\"row2\" width=\"130\" align=\"center\"><p class=\"topicauthor\"><a href=\"playercard.php?auteur=".$row['auteur']."&forum=".$row['forum']."\" style=\"color: ".$color."\" class=\"username-coloured\">".$row['auteur']."";
        $html .= "<td class=\"row1\" width=\"50\" align=\"center\"><p class=\"topicdetails\">".$row4['count(reactie)']."</p></td>";
		$html .= "<td class=\"row2\" width=\"50\" align=\"center\"><p class=\"topicdetails\">".$row['hits']."</p></td>";
		$html .= "<td class=\"row1\" width=\"140\" align=\"center\">";
		...
		<tr align="center">
        				...
        				<table width="100%" cellspacing="1">
		<tr>
							<td align="left" valign="middle"><a href="./newtopic.php?forum=<?php $forum = "{$_GET['forum']}"; echo $forum ?>" onmouseout="MM_swapImgRestore()" onmouseover="MM_swapImage('rollover2','','img/button_topic_new2.gif',1)"><img src="img/button_topic_new.gif" alt="Nieuw Onderwerp" name="rollover2" width="141" height="20" border="0" id="rollover2" /></a></td>
							<?php 
							$forum = "{$_GET['forum']}";
							$res=mysql_query("select username, authlevel from beta_users where username ='".$_SESSION['suser']."'") or die(mysql_error());
							$row=mysql_fetch_assoc($res);
							...
							{
								$html = "<td align=\"left\" valign=\"middle\">";
								$html .= "<a href=\"./newmede.php?forum=".$forum."\" onmouseout=\"MM_swapImgRestore()\" onmouseover=\"MM_swapImage('rollover4','','img/button_topicmede_new2.gif',1)\">";
