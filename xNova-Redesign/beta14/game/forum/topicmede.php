...
?>
	...
	<br />
<div id="pageheader">
	<h2><a class="titles" href="#" onClick="loadpage('./topicmede.php<?php $onderwerp = "{$_GET['onderwerp']}"; $forum = "{$_GET['forum']}"; echo "?onderwerp=".$onderwerp."&forum=".$forum; ?>','XNova Forum','bodyid');"><?php $onderwerp = "{$_GET['onderwerp']}"; echo $onderwerp; ?></a></h2>
...
<div id="pagecontent">
	...
	<tr>
		<?php 
		$res=mysql_query("select status from beta_board_subject where topic='mededeling' and onderwerp = '".$onderwerp."' and forum ='".$forum."'") or die(mysql_error());
		$row=mysql_fetch_assoc($res);
		if($row['status'] == 'open'){
		$antwoord = "<td align=\"left\" valign=\"middle\" nowrap=\"nowrap\">";
		$antwoord .= "<a href=\"#\" onClick=\"loadpage('./postingmede.php?onderwerp={$_GET['onderwerp']}&forum={$_GET['forum']}','XNova Forum','bodyid');\" onmouseout=\"MM_swapImgRestore()\" onmouseover=\"MM_swapImage('rollover','','img/button_topic_reply2.gif',1)\">";
		...
		?>
			...
			$forum = "{$_GET['forum']}";
			$onderwerp = "{$_GET['onderwerp']}" ;
			$res=mysql_query("SELECT count(reactie) from beta_board_reactie where topic='mededeling' and onderwerp='".$onderwerp."' and forum ='".$forum."'");
			$row=mysql_fetch_assoc($res);
...
/*
	<tr class="row1">
		...
		$forum = "{$_GET['forum']}";
		$onderwerp = "{$_GET['onderwerp']}" ;
		$res=mysql_query("select id,onderwerp,auteur,time,reactie,topic from beta_board_reactie where topic='mededeling' and onderwerp = '".$onderwerp."' and forum ='".$forum."' order by id") or die(mysql_error());
		$res3=mysql_query("select auteur,type from beta_board_subject where topic='mededeling' and onderwerp = '".$onderwerp."' and forum ='".$forum."' order by id") or die(mysql_error());
		$res4=mysql_query("select auteur,time,keuze1,keuze2,keuze3,keuze4,keuze5 from beta_board_pol where topic='mededeling' and onderwerp = '".$onderwerp."' and forum ='".$forum."' order by id_pol") or die(mysql_error());
		$res5=mysql_query("select onderwerp,auteur,forum,topic,keuze1,keuze2,keuze3,keuze4,keuze5 from beta_board_polstem where topic='mededeling' and onderwerp = '".$onderwerp."' and forum ='".$forum."' order by id") or die(mysql_error());
		$res6=mysql_query("select id_pol,auteur from beta_board_pol where topic='mededeling' and onderwerp = '".$onderwerp."' and forum ='".$forum."' order by id_pol") or die(mysql_error());
		...
		$pol .= "<table width=\"100%\" heigth=\"50%\" align=\"center\">";
		$pol .= "<tr>";
		$pol .= "<td>";
		...
		if($row8['authlevel'] > 1){
			$html .= "<a href=\"#\" onclick=\"loadpage('removereactie.php?onderwerp=".$row['onderwerp']."&forum=".$forum."&reactie=".$row['reactie']."&topic=mededeling','XNova Forum','bodyid');\"><img src=\"img/r.png\" width=\"16\" height=\"16\" alt=\"Remove\" /></a>";
		}else{
			$html .= "";
		}
		$html .= "&nbsp;";
		if($row8['authlevel'] > 1){
			$html .= "<a href=\"#\" onclick=\"loadpage('wijzig.php?onderwerp=".$row['onderwerp']."&forum=".$forum."&reactie=".$row['reactie']."&topic=mededeling&id=".$row['id']."&auteur=".$row['auteur']."','XNova Forum','bodyid');\"><img src=\"img/wijzig.png\" width=\"16\" height=\"16\" alt=\"Remove\" /></a>";
		}else{
			$html .= "";
		}
...
<tr align="center">
						...
						$onderwerp = "{$_GET['onderwerp']}";
						$forum = "{$_GET['forum']}";
						echo "<a href=\"#\" onclick=\"loadpage('./topicmede.php?forum=".$forum."&onderwerp=".$onderwerp."','XNova Forum','bodyid');\">Go to top</a>";
						?>
					    </td>
	</tr>
	</table>
	...
	<tr>
		...
		if($row['status'] == 'open'){
		$antwoord = "<td align=\"left\" valign=\"middle\" nowrap=\"nowrap\">";
		$antwoord .= "<a href=\"#\" onClick=\"loadpage('./postingmede.php?onderwerp={$_GET['onderwerp']}&forum={$_GET['forum']}','XNova Forum','bodyid');\" onmouseout=\"MM_swapImgRestore()\" onmouseover=\"MM_swapImage('rollover2','','img/button_topic_reply2.gif',1)\">";
