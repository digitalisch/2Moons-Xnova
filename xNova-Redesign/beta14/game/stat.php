...
include($xnova_root_path . 'common.' . $phpEx);
	...
	if ($who == 2) {
		...
		$MaxUsers = doquery ("SELECT COUNT(*) AS `count` FROM {{table}} WHERE `db_deaktjava` = '0' AND `bana` <> '1' ;", 'users', true);
		...
		$start = floor($range / 100 % 100) * 100;
		$query = doquery("SELECT * FROM `{{table}}statpoints` WHERE `stat_type` = '1' AND `stat_code` = '1' AND (SELECT `bana` FROM `{{table}}users` WHERE `{{table}}users`.`id` = `{{table}}statpoints`.`id_owner`) <> '1' ORDER BY `". $Order ."` DESC LIMIT ". $start .",100;", '');
		...
		while ($StatRow = mysql_fetch_assoc($query)) {
			...
			$UsrRow                   = doquery("SELECT `id`,`username`,`ally_name`,`bana` FROM {{table}} WHERE `id` = '". $StatRow['id_owner'] ."';", 'users',true);
...
// 1.1 - Only show unbanned players
?>
