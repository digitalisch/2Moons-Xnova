...
$countMax = doquery("SELECT * FROM {{table}}", "chat"); 
$countMax = mysql_num_rows($countMax);
$countMin = ($countMax - 26);

if($countMax > 26)
  $query = doquery("SELECT * FROM {{table}} ORDER BY messageid ASC LIMIT $countMin ,$countMax", "chat");
else
  $query = doquery("SELECT * FROM {{table}} ORDER BY messageid ASC", "chat"); 
//
