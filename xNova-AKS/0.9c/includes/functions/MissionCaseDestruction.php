...
function MissionCaseDestruction($FleetRow) {
   ...
   if ($FleetRow['fleet_start_time'] <= time()) {
      ...
      if ($FleetRow['fleet_mess'] == 0) {
		 ...
		 $TargetOwner = doquery("SELECT * FROM {{table}} WHERE `id` = '". $TargetUserID ."';", 'users', true);
         ...
         //Nous y voila! l attaquant a gagne, nous allons voir ses chances de detruire la lune
            $destructionl1 = 100000-sqrt($TargetPlanet['diameter']);
            $destructionl21 = $destructionl1*sqrt($CurrentSet['2140000']['count']);

            $destructionl2 = $destructionl21/1;//ici c est la sensibilite de la destruction 1 c est l equivalent d  a 12 on a environ 2% pour 1000 rip
         ...
         switch ($FleetResult) {
            ...
            case "a":
               ...
               //  Kierujemy floty na planetê
               doquery("UPDATE {{table}} SET `fleet_start_type` = '1' WHERE `fleet_start_galaxy` = '". $TargetMoon['galaxy'] ."' AND `fleet_start_system` = '". $TargetMoon['system'] ."' AND `fleet_start_planet` = '". $TargetMoon['planet'] ."' AND `fleet_start_type` = '3';", 'fleets');
               //Ustawiamy CurrentPlanet - usuniêcie blêd mysql (to poprawia³em w samochodzie bez plików)
               doquery("UPDATE {{table}} SET `current_planet` = '". $TargetOwner['planet'] ."' WHERE `id` = '". $TargetOwner['id'] ."';", 'users');
               break;
