...
function FleetBuildingPage ( &$CurrentPlanet, $CurrentUser ) {
	...
	if (isset($_POST['fmenge'])) {
		...
		foreach($_POST['fmenge'] as $Element => $Count) {
			// Construction d'Element recuper&eacute;s sur la page de Flotte ...
			// ATTENTION ! La file d'attente Flotte est Commune a celle des Defenses
			// Dans fmenge, on devrait trouver un tableau des elements constructibles et du nombre d'elements souhait&eacute;s
	...
	foreach($lang['tech'] as $Element => $ElementName) {
		if ($Element > 201 && $Element <= 399) {
			if (IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Element)) {
				...
				if ($CanBuildOne) {
					$TabIndex++;
					$PageTable .= "<input type=text name=fmenge[".$Element."] alt='".$lang['tech'][$Element]."' size=11 maxlength=12 value=0 tabindex=".$TabIndex.">";
