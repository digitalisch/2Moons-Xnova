 ...
 * @BBCodes hinzugefügt by Mwieners
 */
...
if ($_GET['mode'] == 'ainfo') {
	...
	$patterns[] = "#\[/s\]#Ssi";
	$replacements[] = '</strike>';
	$patterns[] = "#\[swf=([a-z0-9\#\ \[\]]+)\]#Ssi";
	$replacements[] = '<object data="\1" type="application/x-shockwave-flash"><param name="movie" value="p2.swf"</object>';
	...
	if ($mode == 'admin' && $edit == 'ally') { // Administrar la alianza *pendiente urgente*
		...
		if ($_POST['options']) {
			...
			if ($t == 3) {
				...
				$ally['ally_description'] = mysql_real_escape_string(strip_tags($_POST['text'], '<a><p><br><b><u><big><h2><i><font>'));
	...
	*/
	...
	if ($mode == 'admin' && $edit == 'requests') { // Administrar solicitudes
		...
		if ($_POST['action'] == "Akzeptieren") {
			...
			`message_text`='Hi!<br>L\'Alliance <b>" . $ally['ally_name'] . "</b> Ihre Bewerbung wurde Akzeptiert!<br>Charte:<br>" . $_POST['text'] . "'", "messages");
			...
			`message_text`='Hi!<br>L\'Alliance <b>" . $ally['ally_name'] . "</b> Ihre Bewerbung wurde Abgelehnt!<br>Begr&uuml;ndung/Text:<br>" . $_POST['text'] . "'", "messages");
