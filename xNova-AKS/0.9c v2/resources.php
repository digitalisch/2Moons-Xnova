...
function BuildRessourcePage ( $CurrentUser, $CurrentPlanet ) {
	...
	foreach($reslist['prod'] as $ProdID) {
		if ($CurrentPlanet[$resource[$ProdID]] > 0 && isset($ProdGrid[$ProdID])) {
			...
			$metal     = floor( eval ( $ProdGrid[$ProdID]['formule']['metal']     ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.05 ) )  * ( 1 + ( $CurrentUser['prod_op']  * 0.02 ) ));
			$crystal   = floor( eval ( $ProdGrid[$ProdID]['formule']['crystal']   ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.05 ) )  * ( 1 + ( $CurrentUser['prod_op']  * 0.02 ) ));
			$deuterium = floor( eval ( $ProdGrid[$ProdID]['formule']['deuterium'] ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.05 ) )  * ( 1 + ( $CurrentUser['prod_op']  * 0.02 ) ));
			    // Prise en compte du bonus de l'ing&eacute;nieur :
         if( $ProdID >= 4 ) { 
            $energy = floor( eval ( $ProdGrid[$ProdID]['formule']['energy']    ) * ( $game_config['resource_multiplier'] ));// * ( 1 + ( $CurrentUser['rpg_ingenieur'] * 0.05 ) ) * ( 1 + ( $CurrentUser['prod_op']  * 0.02 ) ) );
