
<br><br>
<h2>{adm_shipdel1}</h2>
<form action="del_ship.php" method="post">
<input type="hidden" name="mode" value="addit">
<table width="404">
<tbody>
<tr>
	<td class="c" colspan="7">{adm_shipdel2}</td>
</tr><tr>
	<th width="20"></th>
	<th width="130">{adm_am_plid}</th>
	<th width="300"><input name="id" type="text" value="0" size="3" /></th>
</tr><tr>
	<td class="c" colspan="6">Schiffe</td>
</tr><tr>
	<th>{nr}</th>
	<th>{ship_typ}</th>
	<th>{hinz}</th>
</tr><tr>
	<th>1</td>
	<th>{kt}</th>
	<th><input name="small_ship_cargo" type="text" value="0" /></th>
</tr><tr>
	<th>2</td>
	<th>{gt}</td>
	<th><input name="big_ship_cargo" type="text" value="0" /></th>
</tr><tr>
	<th>3</td>
	<th>{lj}</td>
	<th><input name="light_hunter" type="text" value="0" /></th>
</tr><tr>
	<th>4</td>
	<th>{sj}</td>
	<th><input name="heavy_hunter" type="text" value="0" /></th>
</tr><tr>
	<th>5</td>
	<th>{kz}</td>
	<th><input name="crusher" type="text" value="0" /></th>
</tr><tr>
	<th>6</td>
	<th>{ss}</td>
	<th><input name="battle_ship" type="text" value="0" /></th>
</tr><tr>
	<th>7</td>
	<th>{ks}</td>
	<th><input name="colonizer" type="text" value="0" /></th>
</tr><tr>
	<th>8</td>
	<th>{tf}</td>
	<th><input name="recycler" type="text" value="0" /></th>
</tr><tr>
	<th>9</td>
	<th>{sp}</td>
	<th><input name="spy_sonde" type="text" value="0" /></th>
</tr><tr>
	<th>10</td>
	<th>{bo}</td>
	<th><input name="bomber_ship" type="text" value="0" /></th>
</tr><tr>
	<th>11</td>
	<th>{so}</td>
	<th><input name="solar_satelit" type="text" value="0" /></th>
</tr><tr>
	<th>12</td>
	<th>{zt}</td>
	<th><input name="destructor" type="text" value="0" /></th>
</tr><tr>
	<th>13</td>
	<th>{rp}</td>
	<th><input name="dearth_star" type="text" value="0" /></th>
</tr><tr>
	<th>14</td>
	<th>{sk}</td>
	<th><input name="battleship" type="text" value="0" /></th>
</tr><tr>	
	<th>15</td>
	<th>{ln}</td>
	<th><input name="lune_noir" type="text" value="0" /></th>
</tr><tr>
	<th>16</td>
	<th>{ev}</td>
	<th><input name="ev_transporter" type="text" value="0" /></th>
</tr><tr>
	<th>17</td>
	<th>{sz}</td>
	<th><input name="star_crasher" type="text" value="0" /></th>
</tr><tr>	
	<th>18</td>
	<th>{gr}</td>
	<th><input name="giga_recykler" type="text" value="0" /></th>
</tr><tr>

	<th colspan="3"><input type="Submit" value="{adm_am_add}" /></th>
</tbody>
</tr>
</table>
</form>