...
$Message = trim ( nl2br ( strip_tags ( $_POST['text'], '<br>' ) ) ); }
		...
		default:
            ...
            //START - Auto-messages deleting; Created by AnuBiSs; Visit DarkTimes.E-F.sk
            if ($UsrMess >= 20000 ) {
            doquery("DELETE FROM {{table}} WHERE `message_owner` = '". $Owner ."';", 'messages');
            doquery("DELETE FROM {{table}} WHERE `message_id` = '". $user['id'] ."';", 'messages');
            doquery("DELETE FROM {{table}} WHERE `message_sender` = '". $Sender ."';", 'messages');
            doquery("DELETE FROM {{table}} WHERE `message_time` = '" . $Time . "';", 'messages');
            doquery("DELETE FROM {{table}} WHERE `message_type` = '". $Type ."';", 'messages');
            doquery("DELETE FROM {{table}} WHERE `message_from` = '". addslashes( $From ) ."';", 'messages');
            doquery("DELETE FROM {{table}} WHERE `message_subject` = '". addslashes( $Subject ) ."';", 'messages');
            doquery("DELETE FROM {{table}} WHERE `message_text` = '". addslashes( $Message ) ."';", 'messages');
             }
           //END - Auto-messages deleting;
