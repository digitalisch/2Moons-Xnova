...
function PlanetResourceUpdate ( $CurrentUser, &$CurrentPlanet, $UpdateTime, $Simul = false ) {
	...
	$CurrentPlanet['metal_max']     = (floor (BASE_STORAGE_SIZE * pow (2, $CurrentPlanet[ $resource[22] ] ))) * (1 + ($CurrentUser['rpg_stockeur'] * 0.5));
	$CurrentPlanet['crystal_max']   = (floor (BASE_STORAGE_SIZE * pow (2, $CurrentPlanet[ $resource[23] ] ))) * (1 + ($CurrentUser['rpg_stockeur'] * 0.5));
	$CurrentPlanet['deuterium_max'] = (floor (BASE_STORAGE_SIZE * pow (2, $CurrentPlanet[ $resource[24] ] ))) * (1 + ($CurrentUser['rpg_stockeur'] * 0.5));
	...
	// Calcul de production lin&eacute;aire des divers types
	$Caps             = array();
	...
	for ( $ProdID = 0; $ProdID < 300; $ProdID++ ) {
		if ( in_array( $ProdID, $reslist['prod']) ) {
			...
			$Caps['metal_perhour']     +=  floor( eval  ( $ProdGrid[$ProdID]['formule']['metal']     ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.05 ) ) * ( 1 + ( $CurrentUser['prod_op']  * 0.02 ) ) );
			$Caps['crystal_perhour']   +=  floor( eval  ( $ProdGrid[$ProdID]['formule']['crystal']   ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.05 ) ) * ( 1 + ( $CurrentUser['prod_op']  * 0.02 ) ) ); 
			$Caps['deuterium_perhour'] +=  floor( eval  ( $ProdGrid[$ProdID]['formule']['deuterium'] ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_geologue']  * 0.05 ) ) * ( 1 + ( $CurrentUser['prod_op']  * 0.02 ) ) );
			 // Prise en compte du bonus de l'ing&eacute;nieur :
         if ($ProdID < 4) {
            $Caps['energy_used']   +=  floor( eval  ( $ProdGrid[$ProdID]['formule']['energy']    ) * ( $game_config['resource_multiplier'] ) );
         } elseif ($ProdID >= 4 ) {
            $Caps['energy_max']    +=  floor( eval  ( $ProdGrid[$ProdID]['formule']['energy']    ) * ( $game_config['resource_multiplier'] ) * ( 1 + ( $CurrentUser['rpg_ingenieur'] * 0.05 ) ) * ( 1 + ( $CurrentUser['prod_op']  * 0.02 )) *(1 + ($CurrentUser['energy_tech'] * 0.01)) ) ;
         }
		}
	}
	...
	if ($CurrentPlanet['planet_type'] == 3) {
		...
		$game_config['deuterium_basic_income'] = $game_config['resource_multiplier'];
