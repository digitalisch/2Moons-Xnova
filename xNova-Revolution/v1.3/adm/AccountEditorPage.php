...
switch($_GET[page])
{
	case'resources':
		...
		if ($_POST){
		if(is_numeric($id) && is_numeric($metal) && is_numeric($cristal) && is_numeric($deut) && is_numeric($dark))
		{
			if ($_POST['add'])
			{
				...
				$QryUpdatePlanet .= "`darkmatter` = `darkmatter` + '". $dark ."' ";
	...
	case'ships':
		if($_POST)
		{
			...
			if(is_numeric($id) && is_numeric($light_hunter) && is_numeric($heavy_hunter) && is_numeric($small_ship_cargo) && is_numeric($big_ship_cargo) &&
				...
				is_numeric($battleship) && is_numeric($supernova) && is_numeric($corbeta))
			{
				if ($_POST['add'])
				{
					$QryUpdatePlanet  = "UPDATE {{table}} SET ";
					...
					$QryUpdatePlanet .= "`nave_de_asalto` = `nave_de_asalto` + '". $corbeta ."' ";
					$QryUpdatePlanet .= "WHERE ";
					$QryUpdatePlanet .= "`id` = '". $id ."' ";
					doquery( $QryUpdatePlanet, "planets");
				...
				if ($_POST['add'] || $_POST['delete'])
				{
					...
					$Log	.=	$lang['nave_de_asalto'].": ".$corbeta."\n";
					$Log	.=	$lang['log_to_planet'].$id."\n";
	...
	case'buildings':
		if ($_POST)
		{
			...
			$darkmatter_mine        = $_POST['darkmatter_mine'];
			...
			$darkmatter_store     	= $_POST['darkmatter_store'];
			...
			if(is_numeric($id) && is_numeric($metal_mine) && is_numeric($crystal_mine) && is_numeric($deuterium_sintetizer) && is_numeric($solar_plant) &&
				is_numeric($darkmatter_mine) && is_numeric($fusion_plant) && is_numeric($robot_factory) && is_numeric($nano_factory) && is_numeric($hangar) &&
				is_numeric($metal_store) && is_numeric($crystal_store) && is_numeric($deuterium_store) && is_numeric($darkmatter_store) && is_numeric($laboratory) &&
				...
				is_numeric($phalanx) && is_numeric($sprungtor))
			{
				...
				if ($_POST['add'])
				{
					...
					$QryUpdatePlanet .= "`darkmatter_mine` = `darkmatter_mine` + '". $darkmatter_mine ."', ";
					...
					$QryUpdatePlanet .= "`darkmatter_store` = `darkmatter_store` + '". $darkmatter_store ."', ";
				...
				if ($_POST['add'] || $_POST['delete'])
				{
					...
					$Log	.=	$lang['darkmatter_mine'].": ".$darkmatter_mine."\n";
					...
					$Log	.=	$lang['darkmatter_store'].": ".$darkmatter_store."\n";
