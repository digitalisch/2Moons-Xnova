...
function DisplayGameSettingsPage ( $CurrentUser )
{
	...
	if ($_POST['opt_save'] == "1")
	{
       ...
       if (isset($_POST['darkmatter_basic_income']) && is_numeric($_POST['darkmatter_basic_income'])) {
		$game_config['darkmatter_basic_income'] = $_POST['darkmatter_basic_income'];
		$Log	.=	$lang['log_sett_basic_e'].": ".$_POST['darkmatter_basic_income']."\n";
		}
        ...
        doquery("UPDATE {{table}} SET `config_value` = '". $game_config['darkmatter_basic_income'] ."' WHERE `config_name` = 'darkmatter_basic_income';", 'config');
	...
	else
	{
		...
		$parse['darkmatter_basic_income'] 	= $game_config['darkmatter_basic_income'];
