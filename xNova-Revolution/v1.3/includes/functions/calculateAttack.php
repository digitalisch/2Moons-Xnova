...
if(!defined('INSIDE')){ die(header("location:../../"));}
    ...
    function calculateAttack (&$attackers, &$defenders)
    {
        ...
        $resourcePointsAttacker = array('metal' => 0, 'crystal' => 0, 'darkmatter' => 0);
        ...
        foreach ($attackers as $fleetID => $attacker) {
            foreach ($attacker['detail'] as $element => $amount) {
                ...
                $resourcePointsAttacker['darkmatter'] += $pricelist[$element]['darkmatter'] * $amount ;
                
                $totalResourcePoints['attacker'] += $pricelist[$element]['metal'] * $amount ;
                $totalResourcePoints['attacker'] += $pricelist[$element]['crystal'] * $amount ;
                $totalResourcePoints['attacker'] += $pricelist[$element]['darkmatter'] * $amount ;
            }
        }

        $resourcePointsDefender = array('metal' => 0, 'crystal' => 0, 'darkmatter' => 0);
        foreach ($defenders as $fleetID => $defender) {
            foreach ($defender['def'] as $element => $amount) {                                //Line20
                if ($element < 300) {
                    ...
                    $resourcePointsDefender['darkmatter'] += $pricelist[$element]['darkmatter'] * $amount ;
                    
                    $totalResourcePoints['defender'] += $pricelist[$element]['metal'] * $amount ;
                    $totalResourcePoints['defender'] += $pricelist[$element]['crystal'] * $amount ;
                    $totalResourcePoints['defender'] += $pricelist[$element]['darkmatter'] * $amount ;
                } else {
        ...
        for ($round = 0, $rounds = array(); $round < $max_rounds; $round++) {
            ...
            foreach ($attackers as $fleetID => $attacker) {
                ...
                foreach ($attacker['detail'] as $element => $amount) {
                    ...
                    $thisAtt    = $amount * ($CombatCaps[$element]['attack']) * $attTech * (mt_rand(80, 120) / 100); //attaque
                    $thisDef    = $amount * ($CombatCaps[$element]['shield']) * $defTech ; //bouclier
                    $thisShield    = $amount * ($pricelist[$element]['metal'] + $pricelist[$element]['crystal'] + $pricelist[$element]['darkmatter']) / 10 * $shieldTech; //coque
                ...
                foreach($attacker['detail'] as $element => $amount) {
                    ...
                    if ($amount > 0) {
                        if ($attArray[$fleetID][$element]['def']/$amount < $defender_moc) {
                            ...
                            $ile_removePoints = floor($defender_moc / (($pricelist[$element]['metal'] + $pricelist[$element]['crystal'] + $pricelist[$element]['darkmatter'])  / 10));
                ...
                foreach($defender['def'] as $element => $amount) {
                    ...
                    if ($amount > 0) {
                        if ($defArray[$fleetID][$element]['def']/$amount < $attacker_moc) {
                            ...
                            $ile_removePoints = floor($attacker_moc / (($pricelist[$element]['metal'] + $pricelist[$element]['crystal'] + $pricelist[$element]['darkmatter']) / 10));
        ...
        $resourcePointsDefenderDefs = array('metal' => 0, 'crystal' => 0, 'darkmatter' => 0);
        foreach ($defenders as $fleetID => $defender) {
            foreach ($defender['def'] as $element => $amount) {
                if ($element < 300) {                                                        // flotte defenseur en CDR
                    ...
                    $resourcePointsDefenderDefs['darkmatter'] += $pricelist[$element]['darkmatter'] * ($lost - $giveback) ;
        ...
        $debAttCry = ($resourcePointsAttacker['darkmatter'] * ($game_config['Fleet_Cdr'] / 100));
        ...
        $debDefCry = ($resourcePointsDefender['darkmatter'] * ($game_config['Fleet_Cdr'] / 100)) + ($resourcePointsDefenderDefs['darkmatter'] * ($game_config['Defs_Cdr'] / 100));
