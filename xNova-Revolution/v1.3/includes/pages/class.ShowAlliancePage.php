				...
				else
				{
					...
					if ($_POST['enviar'] == $lang['al_applyform_send'])
					{
						doquery("UPDATE {{table}} SET `ally_request`='" . intval($allyid) . "', ally_request_text='" . mysql_escape_string(strip_tags($_POST['text'])) . "', ally_register_time='" . time() . "' WHERE `id`='" . $CurrentUser['id'] . "'", "users");
                                               //************* Inicio - Aviso al Fundador de nuevas solicitudes ********************
                        $QryAllyLeader   = "SELECT * FROM {{table}} ";
                        $QryAllyLeader  .= "WHERE ";
                        $QryAllyLeader  .= "`ally_id` = '".intval($allyid)."'";
                        $AllyLeader      = doquery($QryAllyLeader , 'users', true);
                        $IDAllyLeader    = $AllyLeader['id'];

                        $Time = time();
                        $remitente = $alianza['ally_name'];
                        $titulo = "Solicitud de Ingreso pendiente";
                        $mensaje1= "Hay una solicitud de ingreso del jugador <b><font color='lime'> ".$CurrentUser['username']."</font></b> pendiente de aprovacion";
                        SendSimpleMessage ( $IDAllyLeader, '', $time, 2, $remitente, $titulo, $mensaje1 );
                        //************* Fin - Aviso al Fundador de nuevas solicitudes ********************
