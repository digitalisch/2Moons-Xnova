...
private function DoFleetJump ($CurrentUser, $CurrentPlanet)
	...
	private function ShowProductionTable ($CurrentUser, $CurrentPlanet, $BuildID, $Template)
	{
        ...
        $Prod[5]          = (floor(eval($ProdGrid[$BuildID]['formule']['darkmatter']) * $game_config['resource_multiplier']) * (1 + ($CurrentUser['rpg_geologue']  * GEOLOGUE)));
	...
	public function ShowInfosPage ($CurrentUser, $CurrentPlanet, $BuildID)
	{
		...
		if ($DestroyTPL != '')
		{
			if ($CurrentPlanet[$resource[$BuildID]] > 0)
			{
                ...
                $parse['nfo_darkmatter']  = $lang['Darkmatter'];
                ...
                $parse['darkmatter']   = pretty_number ($NeededRessources['darkmatter']);
