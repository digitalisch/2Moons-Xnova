...
function ShowFleet3Page($CurrentUser, $CurrentPlanet)
{
	...
	$_POST['resource3'] = max(0, (int)trim($_POST['resource3']));
     OPEN FIX */
      $GeciciMetal      = $_POST['resource1'];
      $GeciciKristal    = $_POST['resource2'];
      $GeciciDeut       = $_POST['resource3'];
      $GeciciDark       = $_POST['resource4'];
      if(    (strlen($GeciciMetal)>49)    ||
          (strlen($GeciciKristal)>49)    ||
          (strlen($GeciciDeut)>49)    ||
          (strlen($GeciciDark)>49)
        ) {
          message ("Debe introducir solo n&uacute;meros!", "Error", 2);
      }


      for($kop=0;$kop<strlen($GeciciMetal);$kop++){
          if(!is_numeric($GeciciMetal[$kop]))
              message ("Solo n&uacute;meros",$GeciciMetal."Debe introducir solo n&uacute;meros!","Error", 2);
      }

      for($kop=0;$kop<strlen($GeciciKristal);$kop++){
          if(!is_numeric($GeciciKristal[$kop]))
              message ("Solo n&uacute;meros",$GeciciKristal."Debe introducir solo n&uacute;meros!","Error", 2 , true);
      }

      for($kop=0;$kop<strlen($GeciciDeut);$kop++){
          if(!is_numeric($GeciciDeut[$kop]))
              message ("Solo n&uacute;meros",$GeciciDeut."Debe introducir solo n&uacute;meros!", 2);
      }
      for($kop=0;$kop<strlen($GeciciDark);$kop++){
          if(!is_numeric($GeciciDark[$kop]))
              message ("Solo n&uacute;meros",$GeciciDark."Debe introducir solo n&uacute;meros!", 2);
      } // FIN FIX
	...
	$QryInsertFleet .= "`fleet_resource_darkmatter` = '". $TransDarkmatter ."', ";
	...
	$QryUpdatePlanet .= "`darkmatter` = `darkmatter` - ". $TransDarkmatter ." ";
