...
function ShowAccountDataPage()
{
	...
	if (!empty($id_u))
	{
		...
		{
			foreach(array_merge($reslist['dmfunc'], $reslist['tech']) as $ID)
			{
			...
			$techoffi	= "";
			for($i = 0; $i < max(count($reslist['dmfunc']), count($reslist['tech'])); $i++)
			{
			...
			while ($PlanetsWhile	= $db->fetch_array($PlanetsQuery))
			{
				...
				{	
					...
					<tr>
						...
						.(allowedTo('ShowQuickEditorPage') ? "<th><a href=\"javascript:openEdit('".$PlanetsWhile['id']."', 'planet');\" border=\"0\"><img src=\"./styles/images/Adm/GO.png\" title=".$LNG['se_search_edit']."></a></td>" : "").
					"</tr>";
					...
					<tr>
						...
						<td><a title=\"".pretty_number($PlanetsWhile['norio'])."\">".shortly_number($PlanetsWhile['norio'])."</a></td>
