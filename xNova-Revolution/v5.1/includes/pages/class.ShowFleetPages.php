...
class ShowFleetPages extends FleetFunctions
{
	public static function ShowFleetPage()
	{
		...
		$template->assign_vars(array(
			...
			'AKSPage'				=> $AKSPage,
			'bonus_attack'			=> $USER[$resource[109]] * 10 + ((TIMESTAMP - $USER[$resource[700]] <= 0) ? (100 * $ExtraDM[700]['add']) : 0) * 5,
			'bonus_defensive'		=> $USER[$resource[110]] * 10 + ((TIMESTAMP - $USER[$resource[701]] <= 0) ? (100 * $ExtraDM[701]['add']) : 0) * 5,
			'bonus_shield'			=> $USER[$resource[111]] * 10 + ((TIMESTAMP - $USER[$resource[700]] <= 0) ? (100 * $ExtraDM[700]['add']) : 0) * 5,
	...
	public static function ShowFleet2Page()
	{
		...
		{
			$GetInfoPlanet 			= $db->uniquequery("SELECT `id_owner`, `der_metal`, `der_crystal`, `der_norio` FROM `".PLANETS."` WHERE `galaxy` = ".$TargetGalaxy ." AND `system` = ".$TargetSystem." AND `planet` = ".$TargetPlanet." AND `planet_type` = '1';");
