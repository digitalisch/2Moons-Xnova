	...
	function TargetEvent()
	{
		...
		foreach ($defense as $fleetID => $defender)
		{
			...
			else
			{
				...
				$SQL .= $fleetArray;
				$SQL .= "`metal` = `metal` - '".floattostring($steal['metal'])."', `crystal` = `crystal` - '".floattostring($steal['crystal'])."',`deuterium` = `deuterium` - '".floattostring($steal['deuterium'])."', `norio` = `norio` - '".floattostring($steal['norio'])."' WHERE `id` = '".$this->_fleet['fleet_end_id']."';";
			}
		}
		...
		$SQL .= "`der_norio` = `der_norio` + '".$ShootNorio."' ";
		$SQL .= "WHERE ";
		...
		$SQL .= "`kbnorio` = kbnorio + ".floattostring($result['debree']['att'][2]+$result['debree']['def'][2]).", ";
