 ...
 class GalaxyRows
{
	...
	public function GalaxyRowDebris($GalaxyRowPlanet)
	{
		...
		$GRecNeeded = min(ceil(($GalaxyRowPlanet['der_metal'] + $GalaxyRowPlanet['der_crystal'] + $GalaxyRowPlanet['der_norio']) / $pricelist[219]['capacity']), $PLANET[$resource[219]]);
		$RecNeeded 	= min(ceil(max($GalaxyRowPlanet['der_metal'] + $GalaxyRowPlanet['der_crystal'] + $GalaxyRowPlanet['der_norio'] - ($GRecNeeded * $pricelist[219]['capacity']), 0) / $pricelist[209]['capacity']), $PLANET[$resource[209]]);

		$Result = array(
			'metal'			=> pretty_number($GalaxyRowPlanet["der_metal"]),
			'crystal'		=> pretty_number($GalaxyRowPlanet["der_crystal"]),
			'norio'		    => pretty_number($GalaxyRowPlanet["der_norio"]),
			'RecSended'		=> $RecNeeded,			
