...
function calculateAttack(&$attackers, &$defenders, $FleetTF, $DefTF)
{
	...
	$ARES 	= $DRES = array('metal' => 0, 'crystal' => 0, 'norio' => 0);
	$ROUND	= array();
	$RF		= array();
	...
	foreach ($attackers as $fleetID => $attacker) 
	{
		foreach ($attacker['detail'] as $element => $amount) 
		{
			...
			$ARES['norio'] 	    += $pricelist[$element]['norio'] * $amount;
		}
	}
	...
	$TRES['attacker']	= $ARES['metal'] + $ARES['crystal'] + $ARES['norio'];
	...
	for ($ROUNDC = 0; $ROUNDC < MAX_ATTACK_ROUNDS; $ROUNDC++) 
	{
		...
		foreach ($attackers as $fleetID => $attacker) {
			...
			$attTech	= (1 + (0.1 * $attacker['user']['military_tech']) + ((TIMESTAMP - $attacker['user'][$resource[700]] <= 0) ? $ExtraDM[700]['add'] : 0)); //attaque
			$defTech	= (1 + (0.1 * $attacker['user']['defence_tech']) + ((TIMESTAMP - $attacker['user'][$resource[701]] <= 0) ? $ExtraDM[701]['add'] : 0)); //bouclier
			$shieldTech = (1 + (0.1 * $attacker['user']['shield_tech']) + ((TIMESTAMP - $attacker['user'][$resource[701]] <= 0) ? $ExtraDM[701]['add'] : 0)); //coque
		...
		foreach ($defenders as $fleetID => $defender) {
			...
			$attTech	= (1 + (0.1 * $defender['user']['military_tech']) + ((TIMESTAMP - $defender['user'][$resource[700]] <= 0) ? $ExtraDM[700]['add'] : 0)); //attaquue
			$defTech	= (1 + (0.1 * $defender['user']['defence_tech']) + ((TIMESTAMP - $defender['user'][$resource[701]] <= 0) ? $ExtraDM[701]['add'] : 0)); //bouclier
			$shieldTech = (1 + (0.1 * $defender['user']['shield_tech']) + ((TIMESTAMP - $defender['user'][$resource[701]] <= 0) ? $ExtraDM[701]['add'] : 0)); //coque
	...
	foreach ($attackers as $fleetID => $attacker) {					   // flotte attaquant en CDR
		foreach ($attacker['detail'] as $element => $amount) {
		    ...
		    $TRES['attacker'] -= $pricelist[$element]['norio'] * $amount ;
	...
	$DRESDefs = array('metal' => 0, 'crystal' => 0, 'norio' => 0);
	...
	foreach ($defenders as $fleetID => $defender) {
		foreach ($defender['def'] as $element => $amount) {
			if ($element < 300) {							// flotte defenseur en CDR
				...
				$DRESDefs['norio'] += $pricelist[$element]['norio'] * ($lost - $giveback) ;
			}
		}
	}
