function ReBuildView() {
        ...
        $.each(data.build, function(index, val) {
                if (index == 0) {
						...
						HTML    += '<center>';
						HTML    += '<img src="styles/theme/gow/gebaeude/'+val.element+'.png" width="150" height="150" />';
					    ...
					    HTML    += '<img class="tooltip" name="'+val.name+'" src="styles/theme/gow/gebaeude/'+val.element+'.png" width="40" height="40" /><span class="encola_c">'+data.nivel+' '+val.level+'</span> '+val.mode+' ';
                        HTML    += '<a href="game.php?page=buildings&amp;cmd=remove&amp;listid='+(index+1)+'"><div class="cancelar_c_small"></div></a>';
						HTML    += '</div>';
						HTML    += '</div>';
						HTML    += '<div class="misiones_footer"></div>';
                }
