...
{	
	...
	public function __construct()
	{
		...
		foreach($reslist['allow'][$PLANET['planet_type']] as $ID => $Element)
		{
			...
			$BulidLink = ($HaveRessources == true) ? '<a href="game.php?page=buildings&amp;cmd=insert&amp;building='.$Element.'"><img class="tooltip" name="<table><td>'.(($PLANET['b_building'] != 0) ? $LNG['bd_add_to_list'] : (($NextBuildLevel == 1) ? $LNG['bd_build'] : $LNG['bd_build_next_level'] . $NextBuildLevel)).'</td></table>" src="styles/theme/gow/imagenes/navegacion/construir.gif" /></a>' : '<img class="tooltip" name="<table><td>'.(($NextBuildLevel == 1) ? $LNG['no_recursos'] : $LNG['bd_build_next_level'] . $NextBuildLevel).'</td></table>" src="styles/theme/gow/imagenes/navegacion/construir_red.gif" />';
			elseif ($RoomIsOk && !$CanBuildElement)
				$BulidLink = '<img class="tooltip" name="<table><td>'.(($NextBuildLevel == 1) ? $LNG['bd_build'] : $LNG['bd_build_next_level'] . $NextBuildLevel) .'</td></table>" src="styles/theme/gow/imagenes/navegacion/construir_red.gif" />';
			else
				$BulidLink = '<img class="tooltip" name="<table><td>'.$LNG['bd_no_more_fields'].'</td></table>" src="styles/theme/gow/imagenes/navegacion/construir_red.gif" />'; 
			if (($Element == 6 || $Element == 31) && $USER['b_tech'] > TIMESTAMP)
				$BulidLink = '<img class="tooltip" name="<table><td>'.$LNG['bd_working'].'</td></table>" src="styles/theme/gow/imagenes/navegacion/construir_red.gif" />';
		...
		if ($PLANET['b_building'] != 0)
		{
			...
			$template->assign_vars(array(
				'data'				=> json_encode(array('bd_cancel' => $LNG['bd_cancel'], 'bd_continue' => $LNG['bd_continue'], 'bd_finished' => $LNG['bd_finished'], 'build' => $Queue, 'nivel' => $LNG['bd_lvl'] )),
			));
		}
