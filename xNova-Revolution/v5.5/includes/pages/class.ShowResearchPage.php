	...
	public function __construct()
	{
		...
		foreach($reslist['tech'] as $ID => $Element)
		{
			...
			if(isset($pricelist[$Element]['max']) && $USER[$resource[$Element]] >= $pricelist[$Element]['max']) {
				$TechnoLink  =	'<img class="tooltip" name="<table><td>'.$LNG['bd_maxlevel'].'</td></table>" src="styles/theme/gow/imagenes/navegacion/construir_red.gif" />';
			} elseif(MAX_RESEACH_QUEUE_SIZE > 1) {
				$LevelToDo 	= 1 + $USER[$resource[$Element]];
				$TechnoLink = $CanBeDone && $bContinue && MAX_RESEACH_QUEUE_SIZE != count($TechQueue) ? '<a href="game.php?page=buildings&amp;mode=research&amp;cmd=insert&amp;tech='.$Element.'"><img class="tooltip" name="<table><td>'.(($USER['b_tech_id'] != 0) ? $LNG['bd_add_to_list'] : $LNG['bd_research'].(($LevelToDo == 1) ? '' : ' '.$LNG['bd_lvl'].' '.$LevelToDo)).'</td></table>" src="styles/theme/gow/imagenes/navegacion/construir.gif" /></a>' : '<img class="tooltip" name="<table><td>'.$LNG['bd_research'].(($LevelToDo == 1) ? '' : ' '.$LNG['bd_lvl'].' '.$LevelToDo).'</td></table>" src="styles/theme/gow/imagenes/navegacion/construir_red.gif" />';
				
				if($USER['b_tech_id'] != 0) {
					$template->loadscript('researchlist.js');
					$template->execscript('ReBuildView();Techlist();');
					$ScriptInfo	= array('bd_cancel' => $LNG['bd_cancel'], 'bd_continue' => $LNG['bd_continue'], 'bd_finished' => $LNG['bd_finished'], 'build' => $TechQueue, 'nivel' => $LNG['bd_lvl'] );
				}
			} else {
				if ($USER['b_tech_id'] == 0) {
					$LevelToDo = 1 + $USER[$resource[$Element]];
