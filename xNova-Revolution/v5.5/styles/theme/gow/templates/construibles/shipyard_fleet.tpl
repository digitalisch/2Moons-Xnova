...
<div id="planeta_small" style="background-image: url(styles/theme/gow/adds/hangar.jpg);"></div>
   	...
   	<form action="" method="POST"> 
		...
		{foreach name=FleetList item=FleetListRow from=$FleetList}
			<li>
				...
				<br />
				{if $NotBuilding && $FleetListRow.IsAvailable}<div class="cantidad_flota_h"><input type="text" class="campo_comun" name="fmenge[{$FleetListRow.id}]" id="input_{$FleetListRow.id}" size="16" maxlength="{$maxlength}" value="0" tabindex="{$smarty.foreach.FleetList.iteration}" /></div>
				<div class="max_h"><input type="button" class="campo_comun" value="Max" onclick="$('#input_{$FleetListRow.id}').val('{$FleetListRow.GetMaxAmount}')" /></div>{/if}
				<div class="demas_c">
