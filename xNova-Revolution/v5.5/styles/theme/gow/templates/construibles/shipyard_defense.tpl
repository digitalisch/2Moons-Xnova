...
<div id="planeta_small" style="background-image: url(styles/theme/gow/adds/defensas.jpg);"></div>
   	...
   	<form action="" method="POST"> 
		...
		{foreach name=DefenseList item=DefenseListRow from=$DefenseList}
			<li>
				...
				<br />
				{if $NotBuilding && $DefenseListRow.IsAvailable}<div class="cantidad_flota_h"><input type="text" class="campo_comun" name="fmenge[{$DefenseListRow.id}]" id="input_{$DefenseListRow.id}" size="16" maxlength="{$maxlength}" value="0" tabindex="{$smarty.foreach.FleetList.iteration}" /></div>
				<div class="max_h"><input type="button" class="campo_comun" value="Max" onclick="$('#input_{$DefenseListRow.id}').val('{$DefenseListRow.GetMaxAmount}')" /></div>{/if}
				<div class="demas_c">
