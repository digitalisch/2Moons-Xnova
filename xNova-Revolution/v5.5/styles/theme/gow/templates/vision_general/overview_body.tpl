...
<div id="rangos_ov"><img class="tooltip" name="<img src=styles/theme/gow/imagenes/otros/rangos/{$rango}.png /><br /><h3>{$lvl_rg} {$rango}<br />{$nivel}</h3> " src="styles/theme/gow/imagenes/otros/rangos/{$rango}.png" width="70" height="70" /></div>
<div id="luna_ov">
{if $Moon}
<a href="game.php?page=overview&amp;cp={$Moon.id}&amp;re=0" class="tooltip" name="('{$Moon.name} ({$fcm_moon})')">
<img src="{$dpath}imagenes/luna.png" /></a>{else}<img src="{$dpath}imagenes/luna_o.png" />
{/if}
</div>	
<div id="escombros_ov">{$escombros}</div>
<div id="planeta_overview"> <img  src="{$dpath}planeten/{$planetimage}.jpg" /></div>
{if $is_news}
