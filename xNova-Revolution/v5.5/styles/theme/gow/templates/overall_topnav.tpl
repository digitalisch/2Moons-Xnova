<a href="?page=changelog" id="changelog_link" class="tipsStandard">Xnova v({$VERSION})</a>         
    	...
    	<ul id="recursos">
        	<li class="metal">
               <a style="cursor:help" class="tooltip" name="<h3>{$Metal}</h3><hr />{$Metal}: {pretty_number($metal)}&nbsp;&nbsp;&nbsp; <br /> {if $settings_tnstor}{$almacenes}: {$metal_max}{else}{$almacenes}: {$alt_metal_max}{/if}"> <img src="{$dpath}images/metal.jpg" /></a>
                <span class="valor">
						<span id="current_metal"><b>{$metales}</b></span>
				</span>
            </li>
        	<li class="cristal">
               <a style="cursor:help" class="tooltip" name="<h3>{$Crystal}</h3><hr />{$Crystal}: {pretty_number($crystal)}&nbsp;&nbsp;&nbsp; <br /> {if $settings_tnstor}{$almacenes}: {$crystal_max}{else}{$almacenes}: {$alt_crystal_max}{/if}"> <img src="{$dpath}images/cristal.jpg" /></a>
					<span class="valor">
						<span id="current_crystal"><b>{$cristales}</b></span>
                </span>
            </li>
        	<li class="deuterio">
                <a style="cursor:help" class="tooltip" name="<h3>{$Deuterium}</h3><hr />{$Deuterium}: {pretty_number($deuterium)}&nbsp;&nbsp;&nbsp; <br /> {if $settings_tnstor}{$almacenes}: {$deuterium_max}{else}{$almacenes}: {$alt_deuterium_max}{/if}"><img src="{$dpath}images/deuterio.jpg" /></a>
                <span class="valor">
						<span id="current_deuterium"><b>{$deuterios}</b></span>
               	</span>
            </li>
			<li class="norio">
				<a style="cursor:help" class="tooltip" name="<h3>{$Energy}</h3>"><img src="{$dpath}images/energia.jpg" /></a>
					<span class="valor">
						<span><b>{$energy}Mw</b></span>
                </span>
            </li>
        	<li class="energia">
				<a style="cursor:help" class="tooltip" name="<h3>{$Norio}</h3><hr />{$Norio}: {pretty_number($norio)}&nbsp;&nbsp;&nbsp; <br /> {if $settings_tnstor}{$almacenes}: {$norio_max}{else}{$almacenes}: {$alt_norio_max}{/if}"><img src="{$dpath}images/norio.jpg" /></a>
                <span class="valor">
                    	<span id="current_norio"><b>{$norios}</b></span>
                </span>
            </li>
        	<li class="materia_oscura">
			<a href="game.php?page=bonus" class="tooltip" name="{$Darkmatter}: {$darkmatter}"><img src="{$dpath}images/materia.png" /></a>	
                <span class="valor">
